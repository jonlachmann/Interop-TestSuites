...
public class TestSuiteBase : TestClassBase
{
    ...
    {
        base.TestInitialize();
        if (ASAIRSAdapter == null)
        {
            ASAIRSAdapter = Site.GetAdapter<IMS_ASAIRSAdapter>();
        }
    ...
    {
        ASAIRSAdapter.SwitchUser(userInformation.UserName, userInformation.UserPassword, userInformation.UserDomain);
    ...
    protected string GetInitialSyncKey(string collectionId)
    {
        ...
        while (syncResult.Status == 12)
        {
            ...
            syncResult = ASAIRSAdapter.Sync(syncRequest);
        }
    ...
    protected DataStructures.Sync GetAllContentItem(string subject, string collectionId)
    {
        ...
        var item = GetSyncResult(subject, collectionId, null, bodyPreference, bodyPartPreference);

        Site.Assert.IsNotNull(item.Email.Body, "The Body element should be included in Sync command response when the BodyPreference element is specified in Sync command request.");
    ...
    protected DataStructures.Sync GetSyncResult(string subject, string collectionId, object[] commands, Request.BodyPreference[] bodyPreferences, Request.BodyPartPreference[] bodyPartPreferences)
    {
        ...
        var request = TestSuiteHelper.CreateSyncRequest(GetInitialSyncKey(collectionId), collectionId, commands, bodyPreferences, bodyPartPreferences);
    ...
    protected DataStructures.ItemOperations GetItemOperationsResult(string collectionId, string serverId, string fileReference, Request.BodyPreference[] bodyPreferences, Request.BodyPartPreference[] bodyPartPreferences, DeliveryMethodForFetch? deliveryMethod)
    {
        ...
        var request = TestSuiteHelper.CreateItemOperationsRequest(collectionId, serverId, fileReference, bodyPreferences, bodyPartPreferences);
        ...
        {
            Thread.Sleep(waitTime);
            var itemOperationsStore = ASAIRSAdapter.ItemOperations(request, deliveryMethod ?? DeliveryMethodForFetch.Inline);
    ...
    {
        var request = TestSuiteHelper.CreateSearchRequest(subject, collectionId, conversationId, bodyPreferences, bodyPartPreferences);

        var searchStore = ASAIRSAdapter.Search(request);
    ...
    protected void SendEmail(EmailType emailType, string subject, string body)
    {
        ...
        var sendMailResponse = ASAIRSAdapter.SendMail(request);
    ...
    {
        var folderSyncResponse = ASAIRSAdapter.FolderSync(Common.CreateFolderSyncRequest("0"));
    ...
    private void DeleteItemsInFolder(Collection<CreatedItems> createdItemsCollection)
    {
        ...
        {
            var syncKey = GetInitialSyncKey(createdItems.CollectionId);
            var request = TestSuiteHelper.CreateSyncRequest(syncKey, createdItems.CollectionId, null, null, null);
            var result = ASAIRSAdapter.Sync(request);
