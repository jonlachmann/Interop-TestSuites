...
public class S03_Attachment : TestSuiteBase
{
    ...
    public void MSASAIRS_S03_TC01_FileReference_ZeroLengthString()
    {
        ...
        SendEmail(EmailType.NormalAttachment, subject, body);
        #endregion
        ...
        var request = TestSuiteHelper.CreateItemOperationsRequest(User2Information.InboxCollectionId, syncItem.ServerId, string.Empty, null, null);

        var itemOperationsStore = ASAIRSAdapter.ItemOperations(request, DeliveryMethodForFetch.Inline);
    ...
    public void MSASAIRS_S03_TC02_NormalAttachment()
    {
        ...
        #region Verify requirements
        var syncItem = GetSyncResult(subject, User2Information.InboxCollectionId, null, null, null);
        VerifyMethodElementValue(syncItem.Email, 1);

        var itemOperationsItem = GetItemOperationsResult(User2Information.InboxCollectionId, syncItem.ServerId, null, null, null, null);
        VerifyMethodElementValue(itemOperationsItem.Email, 1);

        if (Common.IsRequirementEnabled(53, Site))
        {
            var searchItem = GetSearchResult(subject, User2Information.InboxCollectionId, null, null, null);
            VerifyMethodElementValue(searchItem.Email, 1);
        }
    ...
    public void MSASAIRS_S03_TC03_EmbeddedMessageAttachment()
    {
        ...
        var subject = Common.GenerateResourceName(Site, "Subject");
        var body = Common.GenerateResourceName(Site, "Body");
        SendEmail(EmailType.EmbeddedAttachment, subject, body);
        #endregion

        #region Verify requirements
        var syncItem = GetSyncResult(subject, User2Information.InboxCollectionId, null, null, null);
        VerifyMethodElementValue(syncItem.Email, 5);

        var itemOperationsItem = GetItemOperationsResult(User2Information.InboxCollectionId, syncItem.ServerId, null, null, null, null);
        VerifyMethodElementValue(itemOperationsItem.Email, 5);
        ...
        { 
            ...
            VerifyMethodElementValue(searchItem.Email, 5);
        }
        ...
        Site.CaptureRequirementIfIsTrue(
            ((Response.AttachmentsAttachment)syncItem.Email.Attachments.Items[0]).DisplayName.EndsWith(".eml", StringComparison.CurrentCultureIgnoreCase),
            2299,
    ...
    public void MSASAIRS_S03_TC04_OLEAttachment()
    {
        ...
        #region Verify requirements
        var syncItem = GetSyncResult(subject, User2Information.InboxCollectionId, null, null, null);
        VerifyMethodElementValue(syncItem.Email, 6);

        var itemOperationsItem = GetItemOperationsResult(User2Information.InboxCollectionId, syncItem.ServerId, null, null, null, null);
        VerifyMethodElementValue(itemOperationsItem.Email, 6);

        if (Common.IsRequirementEnabled(53, Site))
        {
            var searchItem = GetSearchResult(subject, User2Information.InboxCollectionId, itemOperationsItem.Email.ConversationId, null, null);
            VerifyMethodElementValue(searchItem.Email, 6);
        }
