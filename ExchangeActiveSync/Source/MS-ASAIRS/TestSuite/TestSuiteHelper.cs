...
internal static class TestSuiteHelper
{
    ...
    internal static SyncRequest CreateSyncRequest(string syncKey, string collectionId, object[] commands, Request.BodyPreference[] bodyPreferences, Request.BodyPartPreference[] bodyPartPreferences)
    {
        ...
        {
            foreach (var bodyPreference in bodyPreferences)
            {
        ...
        {
            foreach (var bodyPartPreference in bodyPartPreferences)
            {
    ...
    {
        var request = new ItemOperationsRequest { RequestData = new Request.ItemOperations() };
        var fetch = new Request.ItemOperationsFetch { Store = SearchName.Mailbox.ToString() };
        ...
        var items = new List<object>();
        var itemsElementName = new List<Request.ItemsChoiceType5>();
    ...
    internal static SearchRequest CreateSearchRequest(string query, string collectionId, string conversationId, Request.BodyPreference[] bodyPreferences, Request.BodyPartPreference[] bodyPartPreferences)
    {
        ...
        var items = new List<object>();
        var itemsElementName = new List<Request.ItemsChoiceType6>();
...
";
        ...
        if (type == EmailType.AttachOLE)
        {
            ...
            for (var i = 1; i < winmailData.Length / 76; i++)
            {
...
--_boundary_--";
    ...
    internal static Sync GetSyncAddItem(SyncStore syncStore, string subject)
    {
        Sync item = null;
    ...
    internal static Search GetSearchItem(SearchStore searchStore, string subject)
    {
    ...
    {
        var bytes = Encoding.ASCII.GetBytes(originalData);
