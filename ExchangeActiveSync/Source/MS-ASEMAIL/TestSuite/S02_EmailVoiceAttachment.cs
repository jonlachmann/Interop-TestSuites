...
public class S02_EmailVoiceAttachment : TestSuiteBase
{
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The UmAttDuration element is not supported when the ActiveSyncProtocolVersion is 12.1.");
        ...
        var firstVoiceFilePath = "testVoice.mp3";
        var secondVoiceFilePath = "secondTestVoice.mp3";
        var mailResult = SendVoiceMail(emailSubject, firstVoiceFilePath, secondVoiceFilePath);
        #endregion
        ...
        var messageClass = mailResult.Email.MessageClass;
        var verifyR804AndR811 = messageClass.StartsWith("IPM.Note.Microsoft.Voicemail", StringComparison.CurrentCulture) || messageClass.StartsWith("IPM.Note.RPMSG.Microsoft.Voicemail", StringComparison.CurrentCulture) || messageClass.StartsWith("IPM.Note.Microsoft.Missed.Voice", StringComparison.CurrentCulture);
        ...
        if (Common.IsRequirementEnabled(829, Site))
        {
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The UmCallerID element is not supported when the ActiveSyncProtocolVersion is 12.1.");

        #region Call SendMail command to send a mail with an electronic voice mail attachment
        var emailSubject = Common.GenerateResourceName(Site, "subject");
        var firstVoiceFilePath = "testVoice.mp3";
        var secondVoiceFilePath = "secondTestVoice.mp3";
        SendVoiceMail(emailSubject, firstVoiceFilePath, secondVoiceFilePath);
        #endregion

        #region Calls Sync command to update email
        var newUmCallerId = "77777777777";
        var umcallerIDElement = "<UmCallerID xmlns=\"Email2\">" + newUmCallerId + "</UmCallerID>";
        var statusCode = UpdateVoiceEmailWithInvalidData(umcallerIDElement);
        #endregion
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The UmUserNotes element is not supported when the ActiveSyncProtocolVersion is 12.1.");
        ...
        #region Calls Sync command to update email
        var umuserNotes = Common.GenerateResourceName(Site, "subject");
        var umuserNotesElement = "<UmUserNotes xmlns=\"Email2\" >" + umuserNotes + "</UmUserNotes>";
        var statusCode = UpdateVoiceEmailWithInvalidData(umuserNotesElement);
        #endregion
    ...
    {
        // Create mail content
        var senderEmail = Common.GetMailAddress(User1Information.UserName, User1Information.UserDomain);
        var receiverEmail = Common.GetMailAddress(User2Information.UserName, User2Information.UserDomain);
        var emailBody = Common.GenerateResourceName(Site, "content");
        var callNumber = "7125550123";

        // Create voice mail content mime
        var voiceMailMime = TestSuiteHelper.CreateVoiceAttachmentMime(
            senderEmail,
            receiverEmail,
            emailSubject,
            emailBody,
        ...
        var sendMailRequest = TestSuiteHelper.CreateSendMailRequest(clientId, false, voiceMailMime);
        ...
        // Sync changes
        var result = GetSyncResult(emailSubject, User2Information.InboxCollectionId, null);
        var emailItem = TestSuiteHelper.GetSyncAddItem(result, emailSubject);
        #endregion
    ...
    private string UpdateVoiceEmailWithInvalidData(string invalidElement)
    {
        ...
        // Sync changes
        var initSyncResult = InitializeSync(User2Information.InboxCollectionId);
        var syncChangeResult = SyncChanges(initSyncResult.SyncKey, User2Information.InboxCollectionId, null);
        var syncKey = syncChangeResult.SyncKey;
        var serverId = User2Information.InboxCollectionId;

        // Create normal Sync change request
        var changeData = TestSuiteHelper.CreateSyncChangeData(true, serverId, null, null);
        var syncRequest = TestSuiteHelper.CreateSyncChangeRequest(syncKey, User2Information.InboxCollectionId, changeData);

        // Calls Sync command to update email with invalid sync request
        var insertTag = "</ApplicationData>";
        var result = EMAILAdapter.InvalidSync(syncRequest, invalidElement, insertTag);
