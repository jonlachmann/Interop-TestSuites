...
public class S04_MeetingRequest : TestSuiteBase
{
    ...
    public void MSASEMAIL_S04_TC01_SyncAdd_CalendarItem()
    {
        ...
        var initialSync = InitializeSync(User1Information.CalendarCollectionId);
        var syncAddRequest = TestSuiteHelper.CreateSyncAddRequest(initialSync.SyncKey, User1Information.CalendarCollectionId, applicationData);
        ...
        var item = TestSuiteHelper.GetSyncAddItem(getChangeResult, subject);
        #endregion
    ...
    public void MSASEMAIL_S04_TC02_MeetingRequest_NoRecurrence()
    {
        ...
        if (Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site).Equals("16.0", StringComparison.CurrentCultureIgnoreCase) || Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site).Equals("16.1", StringComparison.CurrentCultureIgnoreCase))
        {
        ...
        var getChangeResult = GetSyncResult(subject, User1Information.CalendarCollectionId, null);
        var calendar = TestSuiteHelper.GetSyncAddItem(getChangeResult, subject);
        #endregion
        ...
        var calendarItem = calendar.Calendar;
        ...
        var getMeetingResult = GetSyncResult(subject, User2Information.InboxCollectionId, null);
        var item = TestSuiteHelper.GetSyncAddItem(getMeetingResult, subject);

        // Get the calendar item in attendee's calendar folder.
        var getCalendarResult = GetSyncResult(subject, User2Information.CalendarCollectionId, null);
        var calendarOfAttendee = TestSuiteHelper.GetSyncAddItem(getCalendarResult, subject);
        #endregion
        ...
        var reminderInSeconds = (ushort)(uint.Parse(reminder) * 60);
    ...
    public void MSASEMAIL_S04_TC04_MeetingRequest_Monthly_ByDay()
    {
        ...
        GetSyncResult(subject, User2Information.InboxCollectionId, null);
        var getMeetingRequest = GetSyncResult(subject, User2Information.InboxCollectionId, null);
        var emailItem = TestSuiteHelper.GetSyncAddItem(getMeetingRequest, subject);
    ...
    public void MSASEMAIL_S04_TC05_MeetingRequest_Monthly_ByWeek()
    {
        ...
        #region Verify requirements
        var meetingRequest = item.Email.MeetingRequest;
        ...
        var isVerifiedR650 = meetingRequest.Recurrences.Recurrence != null
        ...
        var isVerifiedR1067 = meetingRequest.Recurrences.Recurrence != null
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The MeetingMessageType element is not supported when the MS-ASProtocolVersion header is set to 12.1. MS-ASProtocolVersion header value is determined using Common PTFConfig property named ActiveSyncProtocolVersion.");
        Site.Assume.AreNotEqual<string>("14.0", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The MeetingMessageType element is not supported when the MS-ASProtocolVersion header is set to 14.0. MS-ASProtocolVersion header value is determined using Common PTFConfig property named ActiveSyncProtocolVersion.");

        #region Call Sync command with Add element to add a no recurrence meeting to the server.
        var subject = Common.GenerateResourceName(Site, "Subject");
        var attendeeEmail = Common.GetMailAddress(User4Information.UserName, User4Information.UserDomain);
        ...
        SwitchUser(User4Information, true);
        RecordCaseRelativeItems(User4Information.UserName, User4Information.InboxCollectionId, subject);
        RecordCaseRelativeItems(User4Information.UserName, User4Information.CalendarCollectionId, subject);
        #endregion

        #region Make sure the delegator receives the copy
        SwitchUser(User5Information, true);
        ...
        do
        {
            ...
            var initSyncResult = InitializeSync(User5Information.InboxCollectionId);
            syncItemResult = SyncChanges(initSyncResult.SyncKey, User5Information.InboxCollectionId, null);
        ...
        Site.Assert.IsNotNull(
            ...
            counter);
        RecordCaseRelativeItems(User5Information.UserName, User5Information.InboxCollectionId, subject);

        if (Common.IsRequirementEnabled(1491, Site))
        {
            ...
            {
                UserResponse = 1,
                CollectionId = User5Information.InboxCollectionId,
        ...
        var syncResult = GetSyncResult(subject, User4Information.InboxCollectionId, null);
        var meetingRequestEmail = TestSuiteHelper.GetSyncAddItem(syncResult, subject);
        #endregion
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The Sender element is not supported when the MS-ASProtocolVersion header is set to 12.1. MS-ASProtocolVersion header value is determined using Common PTFConfig property named ActiveSyncProtocolVersion.");
        ...
        GetSyncResult(subject, User2Information.CalendarCollectionId, null);
        // Sync mailbox changes
        var syncResult = GetSyncResult(subject, User2Information.InboxCollectionId, null);
        var meetingRequest = TestSuiteHelper.GetSyncAddItem(syncResult, subject);

        // Call SmartFoward to forward the meeting request to another user
        var from = meetingRequest.Email.From;
        var to = Common.GetMailAddress(User3Information.UserName, User3Information.UserDomain);
        var forwardContent = Common.GenerateResourceName(Site, "forward: body");

        var forwardMime = TestSuiteHelper.CreatePlainTextMime(from, to, string.Empty, string.Empty, subject, forwardContent);

        var forwardRequest = TestSuiteHelper.CreateSmartForwardRequest(User2Information.InboxCollectionId, meetingRequest.ServerId, forwardMime);
        var response = EMAILAdapter.SmartForward(forwardRequest);
        ...
        RecordCaseRelativeItems(User3Information.UserName, User3Information.InboxCollectionId, subject);
        RecordCaseRelativeItems(User3Information.UserName, User3Information.CalendarCollectionId, subject);
        RecordCaseRelativeItems(User3Information.UserName, User3Information.DeletedItemsCollectionId, subject);
        #endregion

        #region Call Sync command to get the forwarded meeting request
        GetSyncResult(subject, User3Information.CalendarCollectionId, null);
        // Sync mailbox changes
        syncResult = GetSyncResult(subject, User3Information.InboxCollectionId, null);
        var forwardedMeetingRequest = TestSuiteHelper.GetSyncAddItem(syncResult, subject);
        #endregion
    ...
    public void MSASEMAIL_S04_TC10_ExceptionToARecurringMeeting()
    {
        ...
        #region Verify requirements
        var globalObjIdBytes = Convert.FromBase64String(item.Email.MeetingRequest.GlobalObjId);
        var totalLength = globalObjIdBytes.Length;
        var pos = 0;
        ...
        var instDate = TestSuiteHelper.BytesToHex(globalObjIdBytes, pos, 4);
        var month = Convert.ToInt32(instDate.Substring(4, 2), 16);
        ...
        var date = Convert.ToInt32(instDate.Substring(6, 2), 16);
    ...
    public void MSASEMAIL_S04_TC11_MeetingRequest_FullUpdate()
    {
        ...
        var calendarUID = Guid.NewGuid().ToString();
        ...
        var newCalendarItem = resultItem.Calendar;
        ...
        SendMeetingRequest(subject, newCalendarItem);
        ...
        var getMeetingResult = GetSyncResult(subject, User2Information.InboxCollectionId, null);
        var emailItem = TestSuiteHelper.GetSyncAddItem(getMeetingResult, subject);
        #endregion
        ...
        var getFirstMeetingResult = GetSyncResult(subject, User2Information.DeletedItemsCollectionId, null);
        var firstMeetingRequestEmail = TestSuiteHelper.GetSyncAddItem(getFirstMeetingResult, subject);
    ...
    public void MSASEMAIL_S04_TC12_OutdatedMeetingRequest()
    {
        ...
        var oldEmailSubject = Common.GenerateResourceName(Site, "subject");
        var organizerEmailAddress = Common.GetMailAddress(User1Information.UserName, User1Information.UserDomain);
        var attendeeEmailAddress = Common.GetMailAddress(User2Information.UserName, User2Information.UserDomain);
        var oldCalendar = TestSuiteHelper.CreateDefaultCalendar(oldEmailSubject, organizerEmailAddress, attendeeEmailAddress);
        ...
        SendMeetingRequest(oldEmailSubject, oldCalendar);

        // Ensure the old meeting request email has arrived at recipient's inbox folder
        // Switch to user2 mailbox
        SwitchUser(User2Information, true);
        GetSyncResult(oldEmailSubject, User2Information.InboxCollectionId, null);
        #endregion
        ...
        var newEmailSubject = Common.GenerateResourceName(Site, "subject");
        var newCalendar = TestSuiteHelper.CreateDefaultCalendar(newEmailSubject, organizerEmailAddress, attendeeEmailAddress);
        ...
        SendMeetingRequest(newEmailSubject, newCalendar);

        // Sync changes and get email item with oldEmailSubject
        // Switch to user2 mailbox
        SwitchUser(User2Information, false);
        RecordCaseRelativeItems(User2Information.UserName, User2Information.InboxCollectionId, oldEmailSubject, newEmailSubject);
        RecordCaseRelativeItems(User2Information.UserName, User2Information.CalendarCollectionId, oldEmailSubject);
        #endregion

        #region Call Sync command to get the meeting request
        // Sync mailbox changes
        var syncChangeResult = GetSyncResult(newEmailSubject, User2Information.InboxCollectionId, null);
        var emailItem = TestSuiteHelper.GetSyncAddItem(syncChangeResult, newEmailSubject);
        #endregion
    ...
    {
        var isGlobalObjIdSupported = Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site).Equals("12.1")
        ...
        #region Call Sync command with Add element to add a meeting to the server
        var calendarSubject = Common.GenerateResourceName(Site, "subject");
        var organizerAddress = Common.GetMailAddress(User1Information.UserName, User1Information.UserDomain);
        var attendeeAddress = Common.GetMailAddress(User2Information.UserName, User2Information.UserDomain);
        var vcalUid = Guid.NewGuid().ToString();

        var startTime = Convert.ToDateTime(DateTime.Now.Date.ToString("yyyy-MM-dd HH:mm:ss")).ToUniversalTime();
        var endTime = startTime.AddDays(1);
        var newCalendar = CreateDefaultCalendar(calendarSubject, organizerAddress, attendeeAddress, vcalUid, null, startTime, endTime);

        // Record the calendar item that created in calendar folder of user1
        RecordCaseRelativeItems(User1Information.UserName, User1Information.CalendarCollectionId, calendarSubject);
        #endregion

        #region The sender calls SendMail command to send to meeting request to the recipient.
        newCalendar.AllDayEvent = 1;
        SendMeetingRequest(calendarSubject, newCalendar);
        #endregion

        #region The recipient calls Sync method to synchronize the meeting request on the server.
        SwitchUser(User2Information, true);
        var syncChangeResult = GetSyncResult(calendarSubject, User2Information.InboxCollectionId, null);
        var item = TestSuiteHelper.GetSyncAddItem(syncChangeResult, calendarSubject);

        RecordCaseRelativeItems(User2Information.UserName, User2Information.InboxCollectionId, calendarSubject);
        RecordCaseRelativeItems(User2Information.UserName, User2Information.CalendarCollectionId, calendarSubject);
        #endregion
        ...
        var byteCount = BitConverter.ToInt32(globalObjIdBytes, pos);
        var uidLength = byteCount - 13;
        ...
        var isVerifiedR20021 = totalLength - pos >= uidLength;
        ...
        var isVerifiedR20016 = (pos == globalObjIdBytes.Length - 1) && (globalObjIdBytes[pos] == 0x00);
    ...
    public void MSASEMAIL_S04_TC14_GlobalObjIdFormat_OutlookID()
    {
        ...
        var outlookID = TestSuiteHelper.GenerateOutlookID(creationTime);

        var newCalendar = CreateDefaultCalendar(calendarSubject, organizerAddress, attendeeAddress, outlookID, creationTime, null, null);
    ...
    public void MSASEMAIL_S04_TC15_RecurrenceIdExistsForException()
    {
        ...
        var attendeeEmail = Common.GetMailAddress(User2Information.UserName, User2Information.UserDomain);
        var attendees = TestSuiteHelper.CreateAttendees(attendeeEmail);
        ...
        var elementsToValueMap = new Dictionary<Request.ItemsChoiceType8, object>();
        ...
        RecordCaseRelativeItems(User1Information.UserName, User1Information.CalendarCollectionId, recurrenceCalendarSubject);
        #endregion

        #region Call Sync command to get the added calendar item
        var getChangeResult = GetSyncResult(recurrenceCalendarSubject, User1Information.CalendarCollectionId, null);
        var calendar = TestSuiteHelper.GetSyncAddItem(getChangeResult, recurrenceCalendarSubject);
        #endregion

        #region Call SendMail command to send the meeting request to attendee
        var calendarItem = calendar.Calendar;
        SendMeetingRequest(recurrenceCalendarSubject, calendarItem);
        #endregion
        ...
        // Sync mailbox changes
        var syncChangeResult = GetSyncResult(recurrenceCalendarSubject, User2Information.InboxCollectionId, null);
        var meetingRequestEmail = TestSuiteHelper.GetSyncAddItem(syncChangeResult, recurrenceCalendarSubject);
        ...
        RecordCaseRelativeItems(User2Information.UserName, User2Information.DeletedItemsCollectionId, recurrenceCalendarSubject);
        RecordCaseRelativeItems(User2Information.UserName, User2Information.CalendarCollectionId, recurrenceCalendarSubject);
        #endregion
        ...
        var syncRequest = TestSuiteHelper.CreateSyncChangeRequest(getChangeResult.SyncKey, User1Information.CalendarCollectionId, appDataChange);
        ...
        getChangeResult = GetSyncResult(recurrenceCalendarSubject, User1Information.CalendarCollectionId, null);
        calendar = TestSuiteHelper.GetSyncAddItem(getChangeResult, recurrenceCalendarSubject);

        var icalendarFormatContent = TestSuiteHelper.CreateiCalendarFormatCancelContent(calendar.Calendar);
        ...
        var sendMailResponse = EMAILAdapter.SendMail(sendMailRequest);

        Site.Assert.AreEqual<string>(
            string.Empty,
            sendMailResponse.ResponseDataXML,
            "The server should return an empty xml response data to indicate SendMail command success.");
        RecordCaseRelativeItems(User2Information.UserName, User2Information.InboxCollectionId, recurrenceCalendarSubject);
        #endregion

        #region Call Sync command to get the udpated meeting request
        SwitchUser(User2Information, false);
        syncChangeResult = GetSyncResult(recurrenceCalendarSubject, User2Information.InboxCollectionId, null);
        var item = TestSuiteHelper.GetSyncAddItem(syncChangeResult, recurrenceCalendarSubject);
        ...
        if (Common.IsRequirementEnabled(1493, Site))
        {
            // Add the debug information
    ...
    public void MSASEMAIL_S04_TC17_BusyStatusIsWorkingElsewhere()
    {
        ...
        var iniSync = InitializeSync(User1Information.CalendarCollectionId);
        var syncAddRequest = TestSuiteHelper.CreateSyncAddRequest(iniSync.SyncKey, User1Information.CalendarCollectionId, applicationData);
