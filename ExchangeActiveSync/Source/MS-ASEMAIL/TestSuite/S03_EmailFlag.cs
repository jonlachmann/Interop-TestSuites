...
public class S03_EmailFlag : TestSuiteBase
{
    ...
    public void MSASEMAIL_S03_TC01_AddFlag_AllElements()
    {
        ...
        var getChangedResult = AddEmailFag(User2Information, getEmailItem, emailSubject, flag);
        item = TestSuiteHelper.GetSyncChangeItem(getChangedResult, item.ServerId);
        Site.Assert.IsNotNull(item, "The message with subject {0} should be found in the folder {1} of user {2}.", emailSubject, FolderType.Inbox.ToString(), User2Information.UserName);
        #endregion

        #region Verify requirements
        if (Common.IsRequirementEnabled(1007, Site))
        {
    ...
    public void MSASEMAIL_S03_TC02_AddFlag_InvalidStatus()
    {
        ...
        var emailItem = TestSuiteHelper.GetSyncAddItem(getEmailItem, emailSubject);

        // Add flag to the email item
        var updateResult = UpdateEmail(User2Information.InboxCollectionId, getEmailItem.SyncKey, null, emailItem.ServerId, flag, null);
    ...
    public void MSASEMAIL_S03_TC03_AddFlag_CompleteTimeAndDateCompleted()
    {
        ...
        #region Verify requirement
        if (Common.IsRequirementEnabled(336, Site))
        {
            // Add the debug information
    ...
    public void MSASEMAIL_S03_TC09_AddFlag_OptionalFlagTypeInMeetingRequest()
    {
        ...
        var calendar = CreateDefaultCalendar(emailSubject, organizerEmailAddress, attendeeEmailAddress, null, null, null, null);

        // Record the calendar item that created in calendar folder of user1
        RecordCaseRelativeItems(User1Information.UserName, User1Information.CalendarCollectionId, emailSubject);

        // Send a meeting request email to user2
        SendMeetingRequest(emailSubject, calendar);
        #endregion
        ...
        RecordCaseRelativeItems(User2Information.UserName, User2Information.CalendarCollectionId, emailSubject);
        #endregion
        ...
        var emailItem = TestSuiteHelper.GetSyncAddItem(syncChangeResult, emailSubject);
        #endregion
    ...
    public void MSASEMAIL_S03_TC10_AddFlag_OptionalFlagTypeInMeetingResponse()
    {
        ...
        var organizerEmailAddress = Common.GetMailAddress(User1Information.UserName, User1Information.UserDomain);
        var attendeeEmailAddress = Common.GetMailAddress(User2Information.UserName, User2Information.UserDomain);
        var calendar = TestSuiteHelper.CreateDefaultCalendar(emailSubject, organizerEmailAddress, attendeeEmailAddress);

        // Send meeting response reply email to organizer
        SendMeetingResponse(calendar);
        #endregion
    ...
    {
        var newFlag = new Request.Flag { Status = "0" };
    ...
    public void MSASEMAIL_S03_TC15_UpdateEmailFlagMetadata()
    {
        ...
        var newFlag = new Request.Flag { Status = "2", FlagType = "for Follow Up" };
        ...
        var getUpdateResult = SyncChanges(getChangedResult.SyncKey, User2Information.InboxCollectionId, null);
        item = TestSuiteHelper.GetSyncChangeItem(getUpdateResult, item.ServerId);
        Site.Assert.IsNotNull(item, "The email item with subject '{0}' should be found in the {1} folder of user {2}.", emailSubject, FolderType.Inbox.ToString(), User2Information.UserName);
        Site.Assert.AreEqual<byte>(
            1,
        ...
        #region Verify requirements
        if (Common.IsRequirementEnabled(1021, Site))
        {
    ...
    public void MSASEMAIL_S03_TC16_UpdateEmailFlag_Error()
    {
        ...
        #region Call Sync command with Change element to update the email flag.
        var newFlag = new Request.Flag { Status = "2" };

        var updateResult = UpdateEmail(User2Information.InboxCollectionId, getChangedResult.SyncKey, null, item.ServerId, newFlag, null);
        Site.Assert.AreEqual<int>(
            6,
            int.Parse(updateResult.ChangeResponses[0].Status),
    ...
    public void MSASEMAIL_S03_TC17_IncludedTwoDueDateElement()
    {
        ...
        var insertTag = "<DueDate xmlns=\"Tasks\">";
        var dueDateElement = "<DueDate xmlns=\"Tasks\">" + DateTime.UtcNow.AddDays(20).ToString("yyyy-MM-ddThh:mm:ss.fffZ") + "</DueDate>";

        // Get status code from the update flag result
        var statusCode = FlagEmailWithAddtionalFlagElement(dueDateElement, insertTag);
        #endregion

        #region Verify requirement
        if (Common.IsRequirementEnabled(581, Site))
        {
            // Add the debug information
    ...
    public void MSASEMAIL_S03_TC18_IncludedTwoOrdinalDateElement()
    {
        ...
        var secondOrdinalDate = DateTime.Now;
        var insertTag = "<OrdinalDate xmlns=\"Tasks\">";
        var ordinalDateElement = "<OrdinalDate xmlns=\"Tasks\">" + secondOrdinalDate.ToString("yyyy-MM-ddThh:mm:ss.fffZ") + "</OrdinalDate>";
        var statusCode = FlagEmailWithAddtionalFlagElement(ordinalDateElement, insertTag);
        #endregion

        #region Verify requirement
        if (Common.IsRequirementEnabled(634, Site))
        {
            // Add the debug information
    ...
    public void MSASEMAIL_S03_TC19_IncludedTwoReminderSetElement()
    {
        ...
        var insertTag = "<ReminderSet xmlns=\"Tasks\">";
        var reminderSetElement = "<ReminderSet xmlns=\"Tasks\">" + secondReminderSet.ToString() + "</ReminderSet>";

        // Get status code from the update flag operation result
        var statusCode = FlagEmailWithAddtionalFlagElement(reminderSetElement, insertTag);
        #endregion
    ...
    public void MSASEMAIL_S03_TC20_IncludedTwoReminderTimeElement()
    {
        ...
        var secondRemindTime = DateTime.Now.AddDays(12);
        var insterTag = "<ReminderTime xmlns=\"Tasks\">";
        var reminderTimeElement = "<ReminderTime xmlns=\"Tasks\">" + secondRemindTime.ToString("yyyy-MM-ddThh:mm:ss.fffZ") + "</ReminderTime>";

        // After Send mail then update mail
        var statusCode = FlagEmailWithAddtionalFlagElement(reminderTimeElement, insterTag);
        #endregion

        #region Verify requirement
        if (Common.IsRequirementEnabled(706, Site))
        {
            // Add the debug information
    ...
    public void MSASEMAIL_S03_TC21_IncludedTwoStartDateElement()
    {
        ...
        var secondStartDate = DateTime.Now;

        // Additional startDate elements
        var insertTag = "<StartDate xmlns=\"Tasks\">";
        var reminderTimeElement = "<StartDate xmlns=\"Tasks\">" + secondStartDate.ToString("yyyy-MM-ddThh:mm:ss.fffZ") + "</StartDate>";

        // Send mail and update email flag
        var statusCode = FlagEmailWithAddtionalFlagElement(reminderTimeElement, insertTag);
        #endregion

        #region Verify requirement
        if (Common.IsRequirementEnabled(746, Site))
        {
            // Add the debug information
    ...
    public void MSASEMAIL_S03_TC22_IncludedTwoStatusElement()
    {
        ...
        var insertTag = "<Status>";
        var sencondStatusElement = "<Status>" + secondStatus + "</Status>";

        // Send email and update email
        var statusCode = FlagEmailWithAddtionalFlagElement(sencondStatusElement, insertTag);
        #endregion

        #region Verify requirement
        if (Common.IsRequirementEnabled(761, Site))
        {
            // Add the debug information
    ...
    public void MSASEMAIL_S03_TC23_IncludedTwoSubOrdinalDateElement()
    {
        ...
        var secondSubOrdinalDate = DateTime.Now.ToString();
        var insertTag = "<SubOrdinalDate xmlns=\"Tasks\">";
        var subOrdinalDateElement = "<SubOrdinalDate xmlns=\"Tasks\">" + secondSubOrdinalDate + "</SubOrdinalDate>";

        // Send email ,then update email flag
        var statusCode = FlagEmailWithAddtionalFlagElement(subOrdinalDateElement, insertTag);
        #endregion

        #region Verify requirement
        if (Common.IsRequirementEnabled(777, Site))
        {
            // Add the debug information
    ...
    public void MSASEMAIL_S03_TC24_IncludedTwoUtcDueDateElement()
    {
        ...
        var secondUtcDueDate = DateTime.UtcNow.AddDays(20);
        var insertTag = "<UtcDueDate xmlns=\"Tasks\">";
        var secondUteStartDateElement = "<UtcDueDate xmlns=\"Tasks\">" + secondUtcDueDate.ToString("yyyy-MM-ddThh:mm:ss.fffZ") + "</UtcDueDate>";
        var statusCode = FlagEmailWithAddtionalFlagElement(secondUteStartDateElement, insertTag);
        #endregion

        #region Verify requirement
        if (Common.IsRequirementEnabled(858, Site))
        {
            // Add the debug information
    ...
    public void MSASEMAIL_S03_TC25_IncludedTwoUtcStartDateElement()
    {
        ...
        var secondUteStartDate = DateTime.UtcNow;
        var insertTag = "<UtcStartDate xmlns=\"Tasks\">";
        var secondUteStartDateElement = "<UtcStartDate xmlns=\"Tasks\">" + secondUteStartDate.ToString("yyyy-MM-ddThh:mm:ss.fffZ") + "</UtcStartDate>";
        ...
        #region Verify requirement
        if (Common.IsRequirementEnabled(869, Site))
        {
            // Add the debug information
    ...
    public void MSASEMAIL_S03_TC28_ClearTaskFlag_Status0()
    {
        ...
        // Get update result from server 
        var getClearResult = SyncChanges(getChangedResult.SyncKey, User2Information.InboxCollectionId, null);
    ...
    public void MSASEMAIL_S03_TC30_MarkTaskComplete()
    {
        ...
        // Get the email item
        getEmailItem = GetSyncResult(emailSubject, User2Information.InboxCollectionId, null);
        item = TestSuiteHelper.GetSyncAddItem(getEmailItem, emailSubject);

        // Add flag to an email item
        getChangedResult = AddEmailFag(User2Information, getEmailItem, emailSubject, flag);
        Site.Assert.AreEqual<byte>(
            1,
            getChangedResult.CollectionStatus,
            "The server should return a Status 1 in the Sync command response to indicate sets flag success.");
    ...
    private string FlagEmailWithAddtionalFlagElement(string additionalFlagElement, string insertTag)
    {
        ...
        // Send email to user2
        SendPlaintextEmail(emailSubject, null, null);
        ...
        var result = UpdateEmailWithMoreData(User2Information.InboxCollectionId, syncChangeResult.SyncKey, true, emailItem.ServerId, flag, null, additionalFlagElement, insertTag);
    ...
    private SyncStore AddEmailFag(UserInformation userInformation, SyncStore getEmailItem, string emailSubject, Request.Flag flag)
    {
        ...
        // Add flag to the email item
        UpdateEmail(userInformation.InboxCollectionId, getEmailItem.SyncKey, null, emailItem.ServerId, flag, null);

        // Get the server changes of adding flag and category
        return SyncChanges(getEmailItem.SyncKey, userInformation.InboxCollectionId, null);
    }
    #endregion
}