...
public class S01_Email : TestSuiteBase
{
    ...
    public void MSASEMAIL_S01_TC01_SyncMail_OnlyMetadata()
    {
        ...
        SendPlaintextEmail(emailSubject, string.Empty, string.Empty);
        #endregion
        ...
        var syncChangeResult = GetSyncResult(emailSubject, User2Information.InboxCollectionId, null);
        var item = TestSuiteHelper.GetSyncAddItem(syncChangeResult, emailSubject);
        #endregion
    ...
    public void MSASEMAIL_S01_TC02_SyncMail_MetadataAndBody()
    {
        ...
        // Get the new added email item
        var syncChangeResult = GetSyncResult(emailSubject, User2Information.InboxCollectionId, bodyPreference);
        var item = TestSuiteHelper.GetSyncAddItem(syncChangeResult, emailSubject);
        #endregion
        ...
        if (Common.IsRequirementEnabled(439, Site))
        {
            ...
            Site.CaptureRequirementIfIsTrue(
                ((XmlElement)EMAILAdapter.LastRawResponseXml).InnerXml.Contains("<Flag xmlns=\"Email\" />"),
                439,
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The Categories element is not supported when the ActiveSyncProtocolVersion is 12.1.");
        ...
        // Get the email item
        var getEmailItem = GetSyncResult(emailSubject, User2Information.InboxCollectionId, null);
        var item = TestSuiteHelper.GetSyncAddItem(getEmailItem, emailSubject);

        // Get the result of adding flag and categories
        var getChangedResult = AddFlagAndCategories(User2Information.InboxCollectionId, getEmailItem.SyncKey, item.ServerId);
        ...
        UpdateEmail(User2Information.InboxCollectionId, getChangedResult.SyncKey, true, item.ServerId, newFlag, null);

        // Synchronize the update result with server
        var updateResult = SyncChanges(getChangedResult.SyncKey, User2Information.InboxCollectionId, null);
    ...
    public void MSASEMAIL_S01_TC04_SyncChange_RemoveCategories()
    {
        ...
        var category = Common.GenerateResourceName(Site, "Category");
        var categories = new Collection<string> { category };

        // Get the new added email item
        var syncChangeResult = GetSyncResult(emailSubject, User2Information.InboxCollectionId, null);
        var item = TestSuiteHelper.GetSyncAddItem(syncChangeResult, emailSubject);
        Site.Assert.IsNotNull(item, "The message with subject {0} should be found in the folder {1} of user {2}.", emailSubject, FolderType.Inbox.ToString(), User2Information.UserName);

        // Add categories to the email item
        UpdateEmail(User2Information.InboxCollectionId, syncChangeResult.SyncKey, true, item.ServerId, null, categories);

        var updateResult = SyncChanges(syncChangeResult.SyncKey, User2Information.InboxCollectionId, null);
        item = TestSuiteHelper.GetSyncChangeItem(updateResult, item.ServerId);
        Site.Assert.IsNotNull(item, "The message with subject {0} should be found in the folder {1} of user {2}.", emailSubject, FolderType.Inbox.ToString(), User2Information.UserName);
        #endregion
        ...
        UpdateEmail(User2Information.InboxCollectionId, updateResult.SyncKey, null, item.ServerId, null, categories);

        // Get changes from server
        var getChangedResult = SyncChanges(updateResult.SyncKey, User2Information.InboxCollectionId, null);
        item = TestSuiteHelper.GetSyncChangeItem(getChangedResult, item.ServerId);
        Site.Assert.IsNotNull(item, "The message with subject {0} should be found in the folder {1} of user {2}.", emailSubject, FolderType.Inbox.ToString(), User2Information.UserName);
        #endregion
    ...
    public void MSASEMAIL_S01_TC05_OnlyReadPropertyChanged()
    {
        ...
        UpdateEmail(User2Information.InboxCollectionId, getChangedResult.SyncKey, false, item.ServerId, null, null);
        ...
        Site.CaptureRequirementIfIsTrue(
            TestSuiteHelper.IsOnlySpecifiedElement((XmlElement)EMAILAdapter.LastRawResponseXml, "ApplicationData", "Read"),
            75,
    ...
    public void MSASEMAIL_S01_TC06_OnlyFlagPropertiesChanged()
    {
        ...
        // Update email flag
        UpdateEmail(User2Information.InboxCollectionId, getChangedResult.SyncKey, null, item.ServerId, newFlag, null);

        // Get update result from server 
        SyncChanges(getChangedResult.SyncKey, User2Information.InboxCollectionId, null);
        #endregion
        ...
        if (!Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site).Equals("12.1"))
        {
            ...
            Site.CaptureRequirementIfIsTrue(
                TestSuiteHelper.IsOnlySpecifiedElement((XmlElement)EMAILAdapter.LastRawResponseXml, "ApplicationData", "Flag"),
                76,
    ...
    public void MSASEMAIL_S01_TC07_BothReadAndFlagChanged()
    {
        ...
        UpdateEmail(User2Information.InboxCollectionId, getChangedResult.SyncKey, false, item.ServerId, newFlag, null);
    ...
    public void MSASEMAIL_S01_TC08_FlagReadAndOtherEmailClassPropertiesChanged()
    {
        ...
        #region Call Sync command with Change element to add flag and categories for the e-mail item and synchronize it with the server.
        // Get the email item
        var store = GetSyncResult(emailSubject, User2Information.InboxCollectionId, null);
        var item = TestSuiteHelper.GetSyncAddItem(store, emailSubject);

        // Get the result of adding flag and categories
        var getChangedResult = AddFlagAndCategories(User2Information.InboxCollectionId, store.SyncKey, item.ServerId);
        item = TestSuiteHelper.GetSyncChangeItem(getChangedResult, item.ServerId);
        Site.Assert.IsNotNull(item, "The message with subject {0} should be found in the folder {1} of user {2}.", emailSubject, FolderType.Inbox.ToString(), User2Information.UserName);
        #endregion
        ...
        var newCategory = Common.GenerateResourceName(Site, "NewCategory");
        var newCategories = new Collection<string> { newCategory };

        // Update Read, Flag and Categories properties
        UpdateEmail(User2Information.InboxCollectionId, getChangedResult.SyncKey, false, item.ServerId, newFlag, newCategories);
        ...
        if (Common.IsRequirementEnabled(1017, Site))
        {
            // Add the debug information
...
Changes to non-E-Mail class properties (Exchange Server 2007 Sp1 and above follow this behavior.)");
    ...
    public void MSASEMAIL_S01_TC09_WithoutAnyEmailClassPropertiesChanged()
    {
        ...
        var syncRequest = TestSuiteHelper.CreateSyncRequest(getChangedResult.SyncKey, User2Information.InboxCollectionId, null);
        var syncChangedResult = EMAILAdapter.Sync(syncRequest);
        #endregion
    ...
    public void MSASEMAIL_S01_TC10_NonBooleanRead()
    {
        ...
        UpdateEmail(User2Information.InboxCollectionId, syncChangeResult.SyncKey, true, item.ServerId, null, null);
        #endregion

        #region Verify requirement
        syncChangeResult = GetSyncResult(emailSubject, User2Information.InboxCollectionId, null);
        ...
        var invalidSyncRequest = TestSuiteHelper.CreateSyncChangeRequest(
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The Sender element is not supported when the ActiveSyncProtocolVersion is 12.1.");
    ...
    public void MSASEMAIL_S01_TC12_SenderAndFromHaveSameValue()
    {
        ...
        var emailSubject = Common.GenerateResourceName(Site, "subject");
        var fromAddress = Common.GetMailAddress(User1Information.UserName, User1Information.UserDomain);
        var senderAddress = Common.GetMailAddress(User1Information.UserName, User1Information.UserDomain);
        SendPlaintextEmail(emailSubject, null, null, null, senderAddress, null, fromAddress);
        #endregion

        #region Record user name, folder collectionId and item subject that are used in this case.
        SwitchUser(User2Information, true);
        RecordCaseRelativeItems(User2Information.UserName, User2Information.InboxCollectionId, emailSubject);
        #endregion
    ...
    public void MSASEMAIL_S01_TC13_SenderValueIsNull()
    {
        ...
        var emailSubject = Common.GenerateResourceName(Site, "subject");
        SendPlaintextEmail(emailSubject, null, null, Common.GetMailAddress(User2Information.UserName, User2Information.UserDomain), null, null, null);
        #endregion
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The ReceivedAsBcc element is not supported when the ActiveSyncProtocolVersion is 12.1.");

        #region The sender calls SendMail method to send the email to recipient.
        var emailSubject = Common.GenerateResourceName(Site, "subject");
        SendPlaintextEmail(emailSubject, string.Empty, string.Concat(User3Information.UserName, '@', User3Information.UserDomain));
        #endregion

        #region Record user name, folder collectionId and item subject that are used in this case.
        SwitchUser(User3Information, true);
        RecordCaseRelativeItems(User3Information.UserName, User3Information.InboxCollectionId, emailSubject);
        #endregion

        #region The recipient calls Sync method to change the value of ReceivedAsBcc element.

        // Synchronize the email item
        var syncChangeResult = GetSyncResult(emailSubject, User3Information.InboxCollectionId, null);
        var item = TestSuiteHelper.GetSyncAddItem(syncChangeResult, emailSubject);

        // Try to change the value of ReceivedAsBcc
        var invalidSyncRequest = TestSuiteHelper.CreateSyncChangeRequest(
            syncChangeResult.SyncKey,
            User3Information.InboxCollectionId,
            new Request.SyncCollectionChange { ServerId = item.ServerId });
        var syncStringResponse = EMAILAdapter.InvalidSync(
            invalidSyncRequest,
    ...
    public void MSASEMAIL_S01_TC15_SendAndReplyToMultipleAddress()
    {
        ...
        var emailSubject = Common.GenerateResourceName(Site, "subject");
        var to = string.Format(
            "{0}, {1}",
            Common.GetMailAddress(User2Information.UserName, User2Information.UserDomain),
            Common.GetMailAddress(User3Information.UserName, User3Information.UserDomain));
        var cc = string.Format(
            "{0}, {1}, {2}",
            Common.GetMailAddress(User1Information.UserName, User1Information.UserDomain),
            Common.GetMailAddress(User2Information.UserName, User2Information.UserDomain),
            Common.GetMailAddress(User3Information.UserName, User3Information.UserDomain));
        var replyTo = string.Format(
            "{0}, {1}",
            Common.GetMailAddress(User1Information.UserName, User1Information.UserDomain),
            Common.GetMailAddress(User3Information.UserName, User3Information.UserDomain));

        SendPlaintextEmail(emailSubject, cc, null, to, null, replyTo, null);
        #endregion

        #region Record user name, folder collectionId and item subject that are used in this case.
        RecordCaseRelativeItems(User1Information.UserName, User1Information.InboxCollectionId, emailSubject);
        ...
        var syncChangeResult = GetSyncResult(emailSubject, User1Information.InboxCollectionId, null);
        var item = TestSuiteHelper.GetSyncAddItem(syncChangeResult, emailSubject);
        #endregion
        ...
        var isVerifiedR400 = item.Email.DisplayTo.Contains(";");
        ...
        var isVerifiedR331 = item.Email.CC.Contains(",");
        ...
        var isVerifiedR711 = item.Email.ReplyTo.Contains(";");
        ...
        if (!Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site).Equals("12.1"))
        {
            ...
            var replyRequest = TestSuiteHelper.CreateSmartReplyRequest(User2Information.InboxCollectionId, item.ServerId, replyMime);
            EMAILAdapter.SmartReply(replyRequest);
            #endregion

            #region Record user name, folder collectionId and item subject that are used in this case.
            RecordCaseRelativeItems(User1Information.UserName, User1Information.InboxCollectionId, replySubject);
            RecordCaseRelativeItems(User3Information.UserName, User3Information.InboxCollectionId, replySubject);
            #endregion
            ...
            syncChangeResult = GetSyncResult(replySubject, User3Information.InboxCollectionId, null);
            var replyItem = TestSuiteHelper.GetSyncAddItem(syncChangeResult, replySubject);
            #endregion

            #region Verify requirements
            var firstEmailIndex = Convert.FromBase64String(item.Email.ConversationIndex);
            var replyEmailIndex = Convert.FromBase64String(replyItem.Email.ConversationIndex);
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The email2:LastVerbExecuted element, email2:LastVerbExecutedTime element and ConversationIndex element are not supported when the ActiveSyncProtocolVersion is 12.1.");

        #region The sender calls SendMail method to send the email to recipient.
        var emailSubject = Common.GenerateResourceName(Site, "subject");
        SendPlaintextEmail(emailSubject, string.Empty, string.Empty, Common.GetMailAddress(User2Information.UserName, User2Information.UserDomain), null, null, null);
        #endregion
        ...
        var forwardRequest = TestSuiteHelper.CreateSmartForwardRequest(User2Information.InboxCollectionId, item.ServerId, forwardMime);
        EMAILAdapter.SmartForward(forwardRequest);
        #endregion

        #region Record user name, folder collectionId and item subject that are used in this case.
        SwitchUser(User3Information, true);
        RecordCaseRelativeItems(User3Information.UserName, User3Information.InboxCollectionId, forwardSubject);
        #endregion

        #region The second recipient calls Sync method to synchronize the email item on the server.
        syncChangeResult = GetSyncResult(forwardSubject, User3Information.InboxCollectionId, null);
        var forwardItem = TestSuiteHelper.GetSyncAddItem(syncChangeResult, forwardSubject);
        #endregion

        #region Verify requirement
        var firstEmailIndex = Convert.FromBase64String(item.Email.ConversationIndex);
        var forwardEmailIndex = Convert.FromBase64String(forwardItem.Email.ConversationIndex);
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The email2:ConversationId element is not supported when the ActiveSyncProtocolVersion is 12.1.");
        ...
        var syncRequest = TestSuiteHelper.CreateSyncChangeRequest(getEmailItem.SyncKey, User2Information.InboxCollectionId, changeData);
        var response = EMAILAdapter.InvalidSync(syncRequest, "<ConversationId xmlns=\"Email2\"><![CDATA[6E83QrDB2UWpEw+EHaJC3A==]]></ConversationId>", "<Read xmlns=\"Email\">");
        #endregion
    ...
    public void MSASEMAIL_S01_TC18_SyncAdd_AdministrativeMessage()
    {
        ...
        var emailMime = TestSuiteHelper.CreatePlainTextMime(
            Common.GetMailAddress(User1Information.UserName, User1Information.UserDomain),
            Common.GetMailAddress("NoneExistUser", User1Information.UserDomain),
            null,
            null,
            emailSubject,
            emailBody);
        ...
        do
        {
            ...
            // Get the new added email item
            var initSyncResult = InitializeSync(User1Information.InboxCollectionId);
            syncItemResult = SyncChanges(initSyncResult.SyncKey, User1Information.InboxCollectionId, null);
            if (syncItemResult != null && syncItemResult.CollectionStatus == 1)
            {
                if (syncItemResult.AddElements != null)
                {
                    foreach (var syncItem in syncItemResult.AddElements)
                    {
    ...
    public void MSASEMAIL_S01_TC19_ItemOperations_OnlyIncludedSubjectInResponse()
    {
        ...
        var itemOperationRequest = TestSuiteHelper.CreateItemOperationsFetchRequest(User2Information.InboxCollectionId, item.ServerId, null, null, schema);
        var itemOperationResult = EMAILAdapter.ItemOperations(itemOperationRequest);
        var itemOperationsItem = TestSuiteHelper.GetItemOperationsItem(itemOperationResult, emailSubject);
        Site.Assert.IsNotNull(itemOperationsItem, "The email item with subject '{0}' should be found in the {1} folder of user {2}.", emailSubject, FolderType.Inbox.ToString(), User2Information.UserName);
        #endregion
        ...
        Site.CaptureRequirementIfIsTrue(
            TestSuiteHelper.IsOnlySpecifiedElement((XmlElement)EMAILAdapter.LastRawResponseXml, "Properties", "Subject"),
            63,
    ...
    public void MSASEMAIL_S01_TC20_ItemOperations_IncludedDataInResponse()
    {
        ...
        var itemOperationRequest = TestSuiteHelper.CreateItemOperationsFetchRequest(User2Information.InboxCollectionId, item.ServerId, bodyPreference, bodyPartPreference, null);
        var itemOperationResult = EMAILAdapter.ItemOperations(itemOperationRequest);
        #endregion
    ...
    {
        var fetchBodyPreference = new Request.BodyPreference
        {
            TruncationSize = 10,
            TruncationSizeSpecified = true,
            AllOrNone = false,
        ...
        VerifyAllOrNone(fetchBodyPreference);
    }
    #endregion
    ...
    public void MSASEMAIL_S01_TC23_InvalidSearch()
    {
        ...
        GetSyncResult(emailSubject, User2Information.InboxCollectionId, null);
        ...
        var status = doc.SelectSingleNode("/e:Search/e:Response/e:Store/e:Status", xnm);
    ...
    public void MSASEMAIL_S01_TC24_SearchEmail()
    {
        ...
        // Search email from server
        var searchRequest = TestSuiteHelper.CreateSearchRequest(emailSubject, User2Information.InboxCollectionId);
        var searchResponse = EMAILAdapter.Search(searchRequest);
        ...
        {
            foreach (var item in searchStore.Results)
            {
    ...
    public void MSASEMAIL_S01_TC25_OnlyEmailClassPropertiesChangedOtherThanFlagRead()
    {
        ...
        UpdateEmail(User2Information.InboxCollectionId, getChangedResult.SyncKey, null, item.ServerId, null, newCategories);
    ...
    {
        Site.Assume.AreEqual<string>("16.0", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The Bcc element is supported when the ActiveSyncProtocolVersion is 16.0.");
        Site.Assume.AreEqual<string>("16.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The Bcc element is supported when the ActiveSyncProtocolVersion is 16.1.");
        ...
        var draftCollectionId = Common.GetDefaultFolderServerId(folderSyncResponse, FolderType.Drafts, Site);

        var subject = Common.GenerateResourceName(Site, "subject");
        var to = Common.GetMailAddress(User1Information.UserName, User1Information.UserDomain);
        var bcc = Common.GetMailAddress(User2Information.UserName, User2Information.UserDomain);
        ...
        var initSyncResponse = InitializeSync(draftCollectionId);
        var addEMailRequest = TestSuiteHelper.CreateSyncAddRequest(initSyncResponse.SyncKey, draftCollectionId, syncAddCollection);
        EMAILAdapter.Sync(addEMailRequest);
        #endregion

        #region Call Sync command wihtout including BodyPreference to synchronize the e-mail items with server.
        // Get the new added email item
        var syncChangeResult = GetSyncResult(subject, draftCollectionId, null);
        var item = TestSuiteHelper.GetSyncAddItem(syncChangeResult, subject);
        ...
        #region Call Search command to Search email from server.
        // Search email from server
        var searchRequest = TestSuiteHelper.CreateSearchRequest(subject, draftCollectionId);
        var searchResponse = EMAILAdapter.Search(searchRequest);
        ...
        if (searchStore.Results.Count != 0)
        {
            foreach (var resultItem in searchStore.Results)
            {
        ...
        #region Call ItemOperations command without including BodyPreference element to fetch all the information about the e-mail
        var itemOperationRequest = TestSuiteHelper.CreateItemOperationsFetchRequest(draftCollectionId, item.ServerId, null, null, null);
        var itemOperationResult = EMAILAdapter.ItemOperations(itemOperationRequest);
        var itemOperationsItem = TestSuiteHelper.GetItemOperationsItem(itemOperationResult, subject);
    ...
    public void MSASEMAIL_S01_TC27_CreateDraftEMailAndSend()
    {
        ...
        var subject = Common.GenerateResourceName(Site, "subject");
        var to = Common.GetMailAddress(User2Information.UserName, User1Information.UserDomain);
        var syncAddCollection = new Request.SyncCollectionAdd();
    ...
    public void MSASEMAIL_S01_TC30_BothReadandCategoriesChanged()
    {
        ...
        var newCategory = Common.GenerateResourceName(Site, "NewCategory");
        var newCategories = new Collection<string> { newCategory };
        UpdateEmail(User2Information.InboxCollectionId, getChangedResult.SyncKey, false, item.ServerId, null, newCategories);
    ...
    public void MSASEMAIL_S01_TC31_BothFlagandCategoriesChanged()
    {
        ...
        var newCategory = Common.GenerateResourceName(Site, "NewCategory");
        var newCategories = new Collection<string> { newCategory };
        UpdateEmail(User2Information.InboxCollectionId, getChangedResult.SyncKey, null, item.ServerId, newFlag, newCategories);
    ...
    {
        Site.Assume.AreEqual<string>("16.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The Find command is supported when the ActiveSyncProtocolVersion is 16.1.");
        ...
        var findRequest = TestSuiteHelper.CreateFindRequest(User2Information.InboxCollectionId, emailSubject);
        var findResponse = EMAILAdapter.Find(findRequest);
        #endregion
    }
    #endregion
    ...
    private SyncStore AddFlagAndCategories(string collectionId, string syncKey, string serverId)
    {
        ...
        UpdateEmail(collectionId, syncKey, true, serverId, flag, categories);

        // Get the result of adding flag and category
        return SyncChanges(syncKey, collectionId, null);
    }
    ...
    private void VerifyAllOrNone(Request.BodyPreference fetchBodyPreference)
    {
        ...
        // Fetch email from server
        var itemOperationRequest = TestSuiteHelper.CreateItemOperationsFetchRequest(User2Information.InboxCollectionId, item.ServerId, fetchBodyPreference, null, null);
        var itemOperationResult = EMAILAdapter.ItemOperations(itemOperationRequest);
        var itemOperationsItem = TestSuiteHelper.GetItemOperationsItem(itemOperationResult, emailSubject);
