...
--_001_MSASEMAIL_NextPart_--
";
        var firstVoiceFileName = Path.GetFileName(firstVoiceFilePath);
        var firstVoiceContent = Convert.ToBase64String(File.ReadAllBytes(firstVoiceFilePath));
        var firstVoiceFileExt = Path.GetExtension(firstVoiceFilePath);

        var secondVoiceFileName = Path.GetFileName(secondVoiceFilePath);
        var secondVoiceContent = Convert.ToBase64String(File.ReadAllBytes(secondVoiceFilePath));
        var secondVoiceFileExt = Path.GetExtension(secondVoiceFilePath);
...
";
    ...
    internal static SyncRequest CreateSyncRequest(string syncKey, string collectionId, Request.BodyPreference bodyPreference)
    {
        ...
        var syncOptions = new Request.Options();
        var syncOptionItems = new List<object>();
        var syncOptionItemsName = new List<Request.ItemsChoiceType1>();
    ...
    {
        var syncAddRequest = CreateSyncRequest(syncKey, collectionId, null);
        ...
        var commandList = new List<object> { add };
    ...
    internal static Request.SyncCollectionChange CreateSyncChangeData(bool read, string serverId, Request.Flag flag, Collection<object> categories)
    {
        ...
        if (null != categories)
        {
            var mailCategories = new Request.Categories();
            var category = new List<string>();
            foreach (var categoryObject in categories)
            {
    ...
    internal static Sync GetSyncChangeItem(SyncStore syncResult, string serverId)
    {
        ...
        {
            foreach (var syncItem in syncResult.ChangeElements)
            {
    ...
    internal static string GetStatusCode(string changeResponseXml)
    {
        ...
        var status = doc.SelectSingleNode("//e:Change/e:Status", nameSpaceManager);
        if (status == null)
        {
            return null;
        }
    ...
    internal static ItemOperations GetItemOperationsItem(ItemOperationsStore itemOperationsResult, string subject)
    {
        ...
        {
            foreach (var itemOperationsItem in itemOperationsResult.Items)
            {
    ...
    internal static string BytesToHex(byte[] bytes, int startIndex, int count)
    {
        ...
        var endIndex = startIndex + count > bytes.Length ? bytes.Length : startIndex + count;

        var result = new char[(endIndex - startIndex) * 2];
        var resultIndex = 0;
        byte nib;
    ...
    {
        var isOnlySpecifiedElement = false;
        if (rawResponseXml != null)
        {
            var nodes = rawResponseXml.GetElementsByTagName(tagName);
    ...
    internal static string GenerateOutlookID(DateTime createTime)
    {
        ...
        var timeBytes = BitConverter.GetBytes(createTime.ToFileTimeUtc());

        // CreationDateTime
        uidBuilder.Append(BytesToHex(timeBytes));

        // Padding
        uidBuilder.Append("0000000000000000");
        var gloabalIdData = Guid.NewGuid().ToByteArray();
        var dataSizeBytes = BitConverter.GetBytes(gloabalIdData.Length);

        // DataSize
        uidBuilder.Append(BytesToHex(dataSizeBytes));

        // GlobalIdData
        uidBuilder.Append(BytesToHex(gloabalIdData));
    ...
    internal static Dictionary<Request.ItemsChoiceType8, object> SetMeetingProperties(string subject, string attendeeEmailAddress, ITestSite testSite)
    {
        ...
        var attendees = CreateAttendees(attendeeEmailAddress);
    ...
    internal static Calendar CreateDefaultCalendar(string subject, string organizerEmailAddress, string attendeeEmailAddress)
    {
        ...
        var attendeelist = new List<Response.AttendeesAttendee>
        {
