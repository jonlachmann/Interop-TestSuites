...
public class S01_Settings_SendMail_Sync : TestSuiteBase
{
    ...
    public void MSASRM_S01_TC01_Sync_RightsManagedEmailMessages()
    {
        ...
        var lastRawResponse = (XmlElement)ASRMAdapter.LastRawResponseXml;
        var contentExpiryDate = TestSuiteHelper.GetElementInnerText(lastRawResponse, "RightsManagementLicense", "ContentExpiryDate", subject);
        ...
        item = SyncEmail(subject, UserTwoInformation.InboxCollectionId, false, true);
        Site.Assert.IsNotNull(item, "The returned item should not be null.");
        ...
        item = SyncEmail(subject, UserTwoInformation.InboxCollectionId, null, true);
        Site.Assert.IsNotNull(item, "The returned item should not be null.");
        Site.Assert.IsNotNull(item.Email.Attachments, "The Attachments element in expected rights-managed e-mail message should not be null.");
    ...
    public void MSASRM_S01_TC02_Sync_Owner_RightsManagedEmailMessages()
    {
        ...
        var subject = SendMailAndFolderSync(templateID, true, null);
        #endregion
        ...
        item = SyncEmail(subject, UserOneInformation.SentItemsCollectionId, true, true);
    ...
    {
        Site.Assume.AreNotEqual<string>("14.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "Implementation does consider the XML body of the command request to be invalid, if the protocol version specified by in the command request is not 14.1.");
        Site.Assume.AreNotEqual<string>("16.0", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "Implementation does consider the XML body of the command request to be invalid, if the protocol version specified by in the command request is not 16.0.");
        Site.Assume.AreNotEqual<string>("16.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "Implementation does consider the XML body of the command request to be invalid, if the protocol version specified by in the command request is not 16.1.");
