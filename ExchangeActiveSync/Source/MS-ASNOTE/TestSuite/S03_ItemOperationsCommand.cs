...
public class S03_ItemOperationsCommand : TestSuiteBase
{
    ...
    public void MSASNOTE_S03_TC01_ItemOperations_GetZeroOrMoreNotes()
    {
        ...
        var serverIds = new List<string> { result.AddElements[0].ServerId, result.AddElements[1].ServerId };
        var itemOperationRequest = TestSuiteHelper.CreateItemOperationsFetchRequest(UserInformation.NotesCollectionId, serverIds, null, null, null);
        var itemOperationsResult = NOTEAdapter.ItemOperations(itemOperationRequest);
        ...
        serverIds.Clear();
        serverIds.Add(UserInformation.NotesCollectionId + ":notExisting");
        itemOperationRequest = TestSuiteHelper.CreateItemOperationsFetchRequest(UserInformation.NotesCollectionId, serverIds, null, null, null);
        itemOperationsResult = NOTEAdapter.ItemOperations(itemOperationRequest);
    ...
    public void MSASNOTE_S03_TC02_ItemOperations_SchemaViewFetch()
    {
        ...
        var result = NOTEAdapter.Search(UserInformation.NotesCollectionId, addElements[Request.ItemsChoiceType8.Subject1].ToString(), true, 1);
        ...
        var longIds = new List<string> { result.Results[0].LongId };
        ...
        // serverIds:null
        var itemOperationRequest = TestSuiteHelper.CreateItemOperationsFetchRequest(null, null, longIds, bodyReference, schema);
        var itemOperationsResult = NOTEAdapter.ItemOperations(itemOperationRequest);
