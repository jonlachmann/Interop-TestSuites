...
internal static class TestSuiteHelper
{
    ...
    internal static bool CheckSyncChangeCommands(SyncStore result, string subject, ITestSite site)
    {
        ...
        foreach (var sync in result.ChangeElements)
        {
    ...
    {
        foreach (var addElementName in addElements.Keys)
        {
            var changeElementName = (Request.ItemsChoiceType7)Enum.Parse(typeof(Request.ItemsChoiceType7), addElementName.ToString());
    ...
    {
        var reg = new Regex(@"<(html)>(.*\n*)*<\/\1>", RegexOptions.IgnoreCase);
        var isHTML = reg.IsMatch(content);
        return isHTML;
    }
}