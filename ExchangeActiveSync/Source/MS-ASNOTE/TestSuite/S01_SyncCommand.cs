...
public class S01_SyncCommand : TestSuiteBase
{
    ...
    public void MSASNOTE_S01_TC01_Sync_AddNote()
    {
        ...
        SyncAdd(addElements, 1);

        #endregion

        #region Call method Sync to synchronize the note item with the server.

        var result = SyncChanges(1);

        var note = result.AddElements[0].Note;
    ...
    public void MSASNOTE_S01_TC02_Sync_ChangeNote_WithoutBodyInRequest()
    {
        ...
        var addResult = SyncAdd(addElements, 1);
        var item = addResult.AddResponses[0];
        #endregion
        ...
        var changeElements = new Dictionary<Request.ItemsChoiceType7, object>();
        var changedSubject = Common.GenerateResourceName(Site, "subject");
        ...
        var changeResult = SyncChange(addResult.SyncKey, item.ServerId, changeElements);
        ...
        ExistingNoteSubjects.Remove(addElements[Request.ItemsChoiceType8.Subject1].ToString());
        ExistingNoteSubjects.Add(changeElements[Request.ItemsChoiceType7.Subject2].ToString());
        #endregion

        #region Call method Sync to synchronize the note item with the server.
        // Synchronize the changes with server
        var result = SyncChanges(addResult.SyncKey, 1);

        var isNoteFound = TestSuiteHelper.CheckSyncChangeCommands(result, changeElements[Request.ItemsChoiceType7.Subject2].ToString(), Site);
        ...
        var note = result.ChangeElements[0].Note;
    ...
    public void MSASNOTE_S01_TC03_Sync_LastModifiedDateIgnored()
    {
        #region Call method Sync to add a note to the server
        var addElements = CreateNoteElements();
        var lastModifiedDate = DateTime.UtcNow.ToString("yyyyMMddTHHmmssZ", CultureInfo.InvariantCulture);
        ...
        for (var i = 0; i < result.AddElements.Count; i++)
        {
        ...
        var isVerifiedR126 = note.Body != null && note.Subject != null && note.MessageClass != null && note.IsLastModifiedDateSpecified && note.Categories != null && note.Categories.Category != null;
        ...
        SyncChange(result.SyncKey, item.ServerId, changeElements);

        #endregion

        #region Call method Sync to synchronize the changes with the server.

        var result2 = SyncChanges(result.SyncKey, 1);

        bool isNoteFound;
        if (result2.ChangeElements != null)
        {
            isNoteFound = TestSuiteHelper.CheckSyncChangeCommands(result, addElements[Request.ItemsChoiceType8.Subject1].ToString(), Site);
        ...
        result = SyncChanges(result.SyncKey, 1);

        isNoteFound = TestSuiteHelper.CheckSyncChangeCommands(result, changeElements[Request.ItemsChoiceType7.Subject2].ToString(), Site);
        ...
        var isVerifiedR210 = note.Body != null && note.Subject != null && note.MessageClass != null && note.IsLastModifiedDateSpecified && note.Categories != null && note.Categories.Category != null;
    ...
    public void MSASNOTE_S01_TC04_Sync_SupportedError()
    {
        ...
        {
            CollectionId = UserInformation.NotesCollectionId,
            SyncKey = "0",
    ...
    public void MSASNOTE_S01_TC06_Sync_AddNote_WithBodyTypes()
    {
        ...
        var isHTML = TestSuiteHelper.IsHTML(note.Body.Data);
        Site.Assert.IsTrue(
            isHTML,
        ...
        try
        {
            var contentBytes = Convert.FromBase64String(note.Body.Data);
    ...
    public void MSASNOTE_S01_TC07_Sync_ChangeNote_Categories()
    {
        ...
        var addElements = CreateNoteElements();
        var categories = new Request.Categories4 { Category = new string[2] };
        var category = new Collection<string> { "blue category", "red category" };
        ...
        var noteAdded = result.AddElements[0].Note;
        ...
        var changeResult = SyncChange(result.SyncKey, result.AddElements[0].ServerId, changeElements);
        ...
        var isNoteFound = TestSuiteHelper.CheckSyncChangeCommands(result, addElements[Request.ItemsChoiceType8.Subject1].ToString(), Site);
        ...
        changeResult = SyncChange(result.SyncKey, result.ChangeElements[0].ServerId, changeElements);
