...
public partial class MS_ASNOTEAdapter
{
    ...
    private void VerifyNote(Note note, bool isSchemaRestricted)
    {
        if (note.Body != null)
        {
            ...
            var isVerifyR37 = note.Body.Type == 1 || note.Body.Type == 2 || note.Body.Type == 3;
            Site.CaptureRequirementIfIsTrue(
                isVerifyR37,
                37,
        ...
        if (!isSchemaRestricted)
        {
            ...
            var isVerifyR57 = note.MessageClass.Equals("IPM.StickyNote", StringComparison.CurrentCulture) || note.MessageClass.StartsWith("IPM.StickyNote.", StringComparison.CurrentCulture);
            Site.CaptureRequirementIfIsTrue(
                isVerifyR57,
                57,
            ...
            var regex = new Regex(@"\d{4}(0\d|1[0-2])([0-2]\d|3[0-1])T([0-1]\d|2[0-3])[0-5]\d[0-5]\d?Z", RegexOptions.Singleline);
...
seconds       = (""0"" DIGIT) / (""1"" DIGIT) / (""2"" DIGIT) / (""3"" DIGIT) / (""4"" DIGIT) / (""5"" DIGIT)");
    ...
    private void VerifyItemOperationResult(ItemOperationsStore itemResponse, bool isSchemaRestricted)
    {
        ...
        foreach (var itemOperations in itemResponse.Items)
        {
            if (itemOperations.Note != null)
            {
                VerifyNote(itemOperations.Note, isSchemaRestricted);
            }
        }
    ...
    private void VerifySyncResult(SyncStore syncResponse)
    {
        ...
        {
            foreach (var sync in syncResponse.AddElements)
            {
                VerifyNote(sync.Note, false);
            }
        }

        if (syncResponse.ChangeElements != null)
        {
            foreach (var sync in syncResponse.ChangeElements)
            {
                VerifyNote(sync.Note, false);
            }
        }            
    ...
    private void VerifySearchResult(SearchStore searchResponse)
    {
        ...
        foreach (var search in searchResponse.Results)
        {
            VerifyNote(search.Note, false);
        }
        ...
        if (decodedData != null)
        {
            ...
            foreach (var decodeDataItem in decodedData)
            {
                ...
                if (23 == codepage)
                {
                    ...
                    CaptureRequirementsRelateToCodePage23(codepage, tagName, token);
                }
            }
        }
    }
