...
public class S01_Sync : TestSuiteBase
{
    ...
    public void MSASCON_S01_TC01_Sync_MarkRead()
    {
        ...
        var conversationItem = CreateConversation(conversationSubject);
        #endregion

        #region Mark the created conversation as read.
        SyncChange(LatestSyncKey, conversationItem.ServerId, User1Information.InboxCollectionId, true, null);

        // Check whether the read property is changed to read for all messages in the conversation.
        var syncItems = CallSyncCommand(User1Information.InboxCollectionId, false);

        var markRead = true;
        var itemCount = 0;

        foreach (var item in syncItems.AddElements)
        {
        ...
        var syncStore = SyncChange(LatestSyncKey, conversationItem.ServerId, User1Information.InboxCollectionId, false, null);

        // Check whether read property is changed to unread for all messages in the conversation.
        syncItems = CallSyncCommand(User1Information.InboxCollectionId, false);
    ...
    public void MSASCON_S01_TC02_Sync_Flag()
    {
        ...
        var mostRecentEmail = new Email { ConversationIndex = string.Empty };
        foreach (var syncItem in syncItems.AddElements)
        {
        ...
        SyncChange(LatestSyncKey, conversationItem.ServerId, User1Information.InboxCollectionId, null, "0");
        ...
        SyncChange(LatestSyncKey, conversationItem.ServerId, User1Information.InboxCollectionId, null, "1");

        // Check whether flag status property is changed to 1 for all messages in the conversation.
        syncItems = CallSyncCommand(User1Information.InboxCollectionId, false);

        var markCompleteFlags = true;
        itemCount = 0;
    ...
    public void MSASCON_S01_TC03_Sync_Delete()
    {
        #region Create a conversation and sync to get the created conversation item.
        var conversationSubject = Common.GenerateResourceName(Site, "Conversation");
        var inboxConversationItem = CreateConversation(conversationSubject);
        #endregion

        #region Delete the created conversation by Sync command.
        var serverIds = new string[inboxConversationItem.ServerId.Count];
        inboxConversationItem.ServerId.CopyTo(serverIds, 0);
        var syncStore = SyncDelete(User1Information.InboxCollectionId, LatestSyncKey, serverIds);
        RecordCaseRelativeItems(User1Information, User1Information.InboxCollectionId, conversationSubject, true);
        RecordCaseRelativeItems(User1Information, User1Information.DeletedItemsCollectionId, conversationSubject, false);
        ...
        var deleteResult = SyncEmail(conversationSubject, User1Information.InboxCollectionId, false, null, null);

        var deleteConversationItem = GetConversationItem(User1Information.DeletedItemsCollectionId, inboxConversationItem.ConversationId);
        ...
        var isDeleted = deleteConversationItem.ServerId.Count == inboxConversationItem.ServerId.Count && deleteResult == null;
        ...
        var syncResult = SyncEmail(conversationSubject, User2Information.InboxCollectionId, true, null, null);
        ...
        CallSmartReplyCommand(syncResult.ServerId, User2Information.InboxCollectionId, user2MailboxAddress, user1MailboxAddress, conversationSubject);
        RecordCaseRelativeItems(User1Information, User1Information.InboxCollectionId, conversationSubject, false);

        // Switch the current user to User1.
        SwitchUser(User1Information, false);
        syncResult = SyncEmail(conversationSubject, User1Information.InboxCollectionId, true, null, null);
        var futureInboxConversation = GetConversationItem(User1Information.InboxCollectionId, syncResult.Email.ConversationId);

        var isFutureEmailMoved = futureInboxConversation.ServerId.Count == 1 && futureInboxConversation.ConversationId == inboxConversationItem.ConversationId;
    ...
    public void MSASCON_S01_TC04_Sync_Filter()
    {
        ...
        var syncStore = CallSyncCommand(User1Information.InboxCollectionId, true);
    ...
    public void MSASCON_S01_TC05_Sync_ConversationIndex()
    {
        ...
        var user1MailboxAddress = Common.GetMailAddress(User1Information.UserName, User1Information.UserDomain);
        var user2MailboxAddress = Common.GetMailAddress(User2Information.UserName, User2Information.UserDomain);
        CallSendMailCommand(user2MailboxAddress, user1MailboxAddress, conversationSubject, null);
        RecordCaseRelativeItems(User1Information, User1Information.InboxCollectionId, conversationSubject, false);
        #endregion

        #region User1 replies to User2
        SwitchUser(User1Information, false);
        var syncResult = SyncEmail(conversationSubject, User1Information.InboxCollectionId, true, null, null);
        ...
        var conversationIndexHeader = Convert.FromBase64String(syncResult.Email.ConversationIndex);
        ...
        var conversationIdAndIndex = syncResult.Email.ConversationIndex != null && syncResult.Email.ConversationId != null;
        ...
        CallSmartReplyCommand(syncResult.ServerId, User1Information.InboxCollectionId, user1MailboxAddress, user2MailboxAddress, conversationSubject);
        RecordCaseRelativeItems(User2Information, User2Information.InboxCollectionId, conversationSubject, false);
        #endregion

        #region User2 forwards to User3
        SwitchUser(User2Information, false);
        syncResult = SyncEmail(conversationSubject, User2Information.InboxCollectionId, true, null, null);
        var conversationIndexReply = Convert.FromBase64String(syncResult.Email.ConversationIndex);

        var additionalTimestampAdded = conversationIndexReply.Length > conversationIndexHeader.Length && BitConverter.ToString(conversationIndexReply).Replace("-", string.Empty).StartsWith(BitConverter.ToString(conversationIndexHeader).Replace("-", string.Empty), StringComparison.OrdinalIgnoreCase);
        ...
        var user3MailboxAddress = Common.GetMailAddress(User3Information.UserName, User3Information.UserDomain);

        CallSmartForwardCommand(syncResult.ServerId, User2Information.InboxCollectionId, user2MailboxAddress,  user3MailboxAddress, conversationSubject);
        #endregion

        #region User3 gets the email.
        SwitchUser(User3Information, true);
        RecordCaseRelativeItems(User3Information, User3Information.InboxCollectionId, conversationSubject, false);

        syncResult = SyncEmail(conversationSubject, User3Information.InboxCollectionId, true, null, null);
        var conversationIndexForward = Convert.FromBase64String(syncResult.Email.ConversationIndex);
    ...
    public void MSASCON_S01_TC06_Sync_NoConversationId()
    {
        ...
        var syncRequest = Common.CreateInitialSyncRequest(User1Information.CalendarCollectionId);
        ...
        var calendarSubject = Common.GenerateResourceName(Site, "TestCalendar");
        SyncAdd(User1Information.CalendarCollectionId, calendarSubject, syncStore.SyncKey);
        RecordCaseRelativeItems(User1Information, User1Information.CalendarCollectionId, calendarSubject, false);
    ...
    public void MSASCON_S01_TC07_Sync_MessagePart()
    {
        ...
        VerifyMessagePartWithoutPreference(syncItem.Email);
        ...
        syncItem = SyncEmail(subject, User1Information.InboxCollectionId, true, null, bodyPreference);
        VerifyMessagePartWithBodyPreference(syncItem.Email);
        #endregion

        #region Call Sync command with only BodyPartPreference element.
        var bodyPartPreference = new Request.BodyPartPreference()
        {
            Type = 2,
        };

        // Get all the email BodyPart data.
        SyncEmail(subject, User1Information.InboxCollectionId, true, bodyPartPreference, null);
        var lastRawResponse = (XmlElement)CONAdapter.LastRawResponseXml;
        ...
        syncItem = SyncEmail(subject, User1Information.InboxCollectionId, true, bodyPartPreference, null);
        ...
        VerifyMessagePartWithBodyPartPreference(syncItem.Email, truncatedData, allData, (int)bodyPartPreference.TruncationSize);
        ...
        syncItem = SyncEmail(subject, User1Information.InboxCollectionId, true, bodyPartPreference, bodyPreference);
        VerifyMessagePartWithBothPreference(syncItem.Email);
        #endregion
    }
    #endregion
    ...
    public void MSASCON_S01_TC08_Sync_Status164()
    {
        ...
        SyncEmail(subject, User1Information.InboxCollectionId, true, null, null);
        ...
        // Call initial Sync command.
        var syncRequest = Common.CreateInitialSyncRequest(User1Information.InboxCollectionId);
        var syncStore = CONAdapter.Sync(syncRequest);
        ...
        syncStore = CONAdapter.Sync(syncRequest);
        VerifyMessagePartStatus164(syncStore.Status);
        #endregion
    }
    #endregion
}