...
public class S02_GetItemEstimate : TestSuiteBase
{
    ...
    public void MSASCON_S02_TC01_GetItemEstimate_Filter()
    {
        ...
        CreateConversation(conversationSubject);
        #endregion

        #region Initial Sync on Inbox folder.
        // Call Initial Sync command to get the latest SyncKey.
        var syncStore = CONAdapter.Sync(Common.CreateInitialSyncRequest(User1Information.InboxCollectionId));
        #endregion

        #region Send GetItemEstimate request and get response.
        var getItemEstimateResponse = CallGetItemEstimateCommand(syncStore.SyncKey, User1Information.InboxCollectionId);

        // Verify GetItemEstimate command response.
        var isVerifyR211 = getItemEstimateResponse.ResponseData.Response.Length == 1 && getItemEstimateResponse.ResponseData.Response[0].Status == "1";
    ...
    public void MSASCON_S02_TC02_GetItemEstimate_Status4()
    {
        ...
        // Call Initial Sync command to get the latest SyncKey.
        var syncStore = CONAdapter.Sync(Common.CreateInitialSyncRequest(User1Information.CalendarCollectionId));
        #endregion

        #region Call GetItemEstimate command on Calendar folder with setting ConversationMode element in the request.
        var getItemEstimateResponse = CallGetItemEstimateCommand(syncStore.SyncKey, User1Information.CalendarCollectionId);
