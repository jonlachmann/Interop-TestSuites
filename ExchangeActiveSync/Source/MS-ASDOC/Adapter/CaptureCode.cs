...
public partial class MS_ASDOCAdapter
{
    ...
    private void VerifySearch(SearchResponse response)
    {
        ...
        {
            Site.Assert.IsTrue(activeSyncClient.ValidationResult, "Schema should be verified successfully.");
            ...
            foreach (var result in response.ResponseData.Response.Store.Result)
            {
                string linkId = null;
                ...
                var isVerifyR15 = null != lastModifiedDate && null != creationDate;
...
MSS = Number of milliseconds");
                ...
                var isVerifyR45 = (byte)isFolder == 0 || (byte)isFolder == 1;
                ...
                var isVerifyR51 = (byte)isHidden == 0 || (byte)isHidden == 1;
    ...
    private void VerifyItemOperations(ItemOperationsResponse response)
    {
        ...
        foreach (var fetch in response.ResponseData.Response.Fetch)
        {
            var statusString = fetch.Status;
            if (statusString == "1")
            {
                // Add the debug information
        ...
        if (decodedData != null)
        {
            ...
            foreach (var decodeDataItem in decodedData)
            {
                ...
                if (19 == codepage)
                {
                    ...
                    CaptureRequirementsRelateToCodePage19(codepage, tagName, token);
                }
            }
        }
    }
