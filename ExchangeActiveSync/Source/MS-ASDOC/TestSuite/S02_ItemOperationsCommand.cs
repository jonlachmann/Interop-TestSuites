...
public class S02_ItemOperationsCommand : TestSuiteBase
{
    ...
    public void MSASDOC_S02_TC01_ItemOperations_WithoutMultipart()
    {
        ...
        var isMultiPartResponse = itemOperationResponse.Headers.ToString().Contains("application/vnd.ms-sync.multipart");

        var existDataElement = false;

        foreach (var itemsElementName in itemOperationResponse.ResponseData.Response.Fetch[0].Properties.ItemsElementName)
        {
    ...
    public void MSASDOC_S02_TC02_ItemOperations_GetZeroOrMoreDocumentClass()
    {
        ...
        // Build element Fetch.
        var fetch1 = FetchElementBuilder(Common.GetConfigurationPropertyValue("SharedHiddenDocument", Site));
        var fetch2 = FetchElementBuilder(Common.GetConfigurationPropertyValue("SharedVisibleDocument", Site));

        var fetches = new ItemOperationsFetch[] { fetch1, fetch2 };

        // Create an ItemOperation command request that multiple document class value will be returned.
        var itemOperationGetMoreDocRequest = Common.CreateItemOperationsRequest(fetches);

        // Send request and get response.
        var itemOperationGetMoreDocResponse = ASDOCAdapter.ItemOperations(itemOperationGetMoreDocRequest, DeliveryMethodForFetch.Inline);
    ...
    private ItemOperationsFetch FetchElementBuilder(string linkId)
    {
        ...
        var fetch = new ItemOperationsFetch
        {
            LinkId = linkId,
        ...
        fetch.Options.Items[0] = Common.GetConfigurationPropertyValue("UserName", Site);
        fetch.Options.Items[1] = Common.GetConfigurationPropertyValue("UserPassword", Site);
