...
public partial class MS_ASHTTPAdapter
{
    ...
    private void VerifyTransportType()
    {
        ...
        var transport = (ProtocolTransportType)Enum.Parse(typeof(ProtocolTransportType), Common.GetConfigurationPropertyValue("TransportType", Site), true);
    ...
    private void VerifyHTTPPOSTResponse(SendStringResponse postResponse)
    {
        ...
        VerifyHTTPPOSTHeaders(postResponse);
    }
    ...
    private void VerifyHTTPPOSTHeaders(SendStringResponse postResponse)
    {
        ...
        var responseHeaders = postResponse.Headers.ToString();
        Site.Log.Add(LogEntryKind.Debug, "The response headers are: {0}.", responseHeaders);
        var isContentLengthExist = responseHeaders.Contains("Content-Length");
        var isContentTypeExist = responseHeaders.Contains("Content-Type");
        ...
        var isVerifiedR199 = isContentLengthExist && int.TryParse(postResponse.Headers["Content-Length"], out contentLengthNumber);
        ...
        var isVerifiedR521 = responseHeaders.Contains("Content-Length: " + postResponse.Headers["Content-Length"]) && int.TryParse(postResponse.Headers["Content-Length"], out contentLengthNumber);
        ...
        var hasContent = int.Parse(postResponse.Headers["Content-Length"]) > 0;
        bool isVerifiedR212;
        if (hasContent)
        {
            // Add the debug information
    ...
    private void VerifyHTTPOPTIONSResponse(OptionsResponse optionsResponse)
    {
        ...
        VerifyHTTPOPTIONSHeaders(optionsResponse);
    }
    ...
    {
        var commandHeaders = optionsResponse.Headers["MS-ASProtocolCommands"];
        var versionHeaders = optionsResponse.Headers["MS-ASProtocolVersions"];
        ...
        var splitCommand = commandHeaders.Split(',');
