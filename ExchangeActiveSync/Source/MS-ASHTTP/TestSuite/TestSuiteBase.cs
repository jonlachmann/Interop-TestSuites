...
public class TestSuiteBase : TestClassBase
{
    ...
    {
        base.TestInitialize();
        HTTPAdapter = Site.GetAdapter<IMS_ASHTTPAdapter>();
        HTTPSUTControlAdapter = Site.GetAdapter<IMS_ASHTTPSUTControlAdapter>();
        ...
        InvalidUserInformation = new UserInformation();

        // Initialize the HTTP POST request prefix field.
        InitializeRequestPrefixField();
    }
    ...
    protected FolderSyncResponse CallFolderSyncCommand()
    {
        ...
        var folderSyncResponseString = HTTPAdapter.HTTPPOST(CommandName.FolderSync, null, folderSyncRequest);

        // Check the command is executed successfully.
        CheckResponseStatus(folderSyncResponseString.ResponseDataXML);

        // Convert SendStringResponse to FolderSyncResponse.
        var folderSyncResponse = new FolderSyncResponse
        {
    ...
    protected void CallSendMailCommand(string from, string to, string subject, string saveInSent)
    {
        ...
        var mime = Common.CreatePlainTextMime(from, to, string.Empty, string.Empty, subject, "SendMail content");
        var request = Common.CreateSendMailRequest(null, Guid.NewGuid().ToString(), mime);
        ...
        var sendMailResponse = HTTPAdapter.HTTPPOST(CommandName.SendMail, request.CommandParameters, request.GetRequestDataSerializedXML());

        // Check the command is executed successfully.
        CheckResponseStatus(sendMailResponse.ResponseDataXML);
    }
    ...
    protected void CallSmartForwardCommand(string from, string forwardTo, string itemServerId, string forwardSubject, string saveInSent, string longId, string occurrence)
    {
        ...
        var mime = Common.CreatePlainTextMime(from, forwardTo, string.Empty, string.Empty, forwardSubject, "SmartForward content");
        var request = Common.CreateSmartForwardRequest(null, Guid.NewGuid().ToString(), mime, source);
        ...
        if (longId == null)
        {
            var collectionId = itemServerId.Split(':')[0];
            if (saveInSent != null)
            {
                int result;
                if (int.TryParse(saveInSent, out result))
        ...
        var smartForwardResponse = HTTPAdapter.HTTPPOST(CommandName.SmartForward, request.CommandParameters, request.GetRequestDataSerializedXML());

        // Check the command is executed successfully.
        CheckResponseStatus(smartForwardResponse.ResponseDataXML);
    }
    ...
    protected void CallSmartReplyCommand(string from, string replyTo, string itemServerId, string replySubject, string saveInSent, string longId, string occurrence)
    {
        ...
        var mime = Common.CreatePlainTextMime(from, replyTo, string.Empty, string.Empty, replySubject, "SmartReply content");
        var request = Common.CreateSmartReplyRequest(null, Guid.NewGuid().ToString(), mime, source);
        ...
        var smartReplyResponse = HTTPAdapter.HTTPPOST(CommandName.SmartReply, request.CommandParameters, request.GetRequestDataSerializedXML());

        // Check the command is executed successfully.
        CheckResponseStatus(smartReplyResponse.ResponseDataXML);
    }
    ...
    protected SyncStore CallSyncCommand(string collectionId)
    {
        ...
        var syncResponse = CallInitialSyncCommand(collectionId);

        Site.Assert.IsNotNull(TestSuiteHelper.LoadSyncResponse(syncResponse), "The Sync response should not be null.");

        var syncRequest = TestSuiteHelper.GetSyncRequest(collectionId, TestSuiteHelper.LoadSyncResponse(syncResponse).SyncKey);
        ...
        var syncStoreTotal = new SyncStore();
        do
        {
            ...
            syncResponseString = HTTPAdapter.HTTPPOST(CommandName.Sync, null, syncRequest.GetRequestDataSerializedXML());

            if (!string.IsNullOrEmpty(syncResponseString.ResponseDataXML))
            {
                // Check the command is executed successfully.
                CheckResponseStatus(syncResponseString.ResponseDataXML);
                ...
                var syncStore = TestSuiteHelper.LoadSyncResponse(syncResponse);
                syncStoreTotal.SyncKey = syncStore.SyncKey;
                for (var i = 0; i < syncStore.AddCommands.Count; i++)
                {
    ...
    {
        var syncRequest = TestSuiteHelper.GetSyncRequest(collectionId, "0");

        // Call Sync command by HTTP POST.
        var syncResponseString = HTTPAdapter.HTTPPOST(CommandName.Sync, null, syncRequest.GetRequestDataSerializedXML());

        // Check the command is executed successfully.
        CheckResponseStatus(syncResponseString.ResponseDataXML);

        // Convert from SendStringResponse to SyncResponse.
        var syncResponse = TestSuiteHelper.ConvertSyncResponseFromSendString(syncResponseString);
    ...
    protected SearchResponse CallSearchCommand(string collectionId, string freeText)
    {
        ...
        var searchStores = new Request.SearchStore[1];
        ...
        var searchString = searchRequest.GetRequestDataSerializedXML();
        var searchResponseString = HTTPAdapter.HTTPPOST(CommandName.Search, null, searchString);

        // Convert from SendStringResponse to SearchResponse.
        var searchResponse = new SearchResponse { ResponseDataXML = searchResponseString.ResponseDataXML };
    ...
    protected void CallMeetingResponseCommand(string collectionId, string itemServerId)
    {
        ...
        var meetingResponseRequest = new Request.MeetingResponseRequest[1];
        ...
        var request = Common.CreateMeetingResponseRequest(meetingResponseRequest);

        // Call MeetingResponse command by HTTP POST.
        var meetingResponseResponse = HTTPAdapter.HTTPPOST(CommandName.MeetingResponse, null, request.GetRequestDataSerializedXML());

        // Check the command is executed successfully.
        CheckResponseStatus(meetingResponseResponse.ResponseDataXML);
    }
    ...
    protected SendStringResponse CallProvisionCommand(string policyKey)
    {
        ...
        var provisionResponse = HTTPAdapter.HTTPPOST(CommandName.Provision, null, provisionRequest.GetRequestDataSerializedXML());

        // Check the command is executed successfully.
        CheckResponseStatus(provisionResponse.ResponseDataXML);
    ...
    protected SendStringResponse CallItemOperationsCommand(string collectionId, string itemServerId, bool acceptMultiPart)
    {
        ...
        var itemOperationsResponse = HTTPAdapter.HTTPPOST(CommandName.ItemOperations, commandParameter, itemOperationsRequest.GetRequestDataSerializedXML());

        // Check the command is executed successfully.
        CheckResponseStatus(itemOperationsResponse.ResponseDataXML);
    ...
    {
        var folderCreateRequest = Common.CreateFolderCreateRequest(syncKey, (byte)12, createdFolder, parentFolder);
        var folderCreateResponseString = HTTPAdapter.HTTPPOST(CommandName.FolderCreate, null, folderCreateRequest.GetRequestDataSerializedXML());

        // Check the command is executed successfully.
        CheckResponseStatus(folderCreateResponseString.ResponseDataXML);

        var folderCreateResponse = new FolderCreateResponse
        {
    ...
    {
        var folderUpdateRequest = Common.CreateFolderUpdateRequest(syncKey, itemServerId, folderName, parentId);
        var folderUpdateResponse = HTTPAdapter.HTTPPOST(CommandName.FolderUpdate, null, folderUpdateRequest.GetRequestDataSerializedXML());

        // Check the command is executed successfully.
        CheckResponseStatus(folderUpdateResponse.ResponseDataXML);
    }
    ...
    {
        var folderDeleteRequest = Common.CreateFolderDeleteRequest(syncKey, itemServerId);
        var folderDeleteResponse = HTTPAdapter.HTTPPOST(CommandName.FolderDelete, null, folderDeleteRequest.GetRequestDataSerializedXML());

        // Check the command is executed successfully.
        CheckResponseStatus(folderDeleteResponse.ResponseDataXML);
    }
    ...
    {
        var moveItemsMove = new Request.MoveItemsMove[1];
        ...
        var request = Common.CreateMoveItemsRequest(moveItemsMove);
        var moveItemsResponse = HTTPAdapter.HTTPPOST(CommandName.MoveItems, null, request.GetRequestDataSerializedXML());

        // Check the command is executed successfully.
        CheckResponseStatus(moveItemsResponse.ResponseDataXML);
    }
    ...
    protected PingResponse CallPingCommand(string collectionId)
    {
        ...
        var pingFolders = new Request.PingFolder[1];
        pingFolders[0] = new Request.PingFolder { Class = Request.PingFolderClass.Email, Id = collectionId };
        var pingRequest = Common.CreatePingRequest(pingFolders, "80");

        // Call Ping command by HTTP POST.
        var pingResponseString = HTTPAdapter.HTTPPOST(CommandName.Ping, null, pingRequest.GetRequestDataSerializedXML());

        var pingResponse = new PingResponse { ResponseDataXML = pingResponseString.ResponseDataXML };
    ...
    protected SendStringResponse VerifyGetAttachmentsCommandParameter(QueryValueType queryValueType)
    {
        ...
        var sendMailSubject = Common.GenerateResourceName(Site, "EmailWithAttachment");
        var mimeFileName = "EmailWithAttachment.txt";
        Site.Assert.IsTrue(File.Exists(mimeFileName), "The mime file {0} with attachment content should exist.", mimeFileName);
        var mime = "To: " + UserTwoInformation.UserName + "\n" + "Subject: " + sendMailSubject + "\n" + File.ReadAllText(mimeFileName);
        var request = Common.CreateSendMailRequest(null, Guid.NewGuid().ToString("N"), mime);

        // Call SendMail command by HTTP POST.
        var sendMailResponse = HTTPAdapter.HTTPPOST(CommandName.SendMail, null, request.GetRequestDataSerializedXML());
        ...
        var searchResult = LoopToSearchItem(UserTwoInformation.InboxCollectionId, sendMailSubject);
        var fileReference = GetFileReferenceFromSearchResult(searchResult);
        ...
        var commandParameters = new Dictionary<CmdParameterName, object>
        {
            {
        ...
        var getAttachmentResponse = HTTPAdapter.HTTPPOST(CommandName.GetAttachment, commandParameters, Common.CreateGetAttachmentRequest().GetRequestDataSerializedXML());

        // Check the command is executed successfully.
        CheckResponseStatus(getAttachmentResponse.ResponseDataXML);
        #endregion
    ...
    protected void VerifySaveInSentCommandParameter(QueryValueType queryValueType, string sendMailSaveInSent, string smartForwardSaveInSent, string smartReplySaveInSent)
    {
        ...
        var syncResponse = CallInitialSyncCommand(Common.GetDefaultFolderServerId(folderSyncResponse, FolderType.SentItems, Site));

        if (Common.IsRequirementEnabled(1202, Site))
        {
            var isR1202Satisfied = TestSuiteHelper.VerifySyncRequiredResponseHeaders(folderSyncResponse.Headers.AllKeys);
        ...
        CallSendMailCommand(userOneMailboxAddress, userTwoMailboxAddress, sendMailSubject, sendMailSaveInSent);
        ...
        var itemServerId = LoopToSyncItem(UserOneInformation.SentItemsCollectionId, sendMailSubject, TestSuiteHelper.IsRetryNeeded(sendMailSaveInSent));
        VerifySaveInSentCommandParameter_SendMail(queryValueType, itemServerId);

        // Add the saved email to CreatedItemsForUserOne collection to delete.
        if (sendMailSaveInSent == "T" || sendMailSaveInSent == "1")
        {
            AddCreatedItemToCollection("User1", UserOneInformation.SentItemsCollectionId, sendMailSubject);
        }
        #endregion
        ...
        itemServerId = LoopToSyncItem(UserTwoInformation.InboxCollectionId, sendMailSubject, true);
        ...
        CallSmartForwardCommand(userTwoMailboxAddress, userThreeMailboxAddress, itemServerId, smartForwardSubject, smartForwardSaveInSent, null, null);

        // Add the received email to CreatedItemsForUserThree collection to delete.
        SwitchUser(UserThreeInformation, true);
        AddCreatedItemToCollection("User3", UserThreeInformation.InboxCollectionId, smartForwardSubject);
        SwitchUser(UserTwoInformation, false);

        // Call Sync command to get the ServerId of the saved email.
        itemServerId = LoopToSyncItem(UserTwoInformation.SentItemsCollectionId, smartForwardSubject, TestSuiteHelper.IsRetryNeeded(smartForwardSaveInSent));
        VerifySaveInSentCommandParameter_SmartForward(queryValueType, itemServerId);

        // Add the saved email to CreatedItemsForUserTwo collection to delete.
        if (smartForwardSaveInSent == "T" || smartForwardSaveInSent == "1")
        {
            AddCreatedItemToCollection("User2", UserTwoInformation.SentItemsCollectionId, smartForwardSubject);
        }
        #endregion
        ...
        // Call Sync command to get the ServerId of the received email.
        itemServerId = LoopToSyncItem(UserThreeInformation.InboxCollectionId, smartForwardSubject, true);
        if (queryValueType == QueryValueType.PlainText)
        {
            VerifyCollectionIdItemIdCommandParameters_SmartForward(itemServerId);
        }
    
        // Call SmartReply command to reply to the received email to User2 with SaveInSent command parameter.
        CallSmartReplyCommand(userThreeMailboxAddress, userTwoMailboxAddress, itemServerId, smartReplySubject, smartReplySaveInSent, null, null);

        // Add the received email to CreatedItemsForUserTwo collection to delete.
        AddCreatedItemToCollection("User2", UserTwoInformation.InboxCollectionId, smartReplySubject);

        // Call Sync command to get the ServerId of the saved email.
        itemServerId = LoopToSyncItem(UserThreeInformation.SentItemsCollectionId, smartReplySubject, TestSuiteHelper.IsRetryNeeded(smartReplySaveInSent));
        VerifySaveInSentCommandParameter_SmartReply(queryValueType, itemServerId);

        // Add the saved email to CreatedItemsForUserThree collection to delete.
        if (smartReplySaveInSent == "T" || smartReplySaveInSent == "1")
        {
            AddCreatedItemToCollection("User3", UserThreeInformation.SentItemsCollectionId, smartReplySubject);
        }
        #endregion
        ...
        // Call Sync command to get the ServerId of the received email.
        itemServerId = LoopToSyncItem(UserTwoInformation.InboxCollectionId, smartReplySubject, true);
        if (queryValueType == QueryValueType.PlainText)
        {
            VerifyCollectionIdItemIdCommandParameters_SmartReply(itemServerId);
        }
    ...
    protected void VerifyLongIdCommandParameter(QueryValueType queryValueType)
    {
        ...
        CallSmartForwardCommand(userTwoMailboxAddress, userThreeMailboxAddress, null, smartForwardSubject, null, searchResult.LongId, null);
        #endregion
        ...
        // Call Sync command to get the ServerId of the received email.
        var itemServerId = LoopToSyncItem(UserThreeInformation.InboxCollectionId, smartForwardSubject, true);
        ...
        {
            // Call Search command to find the received email and get the LongId of it.
            searchResult = LoopToSearchItem(UserThreeInformation.InboxCollectionId, smartForwardSubject);
            ...
            CallSmartReplyCommand(userThreeMailboxAddress, userTwoMailboxAddress, null, smartReplySubject, null, searchResult.LongId, null);
    ...
    {
        // Get the mailboxes.
        var userThreeMailbox = Common.GetMailAddress(UserThreeInformation.UserName, UserThreeInformation.UserDomain);
        var userTwoMailbox = Common.GetMailAddress(UserTwoInformation.UserName, UserTwoInformation.UserDomain);

        // Create meeting request mime content and call SendMail command to send the meeting request to User2.
        var icalendarContent = TestSuiteHelper.CreateCalendarContent(userThreeMailbox, userTwoMailbox, meetingRequestSubject, "4");
        var mimeContent = Common.CreateMeetingRequestMime(userThreeMailbox, userTwoMailbox, meetingRequestSubject, "Meeting Request", icalendarContent);
        var sendMailRequest = Common.CreateSendMailRequest(null, Guid.NewGuid().ToString(), mimeContent);
        var sendMailResponse = HTTPAdapter.HTTPPOST(CommandName.SendMail, null, sendMailRequest.GetRequestDataSerializedXML());
    ...
    protected string LoopToSyncItem(string folderCollectionId, string subject, bool isRetryNeeded)
    {
        ...
        do
        {
            ...
            var syncResponse = CallSyncCommand(folderCollectionId);
    ...
    protected Response.SearchResponseStoreResult LoopToSearchItem(string collectionId, string subject)
    {
        ...
        while (counter < upperBound)
        {
            ...
            var searchResponse = CallSearchCommand(collectionId, subject);
    ...
    {
        if (!string.IsNullOrEmpty(responseXML) && IsXml(responseXML))
        {
            var regexClass = new Regex(@"<Status>(.*?)</Status>", RegexOptions.Singleline);
            var match = regexClass.Match(responseXML);
            var statusValue = match.Groups[1].Value;
    ...
    protected void AddCreatedItemToCollection(string userName, string parentFolder, string itemSubject)
    {
        ...
        {
            case "User1":
                UserOneInformation.UserCreatedItems.Add(createdItems);
                break;
            case "User2":
                UserTwoInformation.UserCreatedItems.Add(createdItems);
                break;
            case "User3":
                UserThreeInformation.UserCreatedItems.Add(createdItems);
                break;
            default:
    ...
    {
        var queryValueType = Common.GetConfigurationPropertyValue("HeaderEncodingType", Site);
        ...
        var activeSyncProtocolVersion = Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site);
        requestPrefix.Add(HTTPPOSTRequestPrefixField.ActiveSyncProtocolVersion, Common.ConvertActiveSyncProtocolVersion(activeSyncProtocolVersion, Site));

        requestPrefix.Add(HTTPPOSTRequestPrefixField.AcceptEncoding, null);
        requestPrefix.Add(HTTPPOSTRequestPrefixField.PrefixOfURI, Common.GetConfigurationPropertyValue("TransportType", Site));
        requestPrefix.Add(HTTPPOSTRequestPrefixField.Host, Common.GetConfigurationPropertyValue("SutComputerName", Site));
    ...
    private void DeleteCreatedItems(Collection<CreatedItems> itemsToDelete)
    {
        ...
        {
            var syncResponse = CallSyncCommand(itemToDelete.CollectionId);
            ...
            var deleteList = new List<Request.SyncCollectionDelete>();
            foreach (var itemSubject in itemToDelete.ItemSubject)
            {
                var appData = new Request.SyncCollectionDelete
                {
            ...
            var syncCollections = new Request.SyncCollection[1];
            ...
            var syncRequest = Common.CreateSyncRequest(syncCollections);
    ...
    private string GetFileReferenceFromSearchResult(Response.SearchResponseStoreResult result)
    {
        ...
        {
            for (var i = 0; i < result.Properties.ItemsElementName.Length; i++)
            {
                if (result.Properties.ItemsElementName[i] == Response.ItemsChoiceType6.Attachments)
                {
                    var emailAttachments = result.Properties.Items[i] as Response.Attachments;
