...
public static class TestSuiteHelper
{
    ...
    public static string GetStatusCodeFromException(Exception webException)
    {
        ...
        {
            var leftParenthesis = exceptionMessage.IndexOf("(", StringComparison.OrdinalIgnoreCase);
            var rightParenthesis = exceptionMessage.IndexOf(")", StringComparison.OrdinalIgnoreCase);
    ...
    {
        var provisionResponse = new ProvisionResponse
        {
    ...
    public static SyncRequest GetSyncRequest(string collectionId, string syncKey)
    {
        ...
        var synCollections = new Request.SyncCollection[1];
        synCollections[0] = new Request.SyncCollection { SyncKey = syncKey, CollectionId = collectionId };
        var syncRequest = Common.CreateSyncRequest(synCollections);
        return syncRequest;
    }
    ...
    public static SyncStore LoadSyncResponse(ActiveSyncResponseBase<Response.Sync> response)
    {
        ...
        var result = new SyncStore();
        var collection = ((Response.SyncCollections)response.ResponseData.Item).Collection[0];
        for (var i = 0; i < collection.ItemsElementName.Length; i++)
        {
            switch (collection.ItemsElementName[i])
            {
                ...
                case Response.ItemsChoiceType10.Commands:
                    var commands = collection.Items[i] as Response.SyncCollectionsCollectionCommands;
                    if (commands != null)
                    {
                        foreach (var item in LoadAddCommands(commands))
                        {
                ...
                case Response.ItemsChoiceType10.Responses:
                    var responses = collection.Items[i] as Response.SyncCollectionsCollectionResponses;
                    if (responses != null)
                    {
    ...
    public static Collection<SyncItem> LoadAddCommands(Response.SyncCollectionsCollectionCommands collectionCommands)
    {
        ...
        {
            var commands = new Collection<SyncItem>();
            if (collectionCommands.Add.Length > 0)
            {
                foreach (var addCommand in collectionCommands.Add)
                {
                    var syncItem = new SyncItem { ServerId = addCommand.ServerId };
                    for (var i = 0; i < addCommand.ApplicationData.ItemsElementName.Length; i++)
                    {
    ...
    public static string GetServerIdFromSyncResponse(SyncStore syncResponse, string subject)
    {
        ...
        foreach (var add in syncResponse.AddCommands)
        {
    ...
    {
        var headerRP = false;
        var headerASProtocolCommands = false;
        var headerASProtocolVersions = false;
