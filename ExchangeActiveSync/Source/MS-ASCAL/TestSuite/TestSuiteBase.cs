...
using Common;
using TestTools;
using Calendar = Common.DataStructures.Calendar;
...
public class TestSuiteBase : TestClassBase
{
    ...
    {
        var recurrence = new Request.Recurrence { Type = recurrenceType };
        ...
        if (!IsActiveSyncProtocolVersion121 && (recurrence.Type == 2 || recurrence.Type == 3 || recurrence.Type == 5 || recurrence.Type == 6))
        {
    ...
    {
        var recurrence = recurrenceInstance;
    ...
    public Dictionary<Request.ItemsChoiceType8, object> CreateDefaultCalendar()
    {
        var calendarItem = new Dictionary<Request.ItemsChoiceType8, object>
        {
            ...
            {
                Request.ItemsChoiceType8.Body, TestSuiteHelper.CreateCalendarBody(1, Content)
            }
        };

        if (Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site) != "12.1"
            && Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site) != "14.0"
            && Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site) != "14.1")
        {
            calendarItem.Add(Request.ItemsChoiceType8.ClientUid, Guid.NewGuid().ToString());
        }
        else
        {
            calendarItem.Add(Request.ItemsChoiceType8.OrganizerEmail, Common.GetMailAddress(User1Information.UserName, User1Information.UserDomain));
            calendarItem.Add(Request.ItemsChoiceType8.OrganizerName, User1Information.UserName);
    ...
    {
        foreach (var itemsToFolder in createdItemsCollection)
        {
            var result = SyncChanges(itemsToFolder.CollectionId);
            ...
            if (result.AddElements != null)
            {
                ...
                {
                    if (itemsToFolder.CollectionId == CurrentUserInformation.CalendarCollectionId)
                    {
                        foreach (var subject in itemsToFolder.ItemSubject)
                        {
                            if (item.Calendar != null)
                            {
                                if (item.Calendar.Subject.Equals(subject, StringComparison.CurrentCultureIgnoreCase))
                                {
                                    ...
                                    var deleteResult = CALAdapter.Sync(deleteRequest);
                    ...
                    if (itemsToFolder.CollectionId == CurrentUserInformation.InboxCollectionId || itemsToFolder.CollectionId == CurrentUserInformation.DeletedItemsCollectionId || itemsToFolder.CollectionId == CurrentUserInformation.SentItemsCollectionId)
                    {
                        foreach (var subject in itemsToFolder.ItemSubject)
                        {
                            if (item.Email != null)
    ...
    {
        var itemOperationsRequest = TestSuiteHelper.CreateItemOperationsEmptyFolderContentsRequest(collectionId);
        var emptyFolderContentsResponse = CALAdapter.ItemOperations(itemOperationsRequest);
    ...
    public SyncStore AddSyncCalendar(Dictionary<Request.ItemsChoiceType8, object> items)
    {
        ...
        {
            foreach (var response in syncCalendarResponse.AddResponses)
            {
                if (response.Status.Equals(byte.Parse("1")))
                {
                    ...
                    syncCalendarResponse = SyncChanges(CurrentUserInformation.CalendarCollectionId);
                }
                else
                {
    ...
    public SyncStore SyncChanges(string collectionId)
    {
        ...
        do
        {
            ...
            // Sync to get the SyncKey
            var initializeSyncResponse = InitializeSync(collectionId, null);

            // Get the server changes through sync command
            var syncRequest = TestSuiteHelper.CreateSyncRequest(collectionId, initializeSyncResponse.SyncKey, true);
            syncResponse = CALAdapter.Sync(syncRequest);
    ...
    public SyncStore InitializeSync(string collectionId, Request.Supported supported)
    {
        ...
        var initializeSyncRequest = TestSuiteHelper.InitializeSyncRequest(collectionId, supported);
        var initializeSyncResponse = CALAdapter.Sync(initializeSyncRequest);
    ...
    public SyncItem GetChangeItem(string collectionId, string calendarSubject)
    {
        ...
        if (collectionId == CurrentUserInformation.CalendarCollectionId)
        {
            // Get the server changes through sync command
            syncResponse = SyncChanges(collectionId);

            if (syncResponse != null && syncResponse.CollectionStatus == 1)
            {
                foreach (var item in syncResponse.AddElements)
                {
        ...
        else if (collectionId == CurrentUserInformation.InboxCollectionId || collectionId == CurrentUserInformation.DeletedItemsCollectionId)
        {
            var waitTime = int.Parse(Common.GetConfigurationPropertyValue("WaitTime", Site));
            var retryCount = int.Parse(Common.GetConfigurationPropertyValue("RetryCount", Site));
            var counter = 0;
    ...
    {
        var initialSyncResponse = InitializeSync(CurrentUserInformation.CalendarCollectionId, null);
        var sendStringResponse = new SendStringResponse();
        ...
        var response = new SyncStore();
        var doc = new XmlDocument();
        doc.LoadXml(sendStringResponse.ResponseDataXML);
        var nodes = doc.DocumentElement.GetElementsByTagName("Collection");
            ...
            foreach (XmlNode item in node.ChildNodes)
            {
                ...
                if (item.Name == "Responses")
                {
                    foreach (XmlNode add in item)
                    {
                        if (add.Name == "Add")
                        {
                            var res = new Response.SyncCollectionsCollectionResponsesAdd();
    ...
    public SyncStore UpdateCalendarProperty(string serverId, string collectionId, string syncKey, Dictionary<Request.ItemsChoiceType7, object> items)
    {
        ...
        var syncChanageResponse = CALAdapter.Sync(syncChangeRequest);
    ...
    {
        CALAdapter.SwitchUser(userInformation.UserName, userInformation.UserPassword, userInformation.UserDomain);

        if (isFolderSyncNeeded)
        {
            var folderSyncResponse = CALAdapter.FolderSync();
            ...
            if (userInformation.UserName == User2Information.UserName)
            {
                ...
                if (string.IsNullOrEmpty(User2Information.CalendarCollectionId))
                {
                    User2Information.CalendarCollectionId = Common.GetDefaultFolderServerId(folderSyncResponse, FolderType.Calendar, Site);
                }

                if (string.IsNullOrEmpty(User2Information.SentItemsCollectionId))
                {
                    User2Information.SentItemsCollectionId = Common.GetDefaultFolderServerId(folderSyncResponse, FolderType.SentItems, Site);
                }

                if (string.IsNullOrEmpty(User2Information.DeletedItemsCollectionId))
                {
                    User2Information.DeletedItemsCollectionId = Common.GetDefaultFolderServerId(folderSyncResponse, FolderType.DeletedItems, Site);
                }
            }
    ...
    {
        var icalendarContent = string.Empty;
        ...
        var mime = TestSuiteHelper.CreateMeetingRequestMime(
        ...
        var sendMailRequest = TestSuiteHelper.CreateSendMailRequest(TestSuiteHelper.Next(), false, mime);
        var sendMailResponse = CALAdapter.SendMail(sendMailRequest);
    ...
    {
        IsActiveSyncProtocolVersion121 = false;
        IsActiveSyncProtocolVersion140 = false;
        IsActiveSyncProtocolVersion141 = false;
        IsActiveSyncProtocolVersion160 = false;
        IsActiveSyncProtocolVersion161 = false;

        if (CALAdapter == null)
        {
            CALAdapter = BaseTestSite.GetAdapter<IMS_ASCALAdapter>();
        }
        ...
        User1Information = new UserInformation()
        {
            ...
            UserPassword = Common.GetConfigurationPropertyValue("OrganizerUserPassword", Site),
        ...
        {
            UserName = Common.GetConfigurationPropertyValue("AttendeeUserName", Site),
            UserPassword = Common.GetConfigurationPropertyValue("AttendeeUserPassword", Site),
            UserDomain = Common.GetConfigurationPropertyValue("Domain", Site)
        };
        CurrentUserInformation = User1Information;
        ActiveSyncProtocolVersion = Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site);

        if (ActiveSyncProtocolVersion.Equals("12.1"))
        {
            IsActiveSyncProtocolVersion121 = true;
        }
        else if (ActiveSyncProtocolVersion.Equals("14.0"))
        {
            IsActiveSyncProtocolVersion140 = true;
        }
        else if (ActiveSyncProtocolVersion.Equals("14.1"))
        {
            IsActiveSyncProtocolVersion141 = true;
        }
        else if (ActiveSyncProtocolVersion.Equals("16.0"))
        {
            IsActiveSyncProtocolVersion160 = true;
        }
        else if (ActiveSyncProtocolVersion.Equals("16.1"))
        {
            IsActiveSyncProtocolVersion161 = true;
        }
            
        if (Common.GetSutVersion(Site) != SutVersion.ExchangeServer2007 || IsActiveSyncProtocolVersion121)
        {
            SwitchUser(User1Information, true);
        }

        SubjectName = Common.GenerateResourceName(Site, "subject");
        Location = Common.GenerateResourceName(Site, "location");
        Content = Common.GenerateResourceName(Site, "content");
        Category = Common.GenerateResourceName(Site, "category");
            
        // Set StartTime as tomorrow, EndTime as 1 hour after StartTime, PastTime as 4 days ago, FutureTime as 5 days from now.
        StartTime = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 8, 0, 0).AddDays(1);
        EndTime = StartTime.AddHours(1);
        PastTime = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 9, 0, 0).AddDays(-4);
        FutureTime = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 9, 0, 0).AddDays(5);
    }
