...
internal static class TestSuiteHelper
{
    ...
    internal static SyncRequest CreateSyncAddRequest(string collectionId, string syncKey, Request.SyncCollectionAddApplicationData addCalendars)
    {
        ...
        syncAddRequest = CreateSyncRequest(collectionId, syncKey, false);
    ...
    {
        var itemOperations = new Request.ItemOperations();
        ...
        foreach (var item in itemOperations.Items)
        {
            var fetch = item as Request.ItemOperationsFetch;
            if (fetch != null)
            {
    ...
    {
        var emptyFolderContents = new Request.ItemOperationsEmptyFolderContents
        {
            CollectionId = collectionId,
    ...
    {
        var icalendar = new StringBuilder();
...
";
    ...
    internal static Request.Attendees CreateAttendeesRequired(string[] attendeeEmailAddress, string[] attendeeName)
    {
        ...
        for (var i = 0; i < attendeeEmailAddress.Length; i++)
        {
        ...
        var attendees = new Request.Attendees
        {
    ...
    {
        var calendarBody = new Request.Body
        {
            Type = bodyType,
            Data = content
        };
    ...
    {
        var calendarCategories = new Request.Categories();
