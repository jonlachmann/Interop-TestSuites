...
public class S01_SyncCommand : TestSuiteBase
{
    ...
    public void MSASTASK_S01_TC01_CreateTaskItemRecursDaily()
    {
        ...
        if (Common.IsRequirementEnabled(631, Site))
        {
            ...
            var dayOfWeekIndex = stringRequest.IndexOf("<DayOfWeek>");
            var dayOfWeekEndIndex = stringRequest.IndexOf("</DayOfWeek>") + 11;
            stringRequest = stringRequest.Remove(dayOfWeekIndex, dayOfWeekEndIndex - dayOfWeekIndex + 1);
            sendStringResponse = TASKAdapter.SendStringRequest(stringRequest, CommandName.Sync);
        }
        ...
        var syncedTaskItem = GetChangeItem(UserInformation.TasksCollectionId, subject);
    ...
    public void MSASTASK_S01_TC02_CreateTaskItemRecursDailyWithoutStart()
    {
        ...
        var recurrence = new Request.Recurrence1
        {
            Type = 0,
    ...
    public void MSASTASK_S01_TC03_CreateTaskItemFailWithDayOfMonth()
    {
        ...
        taskItem.Add(Request.ItemsChoiceType8.Recurrence1, recurrence);
        syncResponse = SyncAddTask(taskItem);
    ...
    public void MSASTASK_S01_TC04_CreateTaskItemRecursMonthly()
    {
        ...
        var categories = new Request.Categories3 { Category = "Business,Waiting".Split(',') };
    ...
    public void MSASTASK_S01_TC06_CreateTaskItemRecursWeekly()
    {
        ...
        var startTime = DateTime.Now;
        var utcStartTime = startTime.ToUniversalTime();
        var until = startTime.AddDays(21);
        ...
        // Extract status code from string response
        var response = ExtractSyncStore(sendStringResponse);
        Site.Assert.AreEqual<int>(1, int.Parse(response.AddResponses[0].Status), "Task item should be created successfully.");
            ...
            foreach (XmlNode item in node.ChildNodes)
            {
                if (item.Name == "Responses")
                {
                    foreach (XmlNode add in item)
                    {
                        if (add.Name == "Add")
                        {
                            var responseData = new Response.SyncCollectionsCollectionResponsesAdd();
    ...
    public void MSASTASK_S01_TC18_CreateAndChangeSubject()
    {
        ...
        var changeInTaskFolder = SyncChanges(UserInformation.TasksCollectionId);

        #endregion

        SyncItem syncedTaskItem = null;
        foreach (var item in changeInTaskFolder.AddElements)
        {
        ...
        var newSubject = Common.GenerateResourceName(Site, "new subject");
        var changedPropertyValue = new Dictionary<Request.ItemsChoiceType7, object>
        {
            {
        ...
        var updateTaskResponse = SyncChangeTask(changeInTaskFolder.SyncKey, syncedTaskItem.ServerId, changedPropertyValue);
        ...
        var updatedTaskItem = GetChangeItem(UserInformation.TasksCollectionId, newSubject);
    ...
    public void MSASTASK_S01_TC19_CreateTaskItemCategoriesWithoutChildElements()
    {
        ...
        var categories = new Request.Categories3();
        taskItem.Add(Request.ItemsChoiceType8.Categories3, categories);
