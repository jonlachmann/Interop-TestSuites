...
public partial class MS_ASCMDAdapter : ManagedAdapterBase, IMS_ASCMDAdapter
{
    ...
    {
        Site.Assert.IsTrue(activeSyncClient.ValidationResult, "The schema validation result should be true.");
    ...
    private void VerifyElementsForResponses(XmlElement element)
    {
        ...
        {
            var addNodes = element.GetElementsByTagName("Add");
            var changeNodes = element.GetElementsByTagName("Change");
            var fetchNodes = element.GetElementsByTagName("Fetch");
                ...
                foreach (XmlNode addNode in addNodes)
                {
                    if (addNode != null)
                    {
                        ...
                        foreach (XmlNode addChildNode in addNode.ChildNodes)
                        {
                            ...
                            {
                                hasStatus = true;
                                VerifyStatusElementForSync();
                            }

                            if (addChildNode.Name.Equals("ServerId", StringComparison.CurrentCultureIgnoreCase))
                            {
                                VerifyServerIdElementForSyncResponses(addChildNode.InnerXml);
                            }
                ...
                foreach (XmlNode changeNode in changeNodes)
                {
                    if (changeNode != null)
                    {
                        ...
                        foreach (XmlNode changeChildNode in changeNode.ChildNodes)
                        {
                            ...
                            {
                                VerifyServerIdElementForSyncResponses(changeChildNode.InnerXml);
                            }
                        }
                ...
                foreach (XmlNode fetchNode in fetchNodes)
                {
                    if (fetchNode != null)
                    {
                        ...
                        foreach (XmlNode fetchChildNode in fetchNode.ChildNodes)
                        {
                            ...
                            {
                                VerifyServerIdElementForSyncResponses(fetchChildNode.InnerXml);
                            }
    ...
    private void VerifyAutodiscoverCommand(Microsoft.Protocols.TestSuites.Common.AutodiscoverResponse autodiscoverResponse)
    {
        ...
        {
            var response = (Response)autodiscoverResponse.ResponseData.Item;
            ...
            if (response.Action != null)
            {
                ...
                if (((Response)autodiscoverResponse.ResponseData.Item).Action.Settings != null && ((Response)autodiscoverResponse.ResponseData.Item).Action.Settings.Length > 0)
                {
                    ...
                    foreach (var autodiscoverResponseActionServer in ((Response)autodiscoverResponse.ResponseData.Item).Action.Settings)
                    {
                ...
                if (((Response)autodiscoverResponse.ResponseData.Item).Action.Error != null)
                {
                    ...
                    {
                        VerifyMessageElement(((Response)autodiscoverResponse.ResponseData.Item).Action.Error.Message);
                    }

                    if (((Response)autodiscoverResponse.ResponseData.Item).Action.Error.DebugData != null)
                    {
                        VerifyDebugDataElement(((Response)autodiscoverResponse.ResponseData.Item).Action.Error.DebugData);
                    }

                    if (((Response)autodiscoverResponse.ResponseData.Item).Action.Error.ErrorCode != null)
                    {
                        VerifyErrorCodeElement(((Response)autodiscoverResponse.ResponseData.Item).Action.Error.ErrorCode);
                    }
                }
                #endregion
            }
        ...
        else if (autodiscoverResponse.ResponseData.Item is Microsoft.Protocols.TestSuites.Common.Response.AutodiscoverResponse)
        {
            ...
            var response = (Microsoft.Protocols.TestSuites.Common.Response.AutodiscoverResponse)autodiscoverResponse.ResponseData.Item;
            if (response.Error != null)
            {
                ...
                if (response.Error.Time != null)
                {
                    ...
                    VerifyTimeAttribute(((Microsoft.Protocols.TestSuites.Common.Response.AutodiscoverResponse)autodiscoverResponse.ResponseData.Item).Error.Time);
                }
                ...
                {
                    VerifyErrorCodeElement(response.Error.ErrorCode);
                }

                if (response.Error.Message != null)
                {
                    VerifyMessageElement(response.Error.Message);
                }
                ...
                {
                    VerifyDebugDataElement(response.Error.DebugData);
                }
            }
            #endregion
        }
    ...
    private void VerifyTimeAttribute(string time)
    {
        ...
        var minutesValue = Convert.ToInt32(minutes);

        Site.Assert.IsTrue(
            minutesValue >= 0 && minutesValue <= 59,
            "The value for seconds should be valid.");

        var hours = subTimeArr[0];
        ...
        var hoursValue = Convert.ToInt32(hours);
    ...
    private void VerifyFolderSyncCommand(FolderSyncResponse folderSyncResponse)
    {
        ...
        if (folderSyncResponse.ResponseData.Changes != null)
        {
            ...
            if (folderSyncResponse.ResponseData.Changes.Delete != null && folderSyncResponse.ResponseData.Changes.Delete.Length > 0)
            {
                ...
                foreach (var folderSyncChangesDelete in folderSyncResponse.ResponseData.Changes.Delete)
                {
            ...
            if (folderSyncResponse.ResponseData.Changes.Update != null && folderSyncResponse.ResponseData.Changes.Update.Length > 0)
            {
                ...
                foreach (var folderSyncChangesUpdate in folderSyncResponse.ResponseData.Changes.Update)
                {
                    #region Capture code for ServerId
            ...
            if (folderSyncResponse.ResponseData.Changes.Add != null && folderSyncResponse.ResponseData.Changes.Add.Length > 0)
            {
                ...
                foreach (var folderSyncChangesAdd in folderSyncResponse.ResponseData.Changes.Add)
                {
    ...
    private void VerifyGetItemEstimateCommand(Microsoft.Protocols.TestSuites.Common.GetItemEstimateResponse getItemEstimateResponse)
    {
        ...
        {
            foreach (var response in getItemEstimateResponse.ResponseData.Response)
            {
                #region Capture code for Status
                // Add the debug information.
    ...
    private void VerifyItemOperationsCommand(Microsoft.Protocols.TestSuites.Common.ItemOperationsResponse itemOperationsResponse)
    {
        ...
        if (itemOperationsResponse.ResponseData.Response != null)
        {
            ...
            {
                foreach (var content in itemOperationsResponse.ResponseData.Response.EmptyFolderContents)
                {
...
,Status (section 2.2.3.162.7).");
            ...
            {
                foreach (var move in itemOperationsResponse.ResponseData.Response.Move)
                {
                    // Add the debug information.
            ...
            {
                foreach (var fetch in itemOperationsResponse.ResponseData.Response.Fetch)
                {
                    // Add the debug information.
                    ...
                    if (fetch.Properties != null)
                    {
                        ...
                        for (var j = 0; j < fetch.Properties.ItemsElementName.Length; j++)
                        {
                            ...
                            {
                                var data = (string)fetch.Properties.Items[j];
                            ...
                            {
                                var version = (DateTime)fetch.Properties.Items[j];
    ...
    private void VerifyMeetingResponseCommand(MeetingResponseResponse meetingResponse)
    {
        ...
        foreach (var result in meetingResponse.ResponseData.Result)
        {
    ...
    private void VerifyPingCommand(PingResponse pingResponse)
    {
        ...
        if (pingResponse.ResponseData.Folders != null && pingResponse.ResponseData.Folders.Length > 0)
        {
            ...
            foreach (var folder in pingResponse.ResponseData.Folders)
            {
    ...
    private void VerifyResolveRecipientsCommand(Microsoft.Protocols.TestSuites.Common.ResolveRecipientsResponse resolveRecipientsResponse)
    {
        ...
        if (resolveRecipientsResponse.ResponseData.Response != null)
        {
            ...
            VerifyStatusElementForResolveRecipients(status);
            ...
            {
                VerifyRecipientCountElement(Convert.ToInt32(resolveRecipientsResponse.ResponseData.Response[0].RecipientCount));
            ...
            if (resolveRecipientsResponse.ResponseData.Response[0].Recipient != null && resolveRecipientsResponse.ResponseData.Response[0].Recipient.Length > 0)
            {
                ...
                foreach (var recipient in resolveRecipientsResponse.ResponseData.Response[0].Recipient)
                {
                    #region Capture code for EmailAddress
                    ...
                    VerifyUnsignedByteDataType(recipient.Type);
                    ...
                    if (!Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site).Equals("12.1") && recipient.Availability != null)
                    {
                        // Add the debug information.
                        ...
                        if (recipient.Availability.MergedFreeBusy != null)
                        {
                            ...
                            var mergedFreeBusyRegex = new Regex("^[0-4]{0,32768}$");
                    ...
                    if (!Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site).Equals("12.1") && !Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site).Equals("14.0") && recipient.Picture != null && recipient.Picture.Length > 0)
                    {
                        // Add the debug information.
                        ...
                        foreach (var picture in recipient.Picture)
                        {
                            #region Capture code for Status
                    ...
                    if (recipient.Certificates != null)
                    {
                        ...
                        #region Capture code for Status
                        var certificatesStatus = int.Parse(recipient.Certificates.Status);
                        ...
                        {
                            VerifyRecipientCountElement(Convert.ToInt32(recipient.Certificates.RecipientCount));
                        ...
                        {
                            for (var i = 0; i < recipient.Certificates.Certificate.Length; i++)
                            {
                                // Add the debug information.
    ...
    private void VerifySearchCommand(Microsoft.Protocols.TestSuites.Common.SearchResponse searchResponse)
    {
        ...
        if (searchResponse.ResponseData.Response != null)
        {
            ...
            {
                var rangeRegex = new Regex("0-[0-9]*");
                ...
                foreach (var result in searchResponse.ResponseData.Response.Store.Result)
                {
                    ...
                    if (result.Properties != null)
                    {
                        ...
                        if (result.Properties.ItemsElementName != null && result.Properties.ItemsElementName.Length > 0)
                        {
                            ...
                            for (var j = 0; j < result.Properties.ItemsElementName.Length; j++)
                            {
                                #region Capture code for Picture
                                ...
                                {
                                    var picture = (SearchResponseStoreResultPropertiesPicture)result.Properties.Items[j];
...
Total (section 2.2.3.184.1).");
            ...
            {
                foreach (var result in findResponse.ResponseData.Response.Results)
                {
                    // Add the debug information.
            ...
            if (findResponse.ResponseData.Response.Range != null)
            {
                ...
                var m=uint.Parse(findResponse.ResponseData.Response.Range.Split('-')[0]);
                var n = uint.Parse(findResponse.ResponseData.Response.Range.Split('-')[1]);
        ...
        if (settingsResponse.ResponseData.Oof != null)
        {
            ...
            if (settingsResponse.ResponseData.Oof.Get != null)
            {
                ...
                if (settingsResponse.ResponseData.Oof.Get.OofMessage != null && settingsResponse.ResponseData.Oof.Get.OofMessage.Length > 0)
                {
                    ...
                    foreach (var oofMessage in settingsResponse.ResponseData.Oof.Get.OofMessage)
                    {
                    ...
                    {
                        var appliesToInternalNodes = xmlDoc.DocumentElement.GetElementsByTagName("AppliesToInternal");
                        var appliesToExternalKnownNodes = xmlDoc.DocumentElement.GetElementsByTagName("AppliesToExternalKnown");
                        var appliesToExternalUnknownNodes = xmlDoc.DocumentElement.GetElementsByTagName("AppliesToExternalUnknown");
...
OofMessage (section 2.2.3.117)");
        ...
        if (settingsResponse.ResponseData.UserInformation != null)
        {
            ...
            var hasEmailAddresses = false;
            var hasAccounts = false;
            #region Capture code for EmailAddresses
            if (Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site).Equals("12.1") || Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site).Equals("14.0"))
            {
                ...
                if (settingsResponse.ResponseData.UserInformation.Get.EmailAddresses != null)
                {
                    ...
                    VerifyEmailAddresses(settingsResponse.ResponseData.UserInformation.Get.EmailAddresses);
                }
            }
            #endregion
            ...
            if (settingsResponse.ResponseData.UserInformation.Get.Accounts != null && settingsResponse.ResponseData.UserInformation.Get.Accounts.Length > 0)
            {
                ...
                foreach (var account in settingsResponse.ResponseData.UserInformation.Get.Accounts)
                {
                    ...
                    {
                        VerifyEmailAddresses(account.EmailAddresses);
                    }
    ...
    private void VerifyEmailAddresses(EmailAddresses emailAddresses)
    {
        ...
        var hasPrimarySmtpAddress = false;
    ...
    private void VerifySyncCommand(SyncResponse syncResponse)
    {
        ...
        {
            var syncCollections = (SyncCollections)syncResponse.ResponseData.Item;
            ...
            if (syncCollections.Collection != null && syncCollections.Collection.Length > 0)
            {
                ...
                foreach (var collection in syncCollections.Collection)
                {
                    if (collection.ItemsElementName != null && collection.ItemsElementName.Length > 0)
                    {
                        var hasStatus = false;
                        var hasSyncKey = false;
                        var hasCollectionId = false;

                        for (var j = 0; j < collection.ItemsElementName.Length; j++)
                        {
                            #region Capture code for Status
                            ...
                            {
                                var commands = (SyncCollectionsCollectionCommands)collection.Items[j];

                                var hasAddCommand = commands.Add != null && commands.Add.Length > 0;
                                var hasDeleteCommand = commands.Delete != null && commands.Delete.Length > 0;
                                var hasChangeCommand = commands.Change != null && commands.Change.Length > 0;
                                var hasSoftDeleteCommand = commands.SoftDelete != null && commands.SoftDelete.Length > 0;
                                ...
                                if (hasAddCommand)
                                {
                                    ...
                                    foreach (var add in commands.Add)
                                    {
                                        ...
                                        VerifyServerIdElementForSync(add.ServerId);

                                        Site.Assert.IsNotNull(add.ApplicationData, "The ApplicationData element in Sync command should not be null.");

                                        VerifyApplicationDataForSyncAddChange();
                                        for (var i = 0; i < add.ApplicationData.ItemsElementName.Length; i++)
                                        {
                                            if (add.ApplicationData.ItemsElementName[i].ToString() == "MeetingRequst")
                                            {
                                                if (add.ApplicationData.Items[i] != null)
                                                {
                                                    if (((MeetingRequest)add.ApplicationData.Items[i]).Forwardees != null)
                                                    {
                                                        ...
                                                        foreach (var forwardee in ((MeetingRequest)add.ApplicationData.Items[i]).Forwardees)
                                                        {
                                                            if (forwardee != null)
                                                            {
                                                                VerifyForwardeeElementForSyncResponses();
                                            ...
                                            if (add.ApplicationData.ItemsElementName[i].ToString() == "Attendees")
                                            {
                                                if (add.ApplicationData.Items[i] != null)
                                                {
                                                    ...
                                                    {
                                                        VerifyForwardeesElementForSyncResponses();
                                                        foreach (var attende in ((Attendees)add.ApplicationData.Items[i]).Attendee)
                                                        {
                                            ...
                                            if (change.ApplicationData.ItemsElementName[i].ToString() == "MeetingRequst")
                                            {
                                                if (change.ApplicationData.Items[i] != null)
                                                {
                                                    if (((MeetingRequest)change.ApplicationData.Items[i]).Forwardees != null)
                                                    {
                                                        ...
                                                        foreach (var forwardee in ((MeetingRequest)change.ApplicationData.Items[i]).Forwardees)
                                                        {
                                                            if (forwardee!=null)
                                                            {
                                                                VerifyForwardeeElementForSyncResponses();
                            ...
                            {
                                #region Capture code for Change
                                var changeNodes = xmlDoc.DocumentElement.GetElementsByTagName("Change");
                                ...
                                var responsesNodes = xmlDoc.DocumentElement.GetElementsByTagName("Responses");
                                var responseNodes = xmlDoc.DocumentElement.GetElementsByTagName("Response");
                                ...
                                if (responsesNodes != null && responsesNodes.Count > 0)
                                {
                                    foreach (XmlNode responsesNode in responsesNodes)
                                    {
                                        if (responsesNode.ParentNode.Name.Equals("Collection", StringComparison.CurrentCultureIgnoreCase) && responsesNode.ParentNode.ParentNode != null && responsesNode.ParentNode.ParentNode.Name.Equals("Collections", StringComparison.CurrentCultureIgnoreCase) && responsesNode.ParentNode.ParentNode.ParentNode != null && responsesNode.ParentNode.ParentNode.ParentNode.Name.Equals("Sync", StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            ...
                                            VerifyElementsForResponses(responsesNode as XmlElement);
                                        }
                                    }
                                }
                                ...
                                if (responseNodes != null && responseNodes.Count > 0)
                                {
                                    foreach (XmlNode responseNode in responseNodes)
                                    {
                                        ...
                                        {
                                            VerifyElementsForResponses(responseNode as XmlElement);
                                        }
                                    }
                                }
                                #endregion
    ...
    private void VerifyValidateCertCommand(ValidateCertResponse validateCertResponse)
    {
        ...
        var validateCertStatusValue = AdapterHelper.PickUpRootStatusValueFromXMLString(validateCertResponse.ResponseDataXML);
        var certificateStatusValue = AdapterHelper.GetValidateCertStatusCode(validateCertResponse);
        ...
        VerifyStatusElementForValidateCert();
    ...
    private void VerifyGetHierarchyCommand(GetHierarchyResponse response)
    {
        ...
        foreach (var folder in response.ResponseData.Folder)
        {
            VerifyFolderElement(folder);
        }
    }
    ...
    private void VerifyFolderElement(FoldersFolder folder)
    {
        ...
        var isVerifiedR7388 = folder.Type == 1 || folder.Type == 2 || folder.Type == 3 || folder.Type == 4 || folder.Type == 5 || folder.Type == 6;
    ...
    private void VerifyContainerDataType()
    {
        ...
        Site.CaptureRequirementIfIsTrue(
            activeSyncClient.ValidationResult,
            "MS-ASDTYPE",
            8,
...
MSS = Number of milliseconds");
    ...
    private void VerifyWBXMLCapture(CommandName cmdName, object response)
    {
        ...
        msaswbxmlImplementation = activeSyncClient.GetMSASWBXMLImplementationInstance();
        ...
        var adapterHelper = new AdapterHelper();
        var statusOfResponses = adapterHelper.GetStatusFromResponses(response);
        ...
        if (statusOfResponses != 102)
        {
            ...
            var decodeData = msaswbxmlImplementation.DecodeDataCollection;

            foreach (var decodeDataItem in decodeData)
            {
                byte token;
                var tagName = Common.GetTagName(decodeDataItem.Key, out token);
                var codePageName = Common.GetCodePageName(decodeDataItem.Key);
                var currentCodepage = decodeDataItem.Value;

                var isValidCodePage = currentCodepage >= 0 && currentCodepage <= 25;
                ...
                CaptureCodePageRequirement(currentCodepage, codePageName, tagName, token);
    ...
    private void CaptureCodePageRequirement(int codePageNumber, string codePageName, string tagName, byte token)
    {
        ...
        switch (codePageNumber)
        {
            case 0:
            {
                ...
                CaptureRequirementsRelateToCodePage0(codePageNumber, tagName, token);
                break;
            }

            case 1:
            {
                CaptureRequirementsRelateToCodePage1(codePageNumber, tagName, token);
                break;
            }

            case 2:
            {
                CaptureRequirementsRelateToCodePage2(codePageNumber, tagName, token);
                break;
            }
            ...
            case 4:
            {
                CaptureRequirementsRelateToCodePage4(codePageNumber, tagName, token);
                break;
            }
            ...
            case 5:
            {
                ...
                CaptureRequirementsRelateToCodePage5(codePageNumber, tagName, token);
                break;
            }
            ...
            case 6:
            {
                ...
                CaptureRequirementsRelateToCodePage6(codePageNumber, tagName, token);
                break;
            }
            ...
            case 7:
            {
                ...
                CaptureRequirementsRelateToCodePage7(codePageNumber, tagName, token);
                break;
            }
            ...
            case 8:
            {
                ...
                CaptureRequirementsRelateToCodePage8(codePageNumber, tagName, token);
                break;
            }

            case 9:
            {
                CaptureRequirementsRelateToCodePage9(codePageNumber, tagName, token);
                break;
            }
            ...
            case 10:
            {
                ...
                CaptureRequirementsRelateToCodePage10(codePageNumber, tagName, token);
                break;
            }
            ...
            case 11:
            {
                ...
                CaptureRequirementsRelateToCodePage11(codePageNumber, tagName, token);
                break;
            }

            case 12:
            {
                CaptureRequirementsRelateToCodePage12(codePageNumber, tagName, token);
                break;
            }
            ...
            case 13:
            {
                ...
                CaptureRequirementsRelateToCodePage13(codePageNumber, tagName, token);
                break;
            }

            case 14:
            {
                CaptureRequirementsRelateToCodePage14(codePageNumber, tagName, token);
                break;
            }
            ...
            case 15:
            {
                ...
                CaptureRequirementsRelateToCodePage15(codePageNumber, tagName, token);
                break;
            }
            ...
            case 16:
            {
                ...
                CaptureRequirementsRelateToCodePage16(codePageNumber, tagName, token);
                break;
            }
            ...
            case 17:
            {
                ...
                CaptureRequirementsRelateToCodePage17(codePageNumber, tagName, token);
                break;
            }
            ...
            case 18:
            {
                ...
                CaptureRequirementsRelateToCodePage18(codePageNumber, tagName, token);
                break;
            }

            case 19:
            {
                CaptureRequirementsRelateToCodePage19(codePageNumber, tagName, token);
                break;
            }
            ...
            case 20:
            {
                ...
                CaptureRequirementsRelateToCodePage20(codePageNumber, tagName, token);
                break;
            }
            ...
            case 21:
            {
                ...
                CaptureRequirementsRelateToCodePage21(codePageNumber, tagName, token);
                break;
            }

            case 22:
            {
                CaptureRequirementsRelateToCodePage22(codePageNumber, tagName, token);
                break;
            }

            case 23:
            {
                CaptureRequirementsRelateToCodePage23(codePageNumber, tagName, token);
                break;
            }

            case 24:
            {
                CaptureRequirementsRelateToCodePage24(codePageNumber, tagName, token);
                break;
            }
        }
    }
