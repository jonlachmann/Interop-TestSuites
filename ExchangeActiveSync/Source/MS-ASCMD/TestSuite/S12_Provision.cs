...
public class S12_Provision : TestSuiteBase
{
    ...
    public void MSASCMD_S12_TC01_Provision_DownloadPolicy()
    {
        ...
        var policyKey = GetPolicyKeyFromResponse(provisionResponse);
    ...
    public void MSASCMD_S12_TC03_Provision_Status144()
    {
        ...
        // Get policyKey
        var finalPolicyKey = GetPolicyKeyFromResponse(provisionAcknowledgeResponse);
        #endregion

        #region Call FolderSync command with an invalid PolicyKey which is different from the one got from last step.
        CMDAdapter.ChangePolicyKey(finalPolicyKey.Substring(0, 1));
        RecordPolicyKeyChanged();
        ...
        if ("12.1" == Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site))
        {
            ...
            try
            {
                ...
                CMDAdapter.FolderSync(Common.CreateFolderSyncRequest("0"));
            }
    ...
    private static string GetPolicyKeyFromResponse(ProvisionResponse response)
    {
        ...
        {
            var policyInResponse = response.ResponseData.Policies.Policy;
