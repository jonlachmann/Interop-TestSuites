...
public class S23_Find : TestSuiteBase
{
    ...
    {
        Site.Assume.AreEqual<string>("16.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The Find command is only supported when the MS-ASProtocolVersion header is set to 16.1. MS-ASProtocolVersion header value is determined using Common PTFConfig property named ActiveSyncProtocolVersion.");

        #region Create a find request
        var findRequest = CreateDefaultFindMailRequest();
        #endregion

        #region Call Find command
        var findResponse = CMDAdapter.Find(findRequest);
    ...
    public void MSASCMD_S23_TC02_Find_Mail_InvalidRequest()
    {
        ...
        var findRequest = CreateFindMailRequest("",User1Information.InboxCollectionId);            
        #endregion
        ...
        if (Common.IsRequirementEnabled(72172521, Site))
        {
            // Add the debug information
    ...
    public void MSASCMD_S23_TC08_Find_Mail_FolderSyncRequired()
    {
        ...
        var findRequest = CreateFindMailRequest(Guid.NewGuid().ToString(), "-1");
        #endregion
    ...
    public void MSASCMD_S23_TC03_Find_Mail_InvalidRange()
    {
        ...
        #region Create a find request
        var findRequest = CreateFindMailRequest(Guid.NewGuid().ToString(), User1Information.InboxCollectionId, Common.GetConfigurationPropertyValue("User1Name", Site), "2-5"); 
        #endregion

        #region Call Find command
        var findResponse = CMDAdapter.Find(findRequest);
        if (Common.IsRequirementEnabled(72172518, Site))
        {                
    ...
    public void MSASCMD_S23_TC04_Find_GAL_Success()
    {
        ...
        var rangeEndIndex = 22;
        var requestRange = "0-" + rangeEndIndex.ToString();
        // New SearchId, keyWord="MSASCMD", range="0-22", maxPictures=0, maxSize=0
        var keyWord = Common.GetConfigurationPropertyValue("User1Name", Site).Substring(0, 6);
        var findRequest = CreateFindGALRequest(Guid.NewGuid().ToString(), keyWord, requestRange, 0, 0);
        #endregion
        ...
        var findResultCount = findResponse.ResponseData.Response.Results.Length;
        ...
        var displayName = GetElementsFromFindResponse(findResponse, Response.ItemsChoiceType14.DisplayName);
        var alias = GetElementsFromFindResponse(findResponse, Response.ItemsChoiceType14.Alias);
    ...
    public void MSASCMD_S23_TC05_Find_MatchedItems()
    {
        ...
        uint mailIndex = 1;
        var emailSubject = keyWord + Common.GenerateResourceName(Site, "find", mailIndex);
        ...
        mailIndex++;
        var emailSubject2 = keyWord + Common.GenerateResourceName(Site, "find", mailIndex);
        ...
        var findRequest = CreateFindMailRequest(Guid.NewGuid().ToString(), User2Information.InboxCollectionId, keyWord, "0-5", 0, 0);
        #endregion
        ...
        do
        {
            ...
            findResponse = CMDAdapter.Find(findRequest);
            Site.Assert.AreEqual("1", findResponse.ResponseData.Response.Status, "If server successfully completed command, server should return status 1");
    ...
    private FindRequest CreateDefaultFindMailRequest()
    {
        ...
        var findRequest = Common.CreateFindRequest(find);
        return findRequest;
    }
    ...
    {
        var findGALRequest = CreateDefaultFindGALRequest();
        if (searchId!=null)
        {
