...
public class S01_Autodiscover : TestSuiteBase
{
    ...
    {
        Site.Assume.IsFalse(Common.GetSutVersion(Site) == SutVersion.ExchangeServer2007 && string.Equals(Common.GetConfigurationPropertyValue("TransportType", Site).ToUpper(CultureInfo.InvariantCulture), "HTTP"), "Autodiscover request should be passed only through HTTPS to Exchange Server 2007.");

        var acceptableResponseSchema = Common.GetConfigurationPropertyValue("AcceptableResponseSchema", Site);
        ...
        var request = new AutodiscoverRequest
        {
            RequestData = new Request.Autodiscover
            {
                Request = new Request.RequestType
                {
                    ...
                    EMailAddress = Common.GetMailAddress(User1Information.UserName, User1Information.UserDomain)
                }
            }
        };

        var response = CMDAdapter.Autodiscover(request, ContentTypeEnum.Xml);

        var xmlDoc = new XmlDocument();
        xmlDoc.LoadXml(response.ResponseDataXML);
        var xmlElement = (XmlElement)xmlDoc.DocumentElement;
        var schemaNameSpace = xmlElement.GetElementsByTagName("Response")[0].NamespaceURI;
        ...
        if (Common.IsRequirementEnabled(5160, Site))
        {
            // Add the debug information
        ...
        if (Common.IsRequirementEnabled(5823, Site))
        {
            // Add the debug information
        ...
        if (Common.IsRequirementEnabled(5718, Site))
        {
            // Add the debug information
        ...
        if (Common.IsRequirementEnabled(5123, Site))
        {
        ...
        var request = new AutodiscoverRequest
        {
            RequestData = new Request.Autodiscover
            {
                Request = new Request.RequestType
                {
                    ...
                    EMailAddress = Common.GetMailAddress("InvallidEmailAddress", User1Information.UserDomain)
                }
            }
        };
    ...
    public void MSASCMD_S01_TC03_ErrorCode_600()
    {
        ...
        var requestText = request.GetRequestDataSerializedXML();
        var requestStartPosition = requestText.IndexOf("<Request>", StringComparison.OrdinalIgnoreCase);
        var requestEndPosition = requestText.IndexOf("</Autodiscover>", StringComparison.OrdinalIgnoreCase) - 1;
        var requestElementString = requestText.Substring(requestStartPosition, requestEndPosition - requestStartPosition + 1);
        requestText = requestText.Insert(requestEndPosition + 1, requestElementString);

        var response = CMDAdapter.SendStringRequest(CommandName.Autodiscover, null, requestText);
        var xmlDoc = new XmlDocument();
        xmlDoc.LoadXml(response.ResponseDataXML);
        var xmlElement = (XmlElement)xmlDoc.DocumentElement;
        var errorCode = xmlElement.GetElementsByTagName("ErrorCode")[0].InnerText;
