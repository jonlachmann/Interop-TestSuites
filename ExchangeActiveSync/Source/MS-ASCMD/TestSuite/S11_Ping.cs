...
public class S11_Ping : TestSuiteBase
{
    ...
    public void MSASCMD_S11_TC01_Ping_Status3()
    {
        ...
        CMDAdapter.SwitchUser(User3Information.UserName, User3Information.UserPassword, User3Information.UserDomain);

        // Record user that uses new DeviceID
        CMDAdapter.ChangeDeviceID("NewDevice");
        ...
        var pingRequest = CreatePingRequest(User3Information.NotesCollectionId, Request.PingFolderClass.Notes);
        pingRequest.RequestData.Folders = null;
        var pingReponse = CMDAdapter.Ping(pingRequest);
        #endregion
        ...
        var pingRequestWithRequireElement = CreatePingRequest(User3Information.NotesCollectionId, Request.PingFolderClass.Notes);
        var pingResponseWithRequireElement = CMDAdapter.Ping(pingRequestWithRequireElement);
        ...
        var pingRequestWithoutHeartBeatIntervalElement = CreatePingRequest(User3Information.NotesCollectionId, Request.PingFolderClass.Notes);
        pingRequestWithoutHeartBeatIntervalElement.RequestData.HeartbeatInterval = null;
        var pingReponseWithoutHeartBeatIntervalElement = CMDAdapter.Ping(pingRequestWithoutHeartBeatIntervalElement);
        #endregion
        ...
        ClearDevice(User3Information.UserName, User3Information.UserPassword, User3Information.UserDomain);
        #endregion
    }
    ...
    public void MSASCMD_S11_TC02_Ping_Status4()
    {
        ...
        var pingRequest = new PingRequest { RequestData = new Request.Ping() };
        var pingResponse = CMDAdapter.Ping(pingRequest);
        #endregion
    ...
    public void MSASCMD_S11_TC03_Ping_Status5()
    {
        ...
        var pingRequest = CreatePingRequest(User1Information.InboxCollectionId, Request.PingFolderClass.Email);
    ...
    public void MSASCMD_S11_TC04_Ping_Status7_SyncRequired()
    {
        ...
        // Record user that uses new DeviceID
        CMDAdapter.ChangeDeviceID("ChangeNewDevice");
        RecordDeviceInfoChanged();
        #endregion
    ...
    public void MSASCMD_S11_TC05_Ping_Status7_Outdate()
    {
        ...
        // Record user that uses new DeviceID
        RecordDeviceInfoChanged();
        CMDAdapter.ChangeDeviceID("FirstDeviceID");
        #endregion
        ...
        CMDAdapter.ChangeDeviceID("SecondDeviceID");
        var folderSynKey = GetFolderSyncKey();
        ...
        var newFolderName = Guid.NewGuid().ToString().Replace("-", string.Empty);
        var folderCreateRequest = Common.CreateFolderCreateRequest(folderSynKey, 12, newFolderName, User1Information.InboxCollectionId);
        ...
        var folderSyncRequestForNewFolder = Common.CreateFolderSyncRequest("0");
        var folderSyncResponseForNewFolder = CMDAdapter.FolderSync(folderSyncRequestForNewFolder);

        // Gets the server ID for new folder after change DeviceID.
        var serverId = GetCollectionId(folderSyncResponseForNewFolder, newFolderName);
        RecordCaseRelativeFolders(User3Information, serverId);
        #endregion
    }
    ...
    public void MSASCMD_S11_TC06_Ping_ChangesOccurred()
    {
        ...
        var folderCreateCount = 2;
        var folderIDList = new List<string>();
        for (var folderIndex = 0; folderIndex < folderCreateCount; folderIndex++)
        {
            var folderID = CreateFolder((byte)FolderType.UserCreatedMail, Common.GenerateResourceName(Site, "FolderCreate", (uint)(folderIndex + 1)), User2Information.InboxCollectionId);
        ...
        RecordCaseRelativeFolders(User2Information, folderIDList.ToArray());
        #endregion
        ...
        SwitchUser(User2Information);
        var syncInboxFolderResponse = GetMailItem(User2Information.InboxCollectionId, emailSubject);
        var emailItemServerID = FindServerId(syncInboxFolderResponse, "Subject", emailSubject);

        // User2 calls MoveItems command to move the email item from Inbox folder to one of created subfolder
        var moveItemsRequest = CreateMoveItemsRequest(emailItemServerID, User2Information.InboxCollectionId, folderIDList[0]);
        ...
        var pingRequest = CreateMultiFolderPingRequest(folderIDList, Request.PingFolderClass.Email);
        var pingResponse = CMDAdapter.Ping(pingRequest);
        ...
        if (Common.IsRequirementEnabled(5714, Site))
        {
            // Add the debug information
        ...
        pingResponse = CMDAdapter.Ping(pingRequest);
        #endregion
    ...
    {
        var pingFolder = new Request.PingFolder
        {
    ...
    {
        var pingRequestList = new List<Request.PingFolder>();
        foreach (var folderID in folderIDList)
        {
            var pingFolder = new Request.PingFolder { Class = pingFolderClass, Id = folderID };
    ...
    private string GetFolderSyncKey()
    {
        ...
        var folderSynKey = foldersyncResponse.ResponseData.SyncKey;
        return folderSynKey;
    }
    #endregion
}