...
public class S16_Settings : TestSuiteBase
{
    ...
    public void MSASCMD_S16_TC03_Settings_OofState()
    {
        ...
        var settingsRequestWithStartTimeOnly = new SettingsRequest
        {
            RequestData = new Request.Settings
            {
                Oof = new Request.SettingsOof
        ...
        #region Calls Settings command
        var settingsResponseWithStartTimeOnly = CMDAdapter.Settings(settingsRequestWithStartTimeOnly);
        #endregion
        ...
        var settingsRequestWithEndTimeOnly = new SettingsRequest
        {
            RequestData = new Request.Settings
            {
                Oof = new Request.SettingsOof
        ...
        #region Calls Settings command
        var settingsResponseWithEndTimeOnly = CMDAdapter.Settings(settingsRequestWithEndTimeOnly);
        #endregion
    ...
    public void MSASCMD_S16_TC04_Settings_UserInformation()
    {
        ...
        if (Common.IsRequirementEnabled(5898, Site))
        {
            // Add the debug information
        ...
        if (Common.IsRequirementEnabled(5901, Site))
        {
            // Add the debug information
        ...
        if (Common.IsRequirementEnabled(5905, Site))
        {
            // Add the debug information
        ...
        if (Common.IsRequirementEnabled(5915, Site))
        {
            // Add the debug information
        ...
        {
            var getElementStarIndex = settingsResponse.ResponseDataXML.IndexOf("<Get>", StringComparison.OrdinalIgnoreCase);
            var getElementEndIndex = settingsResponse.ResponseDataXML.IndexOf("</Get>", StringComparison.OrdinalIgnoreCase);
            var emailAddressElementIndex = settingsResponse.ResponseDataXML.IndexOf("<EmailAddresses>", StringComparison.OrdinalIgnoreCase);
    ...
    public void MSASCMD_S16_TC05_Settings_OofSetToExternalKnown()
    {
        ...
        var bodyType = "TEXT";
        var enabled = "1";
        var replyMessage = "Out of office";
        ...
        do
        {
            ...
            oofMessage = GetAppliesToExternalKnownOofMessage(settingsResponse);
            counter++;
        }
    ...
    public void MSASCMD_S16_TC06_Settings_OofSetToExternalUnknown()
    {
        ...
        do
        {
            Thread.Sleep(waitTime);
            var settingsResponse = GetOofSettings();
            oofMessage = GetAppliesToExternalUnknownOofMessage(settingsResponse);
            counter++;
        }
        while (counter < retryCount &&
    ...
    public void MSASCMD_S16_TC07_Settings_OofSetToInternal()
    {
        ...
        do
        {
            Thread.Sleep(waitTime);
            var settingsResponse = GetOofSettings();
            oofMessage = GetAppliesToInternalOofMessage(settingsResponse);
            counter++;
        }
        while (counter < retryCount &&
    ...
    public void MSASCMD_S16_TC10_Settings_OofSetForThreeTypeAudiences()
    {
        ...
        var bodyType = "TEXT";
        var internalEnabled = "1";
        var internalReplyMessage = "Out of office";
        var setEnableToInternalUser = CreateOofMessage(bodyType, internalEnabled, internalReplyMessage);
        setEnableToInternalUser.AppliesToInternal = string.Empty;

        // Set external known enabled
        var externalKnownBodyType = "TEXT";
        var externalKnownEnabled = "1";
        var externalKnownReplyMessage = "Call my mobile";
        var setEnableToExternalKnownlUser = CreateOofMessage(externalKnownBodyType, externalKnownEnabled, externalKnownReplyMessage);
        setEnableToExternalKnownlUser.AppliesToExternalKnown = string.Empty;

        // Set external unknown enabled
        var externalUnknownBodyType = "TEXT";
        var externalUnknownEnabled = "1";
        var externalUnknownReplyMessage = "Call my mobile";
        ...
        if (Common.IsRequirementEnabled(5194, Site))
        {
            // Add the debug information
    ...
    public void MSASCMD_S16_TC11_Settings_Status5()
    {
        ...
        var bodyType = "InvalidValue";
        var enabled = "1";
        var replyMessage = "Oof";
        var setEnableToExternalUser = CreateOofMessage(bodyType, enabled, replyMessage);
        setEnableToExternalUser.AppliesToExternalKnown = string.Empty;
    ...
    public void MSASCMD_S16_TC12_Settings_Status6()
    {
        ...
        var externalKnownBodyType = "TEXT";
        var externalKnownEnabled = "1";
        var externalKnownReplyMessage = "Message to external known";
        var setEnableToExternalKnownlUser = CreateOofMessage(externalKnownBodyType, externalKnownEnabled, externalKnownReplyMessage);
        setEnableToExternalKnownlUser.AppliesToExternalKnown = string.Empty;
        ...
        var externalUnknownBodyType = "TEXT";
        var externalUnknownEnabled = "1";
        var externalUnknownReplyMessage = "Message to unknown external users";
        var setEnableToExternalUnknownlUser = CreateOofMessage(externalUnknownBodyType, externalUnknownEnabled, externalUnknownReplyMessage);
        setEnableToExternalUnknownlUser.AppliesToExternalUnknown = string.Empty;
    ...
    public void MSASCMD_S16_TC13_Settings_ResponseInOrder()
    {
        ...
        var oofElementRequestPosition = settingsRequest.GetRequestDataSerializedXML().IndexOf("<Oof>", StringComparison.OrdinalIgnoreCase);
        var userInfomationRequestPosition = settingsRequest.GetRequestDataSerializedXML().IndexOf("<UserInformation>", StringComparison.OrdinalIgnoreCase);
        ...
        var oofElementResponsePosition = settingsResponse.ResponseDataXML.IndexOf("<Oof>", StringComparison.OrdinalIgnoreCase);
        var userInformationResponsePosition = settingsResponse.ResponseDataXML.IndexOf("<UserInformation>", StringComparison.OrdinalIgnoreCase);
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The RightsManagementInformation element is not supported when the MS-ASProtocolVersion header is set to 12.1. MS-ASProtocolVersion header value is determined using Common PTFConfig property named ActiveSyncProtocolVersion.");
        Site.Assume.AreNotEqual<string>("14.0", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The RightsManagementInformation element is not supported when the MS-ASProtocolVersion header is set to 14.0. MS-ASProtocolVersion header value is determined using Common PTFConfig property named ActiveSyncProtocolVersion.");
    ...
    public void MSASCMD_S16_TC15_Settings_SetDeviceInformationPhoneNumber()
    {
        ...
        Site.Assume.AreNotEqual<string>("14.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "This test case is not supported when the MS-ASProtocolVersion header is set to 14.1. MS-ASProtocolVersion header value is determined using Common PTFConfig property named ActiveSyncProtocolVersion.");
        Site.Assume.IsTrue(Common.IsRequirementEnabled(5201, Site), "Only on the initial release version of Exchange 2010, the PhoneNumber element is required to have a value, when the EnableOutboundSMS element is set to 1.");

        #region Set EnableOutboundSMS value to "1" and set PhoneNumber value
        var deviceInformation = GenerateDeviceInformation();
        deviceInformation.Set.EnableOutboundSMS = "1";
        deviceInformation.Set.PhoneNumber = "88888888888";

        var settingsRequest = CreateDefaultDeviceInformationRequest();
        ...
        var emailSubject = "Send and receive mobile text messages on your computer!";
        var counter = 0;
        var waitTime = int.Parse(Common.GetConfigurationPropertyValue("WaitTime", Site));
        var retryCount = int.Parse(Common.GetConfigurationPropertyValue("RetryCount", Site));
        ...
        RecordCaseRelativeItems(User1Information, User1Information.InboxCollectionId, emailSubject);
        #endregion

        #region Restore the default settings on SUT.
        deviceInformation.Set.EnableOutboundSMS = "0";
        settingsRequest.RequestData.DeviceInformation = deviceInformation;
        settingsResponse = CMDAdapter.Settings(settingsRequest);
