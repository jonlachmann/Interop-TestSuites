...
public class S19_Sync : TestSuiteBase
{
    ...
    {
        var syncResponse = Sync(CreateEmptySyncRequest(User1Information.ContactsCollectionId));
        ...
        var isVerifyR648 = CheckElementOfItemsChoiceType10(syncResponse, Response.ItemsChoiceType10.SyncKey);
        ...
        var contactFileAS = Common.GenerateResourceName(Site, "FileAS");
        var addData = CreateAddContactCommand("FirstName", "MiddleName", "LastName", contactFileAS, null);
        var originalClientId = addData.ClientId;
        var syncRequest = CreateSyncAddRequest(LastSyncKey, User1Information.ContactsCollectionId, addData);
        syncResponse = Sync(syncRequest);

        // The value of status is returned by Sync.
        var syncCollectionResponse = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Responses) as Response.SyncCollectionsCollectionResponses;
        ...
        RecordCaseRelativeItems(User1Information, User1Information.ContactsCollectionId, contactFileAS);

        // Add the debug information
        Site.Log.Add(LogEntryKind.Debug, "Verify MS-ASCMD_R957");

        // Verify MS-ASCMD requirement: MS-ASCMD_R957
        var isVerifyR957 = !string.IsNullOrEmpty(syncCollectionResponse.Add[0].ClientId) && (syncCollectionResponse.Add[0].ClientId == originalClientId) && !string.IsNullOrEmpty(syncCollectionResponse.Add[0].ServerId);
        Site.CaptureRequirementIfIsTrue(
            isVerifyR957,
            957,
        ...
        var statusValue = Convert.ToUInt32(GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Status));
        ...
        var isVerifyR748 = !string.IsNullOrEmpty(syncCollectionResponse.Add[0].ClientId) && !string.IsNullOrEmpty(syncCollectionResponse.Add[0].ServerId);
        Site.CaptureRequirementIfIsTrue(
            isVerifyR748,
            748,
        ...
        var syncCollectionCommands = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Commands) as Response.SyncCollectionsCollectionCommands;
    ...
    public void MSASCMD_S19_TC02_Sync_MoreAvailable()
    {
        ...
        var folderCreateRequest = Common.CreateFolderCreateRequest(LastFolderSyncKey, (byte)FolderType.UserCreatedContacts, Common.GenerateResourceName(Site, "FolderCreate"), User1Information.ContactsCollectionId);
        ...
        RecordCaseRelativeFolders(User1Information, folderId);

        // Synchronize the collection hierarchy and changes in a collection between the client and the server.
        Sync(CreateEmptySyncRequest(folderId));

        #region Add two contact items
        var firstContactFileAs = Common.GenerateResourceName(Site, "FileAS", 1);
        var addData = CreateAddContactCommand("FirstName", "MiddleName", "LastName", firstContactFileAs, null);
        var syncRequest = CreateSyncAddRequest(LastSyncKey, folderId, addData);
        var syncResponse = Sync(syncRequest);
        Site.Assert.AreEqual<uint>(1, Convert.ToUInt32(GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Status)), "The first contact item should be added successfully.");

        FolderSync();

        var secondContactFileAs = Common.GenerateResourceName(Site, "FileAS", 2);
        addData = CreateAddContactCommand("FirstName", "MiddleName", "LastName", secondContactFileAs, null);
        syncRequest = CreateSyncAddRequest(LastSyncKey, folderId, addData);
        syncResponse = Sync(syncRequest);
        Site.Assert.AreEqual<uint>(1, Convert.ToUInt32(GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Status)), "The second contact item should be added successfully.");
        #endregion

        #region Synchronize the changes in the new created folder.
        syncRequest = CreateEmptySyncRequest(folderId);
        Sync(syncRequest);
        ...
        var isChecked = CheckElementOfItemsChoiceType10(syncResponse, Response.ItemsChoiceType10.MoreAvailable);
        ...
        var firstContactRetrieved = false;
        ...
        var secondContactRetrieved = false;
        commands = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Commands) as Response.SyncCollectionsCollectionCommands;
        if (commands != null)
        {
            var applicationData = commands.Add[0].ApplicationData;
            for (var i = 0; i < applicationData.ItemsElementName.Length; i++)
            {
                if (applicationData.ItemsElementName[i] == Response.ItemsChoiceType8.FileAs &&
        ...
        var isVerifyR5787 = isChecked && firstContactRetrieved && secondContactRetrieved;
        Site.CaptureRequirementIfIsTrue(
            isVerifyR5787,
            5787,
    ...
    public void MSASCMD_S19_TC03_Sync_Email_Sequence()
    {
        ...
        var olderSyncKey = LastSyncKey;
        var syncResponse = CheckEmail(User2Information.InboxCollectionId, emailSubject, null);
        ...
        var isVerifyR5032 = !string.IsNullOrEmpty(syncKey);
        Site.CaptureRequirementIfIsTrue(
            isVerifyR5032,
            5032,
        ...
        var syncCollCommands = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Commands) as Response.SyncCollectionsCollectionCommands;

        // Add the debug information
        Site.Log.Add(LogEntryKind.Debug, "Verify MS-ASCMD_R5060");

        // Verify MS-ASCMD requirement: MS-ASCMD_R5060
        var isVerified5060 = syncCollCommands.Add.Length > 0;
        Site.CaptureRequirementIfIsTrue(
            isVerified5060,
            5060,
    ...
    public void MSASCMD_S19_TC04_Sync_Status3()
    {
        ...
        var invalidSyncKey = Guid.NewGuid().ToString();
        syncRequest.RequestData.Collections[0].SyncKey = invalidSyncKey;
        var syncResponse = Sync(syncRequest);
        var statusCode = Convert.ToUInt32(GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Status));
    ...
    public void MSASCMD_S19_TC05_Sync_Status4()
    {
        ...
        var contactFileAS = Common.GenerateResourceName(Site, "FileAS");
        var addData = CreateAddContactCommand("FirstName", "MiddleName", "LastName", contactFileAS, null);
        syncRequest = CreateSyncAddRequest(LastSyncKey, User1Information.RecipientInformationCacheCollectionId, addData);
        syncResponse = Sync(syncRequest);
    ...
    {
        var syncResponse = Sync(CreateEmptySyncRequest(User1Information.InboxCollectionId));
        Site.Assert.AreEqual<string>("1", GetStatusCode(syncResponse.ResponseDataXML), "The Status value of the Sync command should be 1, when the RequestData element in the Sync command request is not null.");

        // Synchronize the changes with a request, of which the request data are null.
        var syncRequest = new SyncRequest { RequestData = null };
        syncResponse = Sync(syncRequest, false);
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The HeartbeatInterval tag is not supported when the MS-ASProtocolVersion header is set to 12.1. MS-ASProtocolVersion header value is determined using Common PTFConfig property named ActiveSyncProtocolVersion.");
        ...
        var isVerify4460 = int.Parse(syncResponse.ResponseData.Status) == 14 && syncResponse.ResponseData.Item.ToString() == "3540" && syncResponse.ResponseDataXML.ToString().Contains("Limit");
        Site.CaptureRequirementIfIsTrue(
            isVerify4460,
            4460,
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.InboxCollectionId);
        syncRequest.RequestData.HeartbeatInterval = "59";
        syncResponse = Sync(syncRequest);
        var isValidResponse = int.Parse(syncResponse.ResponseData.Status) == 14 && syncResponse.ResponseData.Item.ToString() == "60" && syncResponse.ResponseDataXML.ToString().Contains("Limit");
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The ConversationMode element is not supported when the MS-ASProtocolVersion header is set to 12.1. MS-ASProtocolVersion header value is determined using Common PTFConfig property named ActiveSyncProtocolVersion.");

        var syncRequest = CreateEmptySyncRequest(User1Information.ContactsCollectionId);
        Sync(syncRequest);
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The Class element is not supported as a child element of the Options element when the MS-ASProtocolVersion header is set to 12.1. MS-ASProtocolVersion header value is determined using Common PTFConfig property named ActiveSyncProtocolVersion.");

        Sync(CreateEmptySyncRequest(User1Information.CalendarCollectionId));

        // Add a calendar item
        var calendarSubject = Common.GenerateResourceName(Site, "canlendarSubject");
        var calendarTo = Common.GetMailAddress(User1Information.UserName, User1Information.UserDomain);
        var location = Common.GenerateResourceName(Site, "Room11");
        var addData = CreateAddCalendarCommand(calendarTo, calendarSubject, location, string.Empty);
    ...
    public void MSASCMD_S19_TC10_Sync_NoChildElementsForCommands()
    {
        ...
        if (Common.IsRequirementEnabled(2058, Site))
        {
            // Add the debug information
            ...
            Site.CaptureRequirementIfAreEqual<uint>(
                1,
                Convert.ToUInt32(GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Status)),
                2058,
    ...
    public void MSASCMD_S19_TC11_Sync_Email_FilterType()
    {
        ...
        var syncRequest = CreateEmptySyncRequest(User1Information.InboxCollectionId, (byte)0);
        var syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.InboxCollectionId, (byte)1);
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.InboxCollectionId, (byte)2);
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.InboxCollectionId, (byte)3);
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.InboxCollectionId, (byte)4);
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.InboxCollectionId, (byte)5);
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.InboxCollectionId, (byte)6);
        syncResponse = Sync(syncRequest);
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.InboxCollectionId, (byte)7);
        syncResponse = Sync(syncRequest);
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.InboxCollectionId, (byte)8);
        syncResponse = Sync(syncRequest);
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The Class element is not supported in a Sync command response when the MS-ASProtocolVersion header is set to 12.1. MS-ASProtocolVersion header value is determined using Common PTFConfig property named ActiveSyncProtocolVersion.");
        ...
        var calendarSubject = Common.GenerateResourceName(Site, "calendarSubject");
        var startTime = DateTime.Now.AddDays(1.0);
        var endTime = startTime.AddMinutes(10.0);
        ...
        var syncRequest = CreateSyncAddRequest(LastSyncKey, User1Information.CalendarCollectionId, calendarData);
        var syncResponse = Sync(syncRequest);

        var responses = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Responses) as Response.SyncCollectionsCollectionResponses;
        Site.Assert.AreEqual<int>(1, int.Parse(responses.Add[0].Status), "The calendar should be added successfully.");
        RecordCaseRelativeItems(User1Information, User1Information.CalendarCollectionId, calendarSubject);
        #endregion

        #region Call Sync command to verify server supports to filter calendar when FilterType set to 0.
        syncRequest = CreateEmptySyncRequest(User1Information.CalendarCollectionId, 0);
        Sync(syncRequest);
        ...
        var commands = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Commands) as Response.SyncCollectionsCollectionCommands;
        var itemsWithFilter = commands.Add.Length;

        syncResponse = SyncChanges(User1Information.CalendarCollectionId);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");

        commands = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Commands) as Response.SyncCollectionsCollectionCommands;
        var itemsWithoutFilter = commands.Add.Length;
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.CalendarCollectionId, 1);
        Sync(syncRequest);
        syncRequest.RequestData.Collections[0].SyncKey = LastSyncKey;
        syncResponse = Sync(syncRequest);
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.CalendarCollectionId, 2);
        Sync(syncRequest);
        syncRequest.RequestData.Collections[0].SyncKey = LastSyncKey;
        syncResponse = Sync(syncRequest);
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.CalendarCollectionId, 3);
        Sync(syncRequest);
        syncRequest.RequestData.Collections[0].SyncKey = LastSyncKey;
        syncResponse = Sync(syncRequest);
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.CalendarCollectionId, 4);
        Sync(syncRequest);
        syncRequest.RequestData.Collections[0].SyncKey = LastSyncKey;
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        ...
        calendarSubject = Common.GenerateResourceName(Site, "canlendarSubject");
        ...
        syncRequest = CreateSyncAddRequest(LastSyncKey, User1Information.CalendarCollectionId, calendarData);
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        responses = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Responses) as Response.SyncCollectionsCollectionResponses;
        Site.Assert.AreEqual<int>(1, int.Parse(responses.Add[0].Status), "The calendar should be added successfully.");
        RecordCaseRelativeItems(User1Information, User1Information.CalendarCollectionId, calendarSubject);
        ...
        var isVerifyR3065 = !string.IsNullOrEmpty(FindServerId(syncResponse, "Subject", calendarSubject));
        ...
        isVerifyR3065 = isVerifyR3065 && !string.IsNullOrEmpty(FindServerId(syncResponse, "Subject", calendarSubject));
        ...
        var recurrenceCalendarSubject = Common.GenerateResourceName(Site, "recurrenceCanlendarSubject");

        var recurrenceCalendarData = new Request.SyncCollectionAdd
        {
            ClientId = ClientId,
            ApplicationData =
                new Request.SyncCollectionAddApplicationData
        ...
        syncRequest = CreateSyncAddRequest(LastSyncKey, User1Information.CalendarCollectionId, recurrenceCalendarData);
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        responses = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Responses) as Response.SyncCollectionsCollectionResponses;
        Site.Assert.AreEqual<int>(1, int.Parse(responses.Add[0].Status), "The calendar should be added successfully.");
        RecordCaseRelativeItems(User1Information, User1Information.CalendarCollectionId, recurrenceCalendarSubject);

        syncResponse = SyncChanges(User1Information.CalendarCollectionId);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");

        var isVerifyR5878 = !string.IsNullOrEmpty(FindServerId(syncResponse, "Subject", recurrenceCalendarSubject));
        ...
        isVerifyR5878 = isVerifyR5878 && !string.IsNullOrEmpty(FindServerId(syncResponse, "Subject", recurrenceCalendarSubject));
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.CalendarCollectionId, 5);
        Sync(syncRequest);
        syncRequest.RequestData.Collections[0].SyncKey = LastSyncKey;
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.CalendarCollectionId, 6);
        Sync(syncRequest);
        syncRequest.RequestData.Collections[0].SyncKey = LastSyncKey;
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.CalendarCollectionId, 7);
        Sync(syncRequest);
        syncRequest.RequestData.Collections[0].SyncKey = LastSyncKey;
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        ...
        commands = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Commands) as Response.SyncCollectionsCollectionCommands;
        var original = commands.Add.Length;
        ...
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        commands = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Commands) as Response.SyncCollectionsCollectionCommands;
        var current = commands.Add.Length;
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.CalendarCollectionId, 8);
        Sync(syncRequest);
        syncRequest.RequestData.Collections[0].SyncKey = LastSyncKey;
        syncResponse = Sync(syncRequest);
    ...
    public void MSASCMD_S19_TC13_Sync_Tasks_FilterType()
    {
        ...
        var syncRequest = CreateEmptySyncRequest(User1Information.TasksCollectionId, 0);
        var syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.TasksCollectionId, 1);
        syncResponse = Sync(syncRequest);
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.TasksCollectionId, 2);
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData, "The ResponseData should not be null.");
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.TasksCollectionId, 3);
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData, "The ResponseData should not be null.");
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.TasksCollectionId, 4);
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData, "The ResponseData should not be null.");
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.TasksCollectionId, 5);
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData, "The ResponseData should not be null.");
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.TasksCollectionId, 6);
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData, "The ResponseData should not be null.");
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.TasksCollectionId, 7);
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData, "The ResponseData should not be null.");
        ...
        syncRequest = CreateEmptySyncRequest(User1Information.TasksCollectionId, 8);
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData, "The ResponseData should not be null.");
    ...
    public void MSASCMD_S19_TC14_Sync_MoreThanOneFilterTypes()
    {
        ...
        CheckEmail(User2Information.InboxCollectionId, emailSubject, null);
        RecordCaseRelativeItems(User2Information, User2Information.InboxCollectionId, emailSubject);
        ...
        Site.Assert.IsNotNull(syncResponse.ResponseData, "The ResponseData should not be null.");
        Site.Assert.IsNotNull(FindServerId(syncResponse, "Subject", emailSubject), "The email should be in the Inbox folder of User2.");

        if (Common.IsRequirementEnabled(3075, Site))
        {
            // Add the debug information
    ...
    public void MSASCMD_S19_TC15_Sync_IncludeFilterTypeOrNot()
    {
        ...
        var syncResponseWithoutFilterType = Sync(CreateEmptySyncRequest(User1Information.ContactsCollectionId));
        Site.Assert.AreEqual<uint>(1, Convert.ToUInt32(GetCollectionItem(syncResponseWithoutFilterType, Response.ItemsChoiceType10.Status)), "The Status code of the Sync response without FilterType should be 1.");

        // Synchronize changes with a FilterType value.
        var syncResponseWithFilterType = Sync(CreateEmptySyncRequest(User1Information.ContactsCollectionId, (byte)0));
        Site.Assert.AreEqual<uint>(1, Convert.ToUInt32(GetCollectionItem(syncResponseWithFilterType, Response.ItemsChoiceType10.Status)), "The Status code of the Sync response with FilterType should be 1.");
    ...
    public void MSASCMD_S19_TC16_Sync_WindowSize_MoreAvailable_Omitted()
    {
        ...
        // Call method FolderCreate to create a new folder as a child folder of the specified parent folder.
        var folderCreateRequest = Common.CreateFolderCreateRequest(folderSyncResponse.ResponseData.SyncKey, (byte)FolderType.UserCreatedContacts, Common.GenerateResourceName(Site, "FolderCreate"), User1Information.ContactsCollectionId);
        var folderCreateResponse = CMDAdapter.FolderCreate(folderCreateRequest);
        ...
        FolderSync();
        syncResponse = SyncChanges(folderId);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");

        RecordCaseRelativeItems(User1Information, folderId, contactFileAS);

        // Check whether the MoreAvailable element appears in response.
        var isVerifyR3456 = CheckElementOfItemsChoiceType10(syncResponse, Response.ItemsChoiceType10.MoreAvailable);
    ...
    public void MSASCMD_S19_TC17_Sync_WindowSize_MoreAvailable()
    {
        ...
        var contactFileAS = Common.GenerateResourceName(Site, "FileAS", 1);
        var addData = CreateAddContactCommand("FirstName", "MiddleName", "LastName", contactFileAS, null);
        ...
        for (var i = 1; i < 101; i++)
        {
            contactFileAS = Common.GenerateResourceName(Site, "FileAS", (uint)(i + 1));
            addData = CreateAddContactCommand("FirstName", "MiddleName", "LastName", contactFileAS, null);
        ...
        #region Synchronize the changes in the Contacts folder.
        var syncResponse = SyncChanges(folderId, false);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The Collections element should exist in the Sync response.");

        var collectionCommands = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Commands) as Response.SyncCollectionsCollectionCommands;

        var isVerifyR3459 = CheckElementOfItemsChoiceType10(syncResponse, Response.ItemsChoiceType10.MoreAvailable) && collectionCommands.Add.Length == 100;
    ...
    public void MSASCMD_S19_TC18_Sync_SMIME()
    {
        ...
        while ((counter < retryCount) && string.IsNullOrEmpty(serverId))
        {
            ...
            syncResponse = Sync(syncRequest);
            serverId = FindServerId(syncResponse, "Subject", emailSubject);
            counter++;
        }

        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");

        var syncCollectionsCollectionCommands = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Commands) as Response.SyncCollectionsCollectionCommands;
        var isR3408Satisfied = false;
        foreach (var element in syncCollectionsCollectionCommands.Add[0].ApplicationData.ItemsElementName)
        {
    ...
    public void MSASCMD_S19_TC19_Sync_NoResponsesElement()
    {
        ...
        // Call method FolderCreate to create a new folder as a child folder of the specified parent folder.
        var folderCreateRequest = Common.CreateFolderCreateRequest(folderSyncResponse.ResponseData.SyncKey, (byte)FolderType.UserCreatedMail, Common.GenerateResourceName(Site, "FolderCreate"), User1Information.InboxCollectionId);
        var folderCreateResponse = CMDAdapter.FolderCreate(folderCreateRequest);
        Site.Assert.AreEqual<int>(1, int.Parse(folderCreateResponse.ResponseData.Status), "The server should return a status code 1 in the FolderCreate command response to indicate success.");
        ...
        var request = CreateEmptySyncRequest(folderId);
        Sync(request);
        request.RequestData.Collections[0].SyncKey = LastSyncKey;
        var syncResponse = Sync(request);
        var collectionResponse = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Responses) as Response.SyncCollectionsCollectionResponses;
    ...
    public void MSASCMD_S19_TC20_Sync_MoreThanOneClasses()
    {
        ...
        Site.Assert.IsNotNull(syncResponse.ResponseData, "The ResponseData should not be null.");
        Site.Assert.IsTrue(!string.IsNullOrEmpty(FindServerId(syncResponse, "Subject", emailSubject)), "The email should be in the Inbox folder of User2.");

        if (Common.IsRequirementEnabled(938, Site))
        {
            // Add the debug information
    ...
    public void MSASCMD_S19_TC21_Sync_MoreThanOneMaxItems()
    {
        ...
        if (Common.IsRequirementEnabled(3280, Site))
        {
            // Add the debug information
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The Options element is not supported when the MS-ASProtocolVersion header is set to 12.1. MS-ASProtocolVersion header value is determined using Common PTFConfig property named ActiveSyncProtocolVersion.");
        ...
        if (Common.IsRequirementEnabled(3404, Site))
        {
            // Add the debug information
    ...
    public void MSASCMD_S19_TC23_Sync_MoreThanOneMIMETruncation()
    {
        ...
        if (Common.IsRequirementEnabled(3432, Site))
        {
            // Add the debug information
    ...
    public void MSASCMD_S19_TC24_Sync_Status6_EndTimeMissing()
    {
        ...
        var calendarSubject = Common.GenerateResourceName(Site, "canlendarSubject");
        var calendarTo = Common.GetMailAddress(User2Information.UserName, User2Information.UserDomain);
        var location = Common.GenerateResourceName(Site, "Room11");
        var addData = CreateAddCalendarCommand(calendarTo, calendarSubject, location, string.Empty);
    ...
    public void MSASCMD_S19_TC25_Sync_Status6_AddEmail()
    {
        ...
        var addData = CreateAddEmailCommand(Common.GetMailAddress(User1Information.UserName, User1Information.UserDomain), ClientId);
        var syncRequest = CreateSyncAddRequest(LastSyncKey, User1Information.InboxCollectionId, addData);
        var syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        var collectionResponse = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Responses) as Response.SyncCollectionsCollectionResponses;
    ...
    public void MSASCMD_S19_TC26_Sync_GetChanges()
    {
        ...
        var request = syncRequest.GetRequestDataSerializedXML().Replace("<GetChanges>1</GetChanges>", "<GetChanges></GetChanges>");
        var response = CMDAdapter.SendStringRequest(CommandName.Sync, null, request);
        ...
        Sync(CreateEmptySyncRequest(User1Information.ContactsCollectionId));

        var contactFileAS = Common.GenerateResourceName(Site, "FileAS");
        var addData = CreateAddContactCommand("FirstName", "MiddleName", "LastName", contactFileAS, null);

        syncRequest = CreateSyncAddRequest(LastSyncKey, User1Information.ContactsCollectionId, addData);
        syncResponse = Sync(syncRequest, false);
        Site.Assert.AreEqual<uint>(1, Convert.ToUInt32(GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Status)), "The new contact should be added successfully.");
        RecordCaseRelativeItems(User1Information, User1Information.ContactsCollectionId, contactFileAS);
        #endregion
        ...
        FolderSync();
        syncRequest = CreateEmptySyncRequest(User1Information.ContactsCollectionId);
        var syncResponseWithSyncKeyIs0 = Sync(syncRequest, false);
        ...
        var syncResponseWithSyncKetIsNot0 = Sync(syncRequest, false);
        ...
        Site.CaptureRequirementIfIsNotNull(
            GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Commands),
            3126,
        ...
        Site.CaptureRequirementIfIsNotNull(
            GetCollectionItem(syncResponseWithSyncKetIsNot0, Response.ItemsChoiceType10.Commands),
            5556,
        ...
        Site.CaptureRequirementIfIsNull(
            GetCollectionItem(syncResponseWithSyncKeyIs0, Response.ItemsChoiceType10.Commands),
            5555,
    ...
    {
        Site.Assume.AreEqual<string>("Base64", Common.GetConfigurationPropertyValue("HeaderEncodingType", Site), "The device ID should be same across all requests, when the HeaderEncodingType is PlainText.");
        ...
        var syncRequest = CreateSyncAddRequest(LastSyncKey, User1Information.ContactsCollectionId, addData);
        var syncResponse = Sync(syncRequest, false);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        var collectionResponse = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Responses) as Response.SyncCollectionsCollectionResponses;
        ...
        GetInitialSyncResponse(User1Information.ContactsCollectionId);
        syncRequest.RequestData.Collections[0].SyncKey = LastSyncKey;
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        var serverId = FindServerId(syncResponse, "FileAs", contactFileAS);
        ...
        syncRequest.RequestData.Collections[0].GetChangesSpecified = true;
        syncResponse = Sync(syncRequest);
        Site.Assert.AreEqual<uint>(1, Convert.ToUInt32(GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Status)), "The Sync command should be conducted successfully.");
        RecordCaseRelativeItems(User1Information, User1Information.ContactsCollectionId, updatedContactFileAS);
        ...
        var isVerifyR879 = false;
        foreach (var itemElementName in commands.Change[0].ApplicationData.ItemsElementName)
        {
        ...
        SwitchUser(User2Information);
        serverId = FindServerId(CheckEmail(User2Information.InboxCollectionId, emailSubject, null), "Subject", emailSubject);
        RecordCaseRelativeItems(User2Information, User2Information.InboxCollectionId, emailSubject);
        #endregion

        #region Call Sync command to set a flag to an email.
        var updatedEmailSubject = Common.GenerateResourceName(Site, "updatedSubject");

        var startDate = DateTime.Now.AddDays(5.0);
        var dueDate = startDate.AddHours(1.0);

        // Define email flag
        var emailFlag = new Request.Flag
        {
            StartDate = startDate,
        ...
        syncRequest = CreateSyncChangeRequest(LastSyncKey, User2Information.InboxCollectionId, appDataChange);
        syncResponse = Sync(syncRequest);
        ...
        var updatedEmailFlag = new Request.Flag
        {
            StartDate = startDate,
            StartDateSpecified = true,
            DueDate = dueDate,
        ...
        syncRequest = CreateSyncChangeRequest(LastSyncKey, User2Information.InboxCollectionId, appDataChange);
        syncResponse = Sync(syncRequest);
        Site.Assert.AreEqual<uint>(1, Convert.ToUInt32(GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Status)), "The email should be updated successfully.");
        ...
        var isVerifyR873 = !string.IsNullOrEmpty(FindServerId(syncResponse, "Subject", emailSubject));
        Site.CaptureRequirementIfIsTrue(
            isVerifyR873,
            873,
        ...
        emailSubject = Common.GenerateResourceName(Site, "subject");
        SendPlainTextEmail(null, emailSubject, User1Information.UserName, User2Information.UserName, null);
        ...
        var isVerifyR5825 = !string.IsNullOrEmpty(FindServerId(syncResponse, "Subject", emailSubject));
        Site.CaptureRequirementIfIsTrue(
            isVerifyR5825,
            5825,
            @"[In Change] Certain in-schema properties remain untouched in the following three cases: If there is only an [email:Flag ([MS-ASEMAIL] section 2.2.2.27),] email:Read ([MS-ASEMAIL] section 2.2.2.47) [, or email:Categories ([MS-ASEMAIL] section 2.2.2.9)] change (that is, if only an email:Flag, email:Categories or email:Read element is present), all other properties will remain unchanged.");
        ...
        var isVerifyR5826 = !string.IsNullOrEmpty(FindServerId(syncResponse, "Subject", emailSubject));
        Site.CaptureRequirementIfIsTrue(
            isVerifyR5826,
            5826,
            @"[In Change] Certain in-schema properties remain untouched in the following three cases: If there is only [an email:Flag ([MS-ASEMAIL] section 2.2.2.27), email:Read ([MS-ASEMAIL] section 2.2.2.47), or] email:Categories ([MS-ASEMAIL] section 2.2.2.9) change (that is, if only an email:Flag, email:Categories or email:Read element is present), all other properties will remain unchanged.");
        ...
        collectionResponse = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Responses) as Response.SyncCollectionsCollectionResponses;
        Site.Assert.IsNotNull(collectionResponse, "The responses element should exist in the Sync response.");
        ...
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        serverId = FindServerId(syncResponse, "FileAs", contactFileAs);
        Site.Assert.IsNotNull(serverId, "The added contact should be synchronized down to the current device.");

        var responseApplicationData = GetAddApplicationData(syncResponse, Response.ItemsChoiceType8.FileAs, contactFileAS);
        ...
        for (var i = 0; i < responseApplicationData.ItemsElementName.Length; i++)
        {
        ...
        var originalPicture = (string)GetElementValueFromSyncResponse(syncResponse, serverId, Response.ItemsChoiceType8.Picture);
        ...
        var updatedContactFileAs = Common.GenerateResourceName(Site, "updatedContactFileAs");
        ...
        syncRequest = CreateSyncChangeRequest(LastSyncKey, User1Information.ContactsCollectionId, appDataChange);
        syncResponse = Sync(syncRequest);
        Site.Assert.AreEqual<uint>(1, Convert.ToUInt32(GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Status)), "The FileAs of the contact should be updated successfully.");
        RemoveRecordCaseRelativeItems(User1Information, User1Information.ContactsCollectionId, contactFileAs);
        RecordCaseRelativeItems(User1Information, User1Information.ContactsCollectionId, updatedContactFileAs);

        syncResponse = SyncChanges(User1Information.ContactsCollectionId);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");

        responseApplicationData = GetAddApplicationData(syncResponse, Response.ItemsChoiceType8.FileAs, updatedContactFileAs);
        ...
        var currentPicture = (string)GetElementValueFromSyncResponse(syncResponse, serverId, Response.ItemsChoiceType8.Picture);
        ...
        var isVerifyR878 = body.Type == currentBody.Type && body.EstimatedDataSize == currentBody.EstimatedDataSize && originalPicture == currentPicture;
        Site.CaptureRequirementIfIsTrue(
            isVerifyR878,
            878,
    ...
    public void MSASCMD_S19_TC33_Sync_Limit_HeartbeatIntervalIsLargerThan3540()
    {
        ...
        // When the Status value is 14, the value of the Item element represents the value of the Limit element.
        var isVerifyR3160 = int.Parse(syncResponse.ResponseData.Status) == 14 && !string.IsNullOrEmpty((string)syncResponse.ResponseData.Item) && (string)syncResponse.ResponseData.Item == "3540";
        Site.CaptureRequirementIfIsTrue(
            isVerifyR3160,
            3160,
    ...
    {
        #region Send a MIME-formatted email to User2.
        var emailSubject1 = Common.GenerateResourceName(Site, "subject");
        SendPlainTextEmail(null, emailSubject1, User1Information.UserName, User2Information.UserName, null);
        #endregion

        SwitchUser(User2Information);
        RecordCaseRelativeItems(User2Information, User2Information.InboxCollectionId, emailSubject1);

        // Check DeletedItems folder
        var syncRequest = CreateEmptySyncRequest(User2Information.DeletedItemsCollectionId);
        Sync(syncRequest);
        syncRequest.RequestData.Collections[0].SyncKey = LastSyncKey;
        var syncResponse = Sync(syncRequest);
        Site.Assert.IsNull(FindServerId(syncResponse, "Subject", emailSubject1), "The email should not be found in the DeletedItems folder.");

        // Check Inbox folder
        syncResponse = CheckEmail(User2Information.InboxCollectionId, emailSubject1, null);
        var serverId = FindServerId(syncResponse, "Subject", emailSubject1);
        ...
        Site.Assert.AreEqual<uint>(1, Convert.ToUInt32(GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Status)), "The Sync delete operation should be successful.");
        #endregion

        #region Verify if the email has been deleted from the Inbox folder and placed into the DeletedItems folder.
        // Check Inbox folder.
        syncResponse = SyncChanges(User2Information.InboxCollectionId, false);
        Site.Assert.IsNull(FindServerId(syncResponse, "Subject", emailSubject1), "The email deleted should not be found in the Inbox folder.");
        RemoveRecordCaseRelativeItems(User2Information, User2Information.InboxCollectionId, emailSubject1);

        // Check DeletedItems folder
        CheckEmail(User2Information.DeletedItemsCollectionId, emailSubject1, null);
        RecordCaseRelativeItems(User2Information, User2Information.DeletedItemsCollectionId, emailSubject1);
        ...
        #region Send a MIME-formatted email to User2.
        SwitchUser(User1Information);
        var emailSubject2 = Common.GenerateResourceName(Site, "subject");
        SendPlainTextEmail(null, emailSubject2, User1Information.UserName, User2Information.UserName, null);
        #endregion
        ...
        Site.Assert.IsNull(FindServerId(syncResponse, "Subject", emailSubject2), "The email should not be found in the DeletedItems folder.");

        // Check Inbox folder
        syncResponse = CheckEmail(User2Information.InboxCollectionId, emailSubject2, null);
        serverId = FindServerId(syncResponse, "Subject", emailSubject2);
        Site.Assert.IsNotNull(serverId, "The email should be found in the inbox folder.");
        ...
        // Check Inbox folder
        syncResponse = SyncChanges(User2Information.InboxCollectionId);
        Site.Assert.IsNull(FindServerId(syncResponse, "Subject", emailSubject2), "The deleted email should not be found in the Inbox folder.");

        // Check DeletedItems folder
        syncResponse = SyncChanges(User2Information.DeletedItemsCollectionId);
        Site.Assert.IsNotNull(FindServerId(syncResponse, "Subject", emailSubject2), "The deleted email should be found in the DeletedItems folder.");

        RemoveRecordCaseRelativeItems(User2Information, User2Information.InboxCollectionId, emailSubject1);
        RecordCaseRelativeItems(User2Information, User2Information.DeletedItemsCollectionId, emailSubject1, emailSubject2);
    ...
    public void MSASCMD_S19_TC35_Sync_DeletesAsMovesIsFalse()
    {
        ...
        var syncResponse = SyncChanges(User2Information.DeletedItemsCollectionId);
        Site.Assert.IsNull(FindServerId(syncResponse, "Subject", emailSubject), "The email should not be found in the DeletedItems folder.");

        syncResponse = GetMailItem(User2Information.InboxCollectionId, emailSubject);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        ...
        var appDataDelete = new Request.SyncCollectionDelete { ServerId = serverId };
        ...
        syncResponse = SyncChanges(User2Information.InboxCollectionId);
        Site.Assert.IsNull(FindServerId(syncResponse, "Subject", emailSubject), "The email deleted should not be found in the Inbox folder.");
        RemoveRecordCaseRelativeItems(User2Information, User2Information.InboxCollectionId, emailSubject);

        syncResponse = SyncChanges(User2Information.DeletedItemsCollectionId);
        Site.Assert.IsNull(FindServerId(syncResponse, "Subject", emailSubject), "The email deleted should not be found in the DeletedItems folder.");
    ...
    public void MSASCMD_S19_TC36_Sync_Fetch()
    {
        ...
        var appDataFetch = new Request.SyncCollectionFetch
        {
            ServerId = FindServerId(syncResponse, "Subject", emailSubject)
        };
    ...
    public void MSASCMD_S19_TC37_Sync_WindowSize_512()
    {
        ...
        var syncRequest = CreateSyncAddRequest(LastSyncKey, folderId, addData);
        Sync(syncRequest, false);
        FolderSync();

        for (var i = 2; i <= 513; i++)
        {
            contactFileAS = Common.GenerateResourceName(Site, "FileAS", (uint)i);
            addData = CreateAddContactCommand("FirstName", "MiddleName", "LastName", contactFileAS, null);
        ...
        syncRequest = CreateEmptySyncRequest(folderId);
        Sync(syncRequest, false);
        var lastSyncKey = LastSyncKey;

        syncRequest.RequestData.Collections[0].SyncKey = lastSyncKey;
        var syncResponse = Sync(syncRequest, false);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The Collections element should exist in the Sync response.");

        var isVerifyR3460WithoutWindowSize = CheckElementOfItemsChoiceType10(syncResponse, Response.ItemsChoiceType10.MoreAvailable);
        ...
        var isVerifyR3460WithWindowSize = CheckElementOfItemsChoiceType10(syncResponse, Response.ItemsChoiceType10.MoreAvailable);
        ...
        var isVerifiedR4764 = itemCount < 512;
    ...
    public void MSASCMD_S19_TC38_Sync_Delete()
    {
        ...
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        Site.Assert.IsTrue(!string.IsNullOrEmpty(FindServerId(syncResponse, "FileAs", contactFileAS)), "The added contact should be synchronized down to the current device.");

        #region Delete the added contact item.
        syncRequest = CreateSyncDeleteRequest(LastSyncKey, User1Information.ContactsCollectionId, FindServerId(syncResponse, "FileAs", contactFileAS));
        ...
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        Site.Assert.IsNull(FindServerId(syncResponse, "FileAs", contactFileAS), "The deleted contact should not exist in the Contacts folder.");
        #endregion
    }
    ...
    public void MSASCMD_S19_TC39_Sync_Supported()
    {
        ...
        var contactFileAS = Common.GenerateResourceName(Site, "FileAS");
        var addData = CreateAddContactCommand("FirstName", "MiddleName", "LastName", contactFileAS, "Vice President");
        ...
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        Site.Assert.AreEqual<uint>(1, Convert.ToUInt32(GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Status)), "The Sync command should succeed.");

        syncResponse = SyncChanges(User1Information.ContactsCollectionId);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");

        var isVerifyR5906 = true;
        var applicationData1 = GetAddApplicationData(syncResponse, Response.ItemsChoiceType8.FileAs, contactFileAS);
        ...
        for (var i = 0; i < applicationData1.ItemsElementName.Length; i++)
        {
        ...
        contactFileAS = Common.GenerateResourceName(Site, "FileAS");
        addData = CreateAddContactCommand("FirstName", "MiddleName", "LastName", contactFileAS, "Vice President");
        ...
        var isVerifyR5907 = true;
        var applicationData2 = GetAddApplicationData(syncResponse, Response.ItemsChoiceType8.FileAs, updatedContactFileAs);
        ...
        for (var i = 0; i < applicationData2.ItemsElementName.Length; i++)
        {
        ...
        var isFirstNameGhosted = false;
        var isMiddleNameGhosted = false;
        var isLastNameGhosted = false;
        foreach (var name in applicationData2.ItemsElementName)
        {
        ...
        syncResponse = SyncChanges(User1Information.ContactsCollectionId);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        var applicationData3 = GetAddApplicationData(syncResponse, Response.ItemsChoiceType8.FileAs, contactFileAS);
        ...
        foreach (var itemElementName in applicationData3.ItemsElementName)
        {
    ...
    public void MSASCMD_S19_TC40_Sync_Conflict()
    {
        ...
        var syncResponse = SyncChanges(User1Information.ContactsCollectionId);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        var serverId = FindServerId(syncResponse, "FileAs", contactFileAS);
        ...
        var isReplacedByServerData = false;
        var commands = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Commands) as Response.SyncCollectionsCollectionCommands;
        Site.Assert.IsNotNull(commands.Change, "The Change element should not be null.");
        ...
        for (var i = 0; i < commands.Change[0].ApplicationData.ItemsElementName.Length; i++)
        {
        ...
        RecordCaseRelativeItems(User1Information, User1Information.ContactsCollectionId, conflictUpdatedFileAs);
        ...
        Site.CaptureRequirementIfIsNotNull(
            FindServerId(syncResponse, "FileAs", conflictUpdatedFileAs),
            2067,
    ...
    public void MSASCMD_S19_TC41_Sync_MoreThanOneConflicts()
    {
        ...
        if (Common.IsRequirementEnabled(2075, Site))
        {
            // Add the debug information
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The Class element is not supported as a child element of the Options element when the ActiveSyncProtocolVersion is 12.1.");
    ...
    public void MSASCMD_S19_TC43_Sync_MIMETruncation()
    {
        #region Send a MIME-formatted email to User2.
        var emailSubject = Common.GenerateResourceName(Site, "subject");
        var emailContent = new string('X', 102500);
        SendPlainTextEmail(null, emailSubject, User1Information.UserName, User2Information.UserName, emailContent);
        #endregion
        ...
        var dataLength = mailBody.Data.Length;
        ...
        Site.Assert.IsNotNull(mailBody, "The body of the received email should not be null.");
        var original = mailBody.Data.Length;
        ...
        var dataSize = GetEmailBodyDataSize(emailSubject, 1);
        ...
        dataSize = GetEmailBodyDataSize(emailSubject, 2);
        ...
        dataSize = GetEmailBodyDataSize(emailSubject, 3);
        ...
        dataSize = GetEmailBodyDataSize(emailSubject, 4);
        ...
        dataSize = GetEmailBodyDataSize(emailSubject, 5);
        ...
        dataSize = GetEmailBodyDataSize(emailSubject, 6);
        ...
        dataSize = GetEmailBodyDataSize(emailSubject, 7);
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "The airsync:ConversationMode element is not supported when the MS-ASProtocolVersion header is set to 12.1. MS-ASProtocolVersion header value is determined using Common PTFConfig property named ActiveSyncProtocolVersion.");
        ...
        var commandsByConversationMode = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Commands) as Response.SyncCollectionsCollectionCommands;
        ...
        var serverIdsByConversationMode = new List<string>();

        foreach (var add in commandsByConversationMode.Add)
        {
        ...
        syncRequest = CreateEmptySyncRequest(User2Information.InboxCollectionId, (byte)1);
        Sync(syncRequest);
        syncRequest.RequestData.Collections[0].SyncKey = LastSyncKey;
        syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        ...
        var isVerifyR2107 = true;
        foreach (var item in serverIdsByConversationMode)
        {
    ...
    {
        Site.Assume.IsTrue(Common.IsRequirementEnabled(5671, Site) || Common.IsRequirementEnabled(5673, Site), "Update Rollup 6 for Exchange 2010 SP2 and Exchange 2013 use the specified limit values by default.");
        ...
        for (var i = 0; i < 50; i++)
        {
            ...
            var fetchData = new Request.SyncCollectionFetch { ServerId = serverId };
            commands.Add(fetchData);
        }

        var contactFileASFor51Add = Common.GenerateResourceName(Site, "FileAS");
        var addDataFor51Add = CreateAddContactCommand("FirstName", "MiddleName", "LastName", contactFileASFor51Add, null);
    ...
    public void MSASCMD_S19_TC49_Sync_SoftDelete()
    {
        ...
        var calendarSubject = Common.GenerateResourceName(Site, "calendarSubject");
        var location = Common.GenerateResourceName(Site, "Room");
        var startTime = DateTime.Now.AddDays(-16);
        var endTime = startTime.AddHours(1.0);
        ...
        var syncRequest = CreateSyncAddRequest(LastSyncKey, User1Information.CalendarCollectionId, calendarData);
        var syncResponse = Sync(syncRequest, true);
        var responses = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Responses) as Response.SyncCollectionsCollectionResponses;
        Site.Assert.AreEqual<int>(1, int.Parse(responses.Add[0].Status), "The calendar should be added successfully.");
        RecordCaseRelativeItems(User1Information, User1Information.CalendarCollectionId, calendarSubject);
        #endregion
        ...
        calendarSubject = Common.GenerateResourceName(Site, "calendarSubject");
        location = Common.GenerateResourceName(Site, "Room");
        ...
        syncRequest = CreateSyncAddRequest(LastSyncKey, User1Information.CalendarCollectionId, calendarData);
        syncResponse = Sync(syncRequest, true);
        responses = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Responses) as Response.SyncCollectionsCollectionResponses;
        Site.Assert.AreEqual<int>(1, int.Parse(responses.Add[0].Status), "The calendar should be added successfully.");
        RecordCaseRelativeItems(User1Information, User1Information.CalendarCollectionId, calendarSubject);
        #endregion
        ...
        var filteredOut = commands.SoftDelete.Length;
        #endregion
        ...
        commands = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Commands) as Response.SyncCollectionsCollectionCommands;
        Site.Assert.IsNotNull(commands.Add, "The Add element in the commands should not be null.");
        var filterdIn = commands.Add.Length;
    ...
    public void MSASCMD_S19_TC50_Sync_Class()
    {
        ...
        Sync(CreateEmptySyncRequest(User1Information.NotesCollectionId));

        var syncRequest = CreateSyncAddRequest(LastSyncKey, User1Information.NotesCollectionId, noteData);
        var syncResponse = Sync(syncRequest, false);
    ...
    public void MSASCMD_S19_TC51_Sync_Change_Exceptions()
    {
        ...
        var recurrenceCalendarSubject = Common.GenerateResourceName(Site, "calendarSubject");
        var location = Common.GenerateResourceName(Site, "Room");
        var currentDate = DateTime.Now.AddDays(1);
        var startTime = new DateTime(currentDate.Year, currentDate.Month, currentDate.Day, 10, 0, 0);
        ...
        var syncRequest = CreateSyncAddRequest(LastSyncKey, User1Information.CalendarCollectionId, recurrenceCalendarData);
        var syncResponse = Sync(syncRequest);
        Site.Assert.IsNotNull(syncResponse.ResponseData.Item, "The items returned in the Sync command response should not be null.");
        var responses = GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Responses) as Response.SyncCollectionsCollectionResponses;
        Site.Assert.AreEqual<int>(1, int.Parse(responses.Add[0].Status), "The calendar should be added successfully.");
        ...
        syncResponse = SyncChanges(User1Information.CalendarCollectionId);
        var serverId = FindServerId(syncResponse, "Subject", recurrenceCalendarSubject);
        ...
        syncRequest = CreateSyncChangeRequest(LastSyncKey, User1Information.CalendarCollectionId, appDataChange);
        syncResponse = Sync(syncRequest);
        Site.Assert.AreEqual<uint>(1, Convert.ToUInt32(GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Status)), "The FileAs of the contact should be updated successfully.");
        RemoveRecordCaseRelativeItems(User1Information, User1Information.CalendarCollectionId, recurrenceCalendarSubject);
        RecordCaseRelativeItems(User1Information, User1Information.CalendarCollectionId, updatedCalendarSubject);

        syncResponse = SyncChanges(User1Information.CalendarCollectionId);
        serverId = FindServerId(syncResponse, "Subject", updatedCalendarSubject);
        Site.Assert.IsNotNull(serverId, "The recurrence calendar should be found.");
        ...
        foreach (var item in commands.Add)
        {
            if (item.ServerId == serverId)
            {
                for (var i = 0; i < item.ApplicationData.ItemsElementName.Length; i++)
                {
                    ...
                    {
                        var currentExceptions = item.ApplicationData.Items[i] as Response.Exceptions;
                        Site.Assert.IsNotNull(currentExceptions, "The Exceptions element should exist.");

                        var currentException = currentExceptions.Exception[0];
        ...
        syncResponse = Sync(syncRequest);
        Site.Assert.AreEqual<uint>(1, Convert.ToUInt32(GetCollectionItem(syncResponse, Response.ItemsChoiceType10.Status)), "The FileAs of the contact should be updated successfully.");
        RemoveRecordCaseRelativeItems(User1Information, User1Information.CalendarCollectionId, updatedCalendarSubject);
        RecordCaseRelativeItems(User1Information, User1Information.CalendarCollectionId, allNewCalendarSubject);

        syncResponse = SyncChanges(User1Information.CalendarCollectionId);
        serverId = FindServerId(syncResponse, "Subject", allNewCalendarSubject);
        Site.Assert.IsNotNull(serverId, "The recurrence calendar should be found.");
    ...
    public void MSASCMD_S19_TC52_Sync_Status8()
    {
        ...
        RecordCaseRelativeItems(User2Information, User2Information.InboxCollectionId, emailSubject);
        var syncResponse = CheckEmail(User2Information.InboxCollectionId, emailSubject, null);
        var itemServerId = FindServerId(syncResponse, "Subject", emailSubject);

        #region Delete the email form Inbox of User2.
        var syncRequest = CreateSyncDeleteRequest(LastSyncKey, User2Information.InboxCollectionId, itemServerId);
        syncRequest.RequestData.Collections[0].DeletesAsMoves = false;
        syncRequest.RequestData.Collections[0].DeletesAsMovesSpecified = true;
        syncResponse = Sync(syncRequest);
        #endregion
    ...
    {
        var appData = new Request.SyncCollectionChange
        {
            ServerId = serverId,
            ApplicationData = new Request.SyncCollectionChangeApplicationData
            {
    ...
    {
        if (GetCollectionItem(syncResponse, element) != null)
        {
            return true;
        }
        else
