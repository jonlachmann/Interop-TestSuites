...
public class S14_Search : TestSuiteBase
{
    ...
    public void MSASCMD_S14_TC02_Search_MultipleAndElements()
    {
        ...
        var sendStringRequest = invalidSearchRequest.GetRequestDataSerializedXML();
        var response = CMDAdapter.SendStringRequest(CommandName.Search, null, sendStringRequest);
        ...
        var counter = 1;
        var searchStatus = doc.SelectSingleNode("/e:Search/e:Status", xnm);

        while (counter < retryCount && searchStatus != null && searchStatus.InnerXml.Equals("10"))
        {
            Thread.Sleep(waitTime);
            response = CMDAdapter.SendStringRequest(CommandName.Search, null, sendStringRequest);
            doc.LoadXml(response.ResponseDataXML);
        ...
        var status = Common.GetSearchStatusCode(response);
    ...
    public void MSASCMD_S14_TC03_Search_TooComplex_Status8_Class()
    {
        ...
        var invalidSearchRequest = CreateInvalidSearchRequest("<Class xmlns=\"AirSync\">Email</Class>", "<And>");
        #endregion

        #region Call method SendStringRequest to send a plain text request.
        var response = CMDAdapter.SendStringRequest(CommandName.Search, null, invalidSearchRequest);
        ...
        {
            Thread.Sleep(waitTime);
            response = CMDAdapter.SendStringRequest(CommandName.Search, null, invalidSearchRequest);
            doc.LoadXml(response.ResponseDataXML);
            xnm = new XmlNamespaceManager(doc.NameTable);
            xnm.AddNamespace("e", "Search");
            searchStatus = doc.SelectSingleNode("/e:Search/e:Status", xnm);
    ...
    public void MSASCMD_S14_TC04_Search_TooComplex_Status8_CollectionId()
    {
        ...
        var collectionIdElement = "<CollectionId xmlns=\"AirSync\">" + User1Information.InboxCollectionId + "</CollectionId>";

        // Insert CollectionId element before And element.
        var invalidSearchRequest = CreateInvalidSearchRequest(collectionIdElement, "<And>");
        #endregion
    ...
    public void MSASCMD_S14_TC05_Search_TooComplex_Status8_FreeText()
    {
        ...
        var freeTextElement = "<FreeText>FreeText</FreeText>";

        // Insert FreeText element before And element.
        var invalidSearchRequest = CreateInvalidSearchRequest(freeTextElement, "<And>");
        #endregion
    ...
    {
        Site.Assume.IsTrue(Common.IsRequirementEnabled(2143, Site), "[In Appendix A: Product Behavior] The implementation does not return a protocol status error in response to such a command request [more than one DeepTraversal element in a Search command request]. (Exchange 2007 and above follow this behavior.)");
    ...
    {
        Site.Assume.IsTrue(Common.IsRequirementEnabled(3374, Site), "[In Appendix A: Product Behavior] The implementation does not return a protocol status error in response to such a command request [more than one airsync:MIMESupport element in a Search command request]. (Exchange 2007 and above follow this behavior.)");
    ...
    {
        Site.Assume.IsTrue(Common.IsRequirementEnabled(3723, Site), "[In Appendix A: Product Behavior] The implementation does not return a protocol status error in response to such a command request [more than one Range element in a Search command request is undefined]. (Exchange 2007 and above follow this behavior.)");
    ...
    {
        Site.Assume.IsTrue(Common.IsRequirementEnabled(3747, Site), "[In Appendix A: Product Behavior] The implementation does not return a protocol status error in response to such a command request [more than one RebuildResults element in a Search command request is undefined]. (Exchange 2007 and above follow this behavior.)");
    ...
    {
        Site.Assume.IsTrue(Common.IsRequirementEnabled(4535, Site), "[In Appendix A: Product Behavior] The implementation does not return a protocol status error in response to such a command request [[more than one Store element in a Search command request is undefined]. (Exchange 2007 and above follow this behavior.)");
    ...
    public void MSASCMD_S14_TC11_Search_Invalid_Status2()
    {
        ...
        var user1EmailAddress = Common.GetMailAddress(User1Information.UserName, User1Information.UserDomain);
        var emailClassElement = "<To xmlns=\"Email\">" + user1EmailAddress + "</To>";
        ...
        var invalidSearchRequest = CreateInvalidSearchRequest(emailClassElement, "</And>");
        #endregion
        ...
        var statusCode = Common.GetSearchStatusCode(response);
    ...
    public void MSASCMD_S14_TC12_Search_Result_NoItemMatched()
    {
        ...
        var keyword = Guid.NewGuid().ToString().Substring(0, 5);
        var emailSubject = Common.GenerateResourceName(Site, keyword);
        var responseSendMail = SendPlainTextEmail(null, emailSubject, User1Information.UserName, User2Information.UserName, null);
        ...
        var searchResponse = CMDAdapter.Search(searchRequest);
        Site.Assert.AreEqual("1", searchResponse.ResponseData.Response.Store.Status, "If server successfully completed command, server should return status 1");
        var statusCode = searchResponse.ResponseData.Status;
        var doc = new XmlDocument();
    ...
    public void MSASCMD_S14_TC13_Search_Result_MultipleMatchedItems()
    {
        ...
        var keyWord = Guid.NewGuid().ToString().Substring(0, 5);
        uint mailIndex = 1;
        var emailSubject = keyWord + Common.GenerateResourceName(Site, "search", mailIndex);
        var responseSendMail = SendPlainTextEmail(null, emailSubject, User1Information.UserName, User2Information.UserName, null);
        Site.Assert.AreEqual(string.Empty, responseSendMail.ResponseDataXML, "If SendMail command executes successfully, server should return empty xml data");
        mailIndex++;
        var emailSubject2 = keyWord + Common.GenerateResourceName(Site, "search", mailIndex);
        var responseSendMail2 = SendPlainTextEmail(null, emailSubject2, User1Information.UserName, User2Information.UserName, null);
        ...
        SwitchUser(User2Information);
        GetMailItem(User2Information.InboxCollectionId, emailSubject);
        GetMailItem(User2Information.InboxCollectionId, emailSubject2);
        RecordCaseRelativeItems(User2Information, User2Information.InboxCollectionId, emailSubject, emailSubject2);
        #endregion
        ...
        {
            Thread.Sleep(waitTime);
            searchResponse = CMDAdapter.Search(searchRequest);
            Site.Assert.AreEqual("1", searchResponse.ResponseData.Response.Store.Status, "If server successfully completed command, server should return status 1");
        ...
        Site.CaptureRequirementIfIsTrue(
            searchResponse.ResponseData.Response.Store.Status.Equals("1") && resultsCount == 2 && searchResponse.ResponseData.Response.Store.Result[0].CollectionId.Equals(User2Information.InboxCollectionId) && searchResponse.ResponseData.Response.Store.Result[1].CollectionId.Equals(User2Information.InboxCollectionId),
            3849,
        ...
        Site.CaptureRequirementIfIsTrue(
            searchResponse.ResponseData.Response.Store.Result[0].CollectionId.Equals(User2Information.InboxCollectionId) && searchResponse.ResponseData.Response.Store.Result[0].Properties.ItemsElementName.Length > 0,
            3851,
    ...
    public void MSASCMD_S14_TC14_Search_GAL_Success()
    {
        ...
        var rangeEndIndex = 2;
        var requestRange = "0-" + rangeEndIndex.ToString();

        // Set search query.
        var keyWord = "MSASCMD_User";
        var searchRequest = CreateSearchGALRequest(0, 0, requestRange, keyWord);
        #endregion
        ...
        var searchResultCount = searchResponse.ResponseData.Response.Store.Result.Length;

        var hasMobilePhoneElement = false;
        var hasPhoneElement = false;
        var hasTitleElement = false;
        var hasOfficeElement = false;
        var hasLastNameElement = false;
        var hasFirstNameElement = false;
        var hasHomePhoneElement = false;
        var hasAliasElement = false;
        var hasCompanyElement = false;
        var hasDisplayNameElement = false;
        var hasEmailAddressElement = false;

        foreach (var result in searchResponse.ResponseData.Response.Store.Result)
        {
            if (result.Properties != null)
            {
                // Verify optional property values.
                foreach (var itemElementName in result.Properties.ItemsElementName)
                {
        ...
        var displayName = GetElementsFromSearchResponse(searchResponse, Response.ItemsChoiceType6.DisplayName1);
        var alias = GetElementsFromSearchResponse(searchResponse, Response.ItemsChoiceType6.Alias1);
        ...
        Site.CaptureRequirementIfIsTrue(
            User1Information.UserName.StartsWith(keyWord, StringComparison.OrdinalIgnoreCase),
            421,
        ...
        var compareResult = string.Compare((string)displayName[0], (string)displayName[1], StringComparison.CurrentCulture);
        var displayNameInOrder = true;
        for (var displayNameIndex = 1; displayNameIndex < rangeEndIndex; displayNameIndex++)
        {
            var otherCompareResult = string.Compare((string)displayName[displayNameIndex], (string)displayName[displayNameIndex + 1], StringComparison.CurrentCulture);
    ...
    {
        Site.Assume.IsTrue(Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site) == "14.1" || Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site) == "14.0", "[In Appendix A: Product Behavior] <14> Section 2.2.3.27.4: The SMS and Notes classes are only available if the MS-ASProtocolVersion header is set to 14.0 [or 14.1].");

        #region Create one item in User2 Inbox folder, Calendar folder, Contacts subfolder
        var searchPrefix = "keyWord" + ClientId;

        // Respectively create one item in User2 Inbox folder, Calendar folder and Contacts sub-folder
        CreateItemsWithKeyword(searchPrefix);
        #endregion
        ...
        {
            searchResponse = LoopSearch(searchRequest, 3);
        }
        #endregion
        ...
        var isAllResultContainClass = false;
        foreach (var result in searchResponse.ResponseData.Response.Store.Result)
        {
    ...
    {
        Site.Assume.AreNotEqual<string>("12.1", Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site), "Notes, SMS class is not supported in ProtocolVersion 12.1");
        ...
        if (searchResponse.ResponseData.Response.Store.Status != "1" || searchResponse.ResponseData.Response.Store.Result.Length != 3)
        {
            searchResponse = LoopSearch(searchRequest);
        }
        #endregion
    }
    ...
    public void MSASCMD_S14_TC17_Search_WithoutClass()
    {
        ...
        #region Call search command
        var searchResponse = LoopSearch(searchRequest);
        #endregion

        var isClassInResultIsServerSupported = false;
    ...
    public void MSASCMD_S14_TC18_Search_DocumentLibraryFolder()
    {
        ...
        var rootFolder = Common.GetConfigurationPropertyValue("SharedFolder", Site);
        var searchRequest = CreateSearchDocumentLibraryRequest(rootFolder, User1Information.UserName, User1Information.UserPassword);
        #endregion
        ...
        var resultCount = searchResponse.ResponseData.Response.Store.Result.Length;
        var firstResultContainsFolderInfo = false;
        var secondResultContainsContentInfo = false;
        if (resultCount >= 2)
        {
            var firstResult = searchResponse.ResponseData.Response.Store.Result[0];
            var isFolder = (byte)GetItemFromSearchResult(firstResult, Response.ItemsChoiceType6.IsFolder);
            if (isFolder == 1)
            {
            ...
            var secondResult = searchResponse.ResponseData.Response.Store.Result[1];
            isFolder = (byte)GetItemFromSearchResult(secondResult, Response.ItemsChoiceType6.IsFolder);
            var linkID = (string)GetItemFromSearchResult(secondResult, Response.ItemsChoiceType6.LinkId);
            var contentType = (string)GetItemFromSearchResult(secondResult, Response.ItemsChoiceType6.ContentType);
    ...
    public void MSASCMD_S14_TC19_Search_DocumentLibraryItem()
    {
        ...
        var sharedItem = Common.GetConfigurationPropertyValue("SharedDocument1", Site);
        var searchRequest = CreateSearchDocumentLibraryRequest(sharedItem, User1Information.UserName, User1Information.UserPassword);
        #endregion
        ...
        var resultCount = searchResponse.ResponseData.Response.Store.Result.Length;
        var linkID = (string)GetItemFromSearchResult(searchResponse.ResponseData.Response.Store.Result[0], Response.ItemsChoiceType6.LinkId);
    ...
    {
        Site.Assume.IsTrue(Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site) != "12.1" && Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site) != "14.0", "Status value 174 is not returned when the MS-ASProtocolVersion header value is set to 12.1 or 14.0.");
        #region Create Search request with options
        ...
        // Set search query.
        var keyWord = "MSASCMD_User";
        var searchRequest = CreateSearchGALRequest(maxPictures, maxSize, requestRange, keyWord);
        #endregion
        ...
        var isStatus174Contains = false;
        var picturesFromResponse = GetElementsFromSearchResponse(searchResponse, Response.ItemsChoiceType6.Picture1);
        foreach (var item in picturesFromResponse)
        {
            var itemPicture = (Response.SearchResponseStoreResultPropertiesPicture)item;
    ...
    {
        Site.Assume.IsTrue(Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site) != "12.1" && Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site) != "14.0", "Status value 175 is not returned when the MS-ASProtocolVersion header value is set to 12.1 or 14.0.");
        #region Create Search request with options
        ...
        var containsStatus175 = false;
        var validStatusValue = true;
        var picturesFromResponse = GetElementsFromSearchResponse(searchResponse, Response.ItemsChoiceType6.Picture1);
        foreach (var item in picturesFromResponse)
        {
            var itemPicture = (Response.SearchResponseStoreResultPropertiesPicture)item;
    ...
    public void MSASCMD_S14_TC22_Search_WithDeepTraversal()
    {
        ...
        var searchPrefix = "keyWord" + ClientId;
        var emailSubject1 = searchPrefix + Common.GenerateResourceName(Site, "subject1");
        var emailSubject2 = searchPrefix + Common.GenerateResourceName(Site, "subject2");
        ...
        SwitchUser(User2Information);
        var emailItemOneServerID = GetItemServerIdFromSpecialFolder(User2Information.InboxCollectionId, emailSubject1);
        GetItemServerIdFromSpecialFolder(User2Information.InboxCollectionId, emailSubject2);
        RecordCaseRelativeItems(User2Information, User2Information.InboxCollectionId, emailSubject1, emailSubject2);

        // User2 creates one subfolder in Inbox folder.
        var folderID = CreateFolder((byte)FolderType.UserCreatedMail, Common.GenerateResourceName(Site, "FolderCreate"), User2Information.InboxCollectionId);
        RecordCaseRelativeFolders(User2Information, folderID);

        // User2 moves the email with emailSubject1 into new subfolder.
        var moveItemRequest = CreateMoveItemsRequest(emailItemOneServerID, User2Information.InboxCollectionId, folderID);
        var moveItemResponse = CMDAdapter.MoveItems(moveItemRequest);
        ...
        GetMailItem(folderID, emailSubject1);
        ...
        var query = new Request.queryType();
        var storeName = SearchName.Mailbox.ToString();
        ...
        {
            searchResponse = LoopSearch(searchRequest, 2);
        }
        #endregion

        var subjectInSearchResult = GetElementsFromSearchResponse(searchResponse, Response.ItemsChoiceType6.Subject1);
        var resultContainsAllEmail = false;
        var findEmailCount = 0;
        for (var listIndex = 0; listIndex < subjectInSearchResult.Count; listIndex++)
        {
            var subject = (string)subjectInSearchResult[listIndex];
    ...
    public void MSASCMD_S14_TC23_Search_WithoutDeepTraversal()
    {
        ...
        var searchResponseWithOutDeepTraversal = LoopSearch(searchRequest);
        #endregion

        var subjectInSearchResult = GetElementsFromSearchResponse(searchResponseWithOutDeepTraversal, Response.ItemsChoiceType6.Subject1);
        var resultCotainsEmail1 = false;
        var resultCotainsEmail2 = false;

        foreach (var result in subjectInSearchResult)
        {
            var subject = (string)result;
    ...
    public void MSASCMD_S14_TC24_Search_DocumentLibraryStatus5()
    {
        ...
        var searchRequest = CreateSearchDocumentLibraryRequest(rootFolder, User2Information.UserName, User2Information.UserPassword);
        #endregion
    ...
    public void MSASCMD_S14_TC25_Search_DocumentLibraryStatus6()
    {
        ...
        var invalidRootFolder = "\\\\" + Common.GetConfigurationPropertyValue("SutComputerName", Site) + "\\" + "InvalidFolder";
        var searchRequest = CreateSearchDocumentLibraryRequest(invalidRootFolder, User1Information.UserName, User1Information.UserPassword);
        #endregion
    ...
    {
        Site.Assume.IsTrue(Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site) != "12.1" && Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site) != "14.0", "Status value 173 is not returned when the MS-ASProtocolVersion header value is set to 12.1 or 14.0.");
        ...
        var isContactPictureElementContainsData = false;
        var isContactPhotoLessThanMaxSize = false;
        var isPictureContainDataWithStatus1 = false;
        var isContactWithoutPhotoWithStatus173 = false;
        var contactWithPhotoCount = 0;
        ...
        foreach (var item in picturesfromResponse)
        {
            ...
            if (itemPicture.Data != null)
            {
                ...
                var pictureSize = Convert.ToInt32(itemPicture.Data);
        ...
        var isAllResultHasProperty = false;
        var isNotAllResultHasSamePropertyLength = false;

        var item0ProperyElementCount = searchResponse.ResponseData.Response.Store.Result[0].Properties.Items.Length;
        foreach (var result in searchResponse.ResponseData.Response.Store.Result)
        {
    ...
    {
        Site.Assume.IsTrue(Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site).Equals("14.1") || Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site).Equals("14.0"), "The ConversationId element is not supported when the MS-ASProtocolVersion header is set to 12.1");
        ...
        var conversationIdElement = "<ConversationId><![CDATA[ BBA4726D4399D44C83297D4BD904ED2D]]></ConversationId>";

        // Insert conversationID element before And element.
        var originalXmlRequest = searchRequest.GetRequestDataSerializedXML();

        // Insert element before tag
        var invalidSearchRequest = originalXmlRequest.Insert(originalXmlRequest.IndexOf("<And", StringComparison.OrdinalIgnoreCase), conversationIdElement);
        #endregion
    ...
    {
        #region User1 calls SendMail command to send 2 email messages to user2.
        var upperCaseSearchKeyword = "KEYWORD" + ClientId;
        uint mailIndex = 1;
        var emailSubject1 = upperCaseSearchKeyword + Common.GenerateResourceName(Site, "search", mailIndex);
        var responseSendMail = SendPlainTextEmail(null, emailSubject1, User1Information.UserName, User2Information.UserName, null);
        Site.Assert.AreEqual(string.Empty, responseSendMail.ResponseDataXML, "If SendMail command executes successfully, server should return empty xml data");
        mailIndex++;
        var emailSubject2 = upperCaseSearchKeyword + Common.GenerateResourceName(Site, "search", mailIndex);
        var responseSendMail2 = SendPlainTextEmail(null, emailSubject2, User1Information.UserName, User2Information.UserName, null);
        Site.Assert.AreEqual(string.Empty, responseSendMail2.ResponseDataXML, "If SendMail command executes successfully, server should return empty xml data");
        #endregion
        ...
        // Switch to user2 mailbox
        SwitchUser(User2Information);
        GetMailItem(User2Information.InboxCollectionId, emailSubject1);
        GetMailItem(User2Information.InboxCollectionId, emailSubject2);
        RecordCaseRelativeItems(User2Information, User2Information.InboxCollectionId, emailSubject1, emailSubject2);
        #endregion
        ...
        #region Call search command
        var searchResponse = LoopSearch(searchRequest, 2);
        #endregion
        ...
        var subjectInSearchResult = (string)GetItemFromSearchResult(searchResponse.ResponseData.Response.Store.Result[0], Response.ItemsChoiceType6.Subject1);
    ...
    {
        Site.Assume.IsTrue(Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site) == "14.1" || Common.GetConfigurationPropertyValue("ActiveSyncProtocolVersion", Site) == "14.0", "[In Appendix A: Product Behavior] <23> Section 2.2.3.35.2: The ConversationId element is not supported when the MS-ASProtocolVersion header is set to 12.1.");
        ...
        if (Common.IsRequirementEnabled(5815, Site))
        {
            // Add the debug information
    ...
    public void MSASCMD_S14_TC31_Search_TooComplex_Status8_GreaterThan()
    {
        ...
        var greaterThanElement = "<GreaterThan><DateReceived  xmlns=\"Email\"/><Value>2012-03-28T07:31:09.314Z</Value></GreaterThan>";

        // Insert GreaterThan element before And element
        var invalidSearchRequest = CreateInvalidSearchRequest(greaterThanElement, "<And>");
        #endregion
    ...
    public void MSASCMD_S14_TC32_Search_TooComplex_Status8_LessThan()
    {
        ...
        var lessThanElement = "<LessThan><DateReceived  xmlns=\"Email\"/><Value>50</Value></LessThan>";

        // Insert LessThan element before And element
        var invalidSearchRequest = CreateInvalidSearchRequest(lessThanElement, "<And>");
        #endregion
    ...
    public void MSASCMD_S14_TC33_Search_MIMESupport()
    {
        ...
        var searchRequest = CreateSearchRequestWithMimeSupport(subject, mimeSupportValue);
        var searchResponse = LoopSearch(searchRequest);

        var containBodyElement = FindBodyElementInSearchResponse(searchResponse);
        ...
        mimeSupportValue = 2;
        var newSearchRequest = CreateSearchRequestWithMimeSupport(subject, mimeSupportValue);
        var newSearchResponse = LoopSearch(newSearchRequest);
        ...
        var bodyElement = (Response.Body)GetItemFromSearchResult(newSearchResponse.ResponseData.Response.Store.Result[0], Response.ItemsChoiceType6.Body);
    ...
    {
        for (var index = 0; index < searchResult.Properties.ItemsElementName.Length; index++)
        {
    ...
    private static bool FindBodyElementInSearchResponse(SearchResponse searchResponse)
    {
        ...
        foreach (var result in searchResponse.ResponseData.Response.Store.Result)
        {
            var containBodyElement = false;
            for (var index = 0; index < result.Properties.ItemsElementName.Length; index++)
            {
    ...
    private static List<object> GetElementsFromSearchResponse(SearchResponse searchResponse, Response.ItemsChoiceType6 elementType)
    {
        ...
        foreach (var result in searchResponse.ResponseData.Response.Store.Result)
        {
            for (var itemIndex = 0; itemIndex < result.Properties.ItemsElementName.Length; itemIndex++)
            {
    ...
    private void CreateItemsWithKeyword(string searchPrefix)
    {
        ...
        var meetingRequestSubject = searchPrefix + Common.GenerateResourceName(Site, "subject");
        ...
        GetMailItem(User2Information.CalendarCollectionId, meetingRequestSubject);
        ...
        var contactSubfolderID = CreateFolder((byte)FolderType.UserCreatedContacts, Common.GenerateResourceName(Site, "FolderCreate"), User2Information.ContactsCollectionId);
        FolderSync();
        RecordCaseRelativeFolders(User2Information, contactSubfolderID);

        var contactFileAS = searchPrefix + Common.GenerateResourceName(Site, "Contact");
        var addData = CreateAddContactCommand("FirstName", "MiddleName", "LastName", contactFileAS, null);

        GetInitialSyncResponse(contactSubfolderID);
        var syncRequest = CreateSyncAddRequest(LastSyncKey, contactSubfolderID, addData);
        Sync(syncRequest);
        FolderSync();
        SyncChanges(contactSubfolderID);
    }
    ...
    private string CreateInvalidSearchRequest(string element, string insertTag)
    {
        ...
        var invalidSearchRequest = originalXmlRequest.Insert(originalXmlRequest.IndexOf(insertTag, StringComparison.OrdinalIgnoreCase), element);
    ...
    private SearchRequest CreateSearchGALRequest(uint maxPictures, uint maxSize, string requestRange, string keyWord)
    {
        ...
        {
            var picture = new Request.OptionsPicture
            {
