...
public class S20_ValidateCert : TestSuiteBase
{
    ...
    public void MSASCMD_S20_TC01_ValidateCert_Success()
    {
        ...
        var emailSubject = Common.GetConfigurationPropertyValue("MIMEMailSubject", Site);
        ...
        var specifiedString = "MIME-Version: 1.0";
        var body = mailBody.Data.Substring(
        ...
        var validateCert = new Request.ValidateCert
        {
            CheckCrl = "1",
            Certificates = new byte[][] { System.Text.Encoding.Default.GetBytes(body) }
        };

        var validateRequest = new ValidateCertRequest { RequestData = validateCert };

        var validateResponse = CMDAdapter.ValidateCert(validateRequest);

        var status = GetValidateCertStatusCode(validateResponse);
    ...
    public void MSASCMD_S20_TC03_ValidateCert_LimitingSize()
    {
        ...
        for (var i = 0; i < validateCert.Certificates.Length; i++)
        {
